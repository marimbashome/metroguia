...
export default function LineBaltimorePage({ params }) {
  ...
  return (
    <main style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
      <IntlLineSchema line={(lineasDetalleBaltimore.find(l => l.id === params.id)) || null} stations={((lineasDetalleBaltimore.find(l => l.id === params.id) || {}).estaciones || []).map(s => (typeof s === 'string' ? (estacionesBaltimore.find(x => x.slug === s) || { slug: s, nombre: s }) : s))} city="Baltimore" citySlug="baltimore" linePathPrefix="/baltimore/line" stationPathPrefix="/baltimore/station" country="US" systemName="Baltimore Metro SubwayLink" locale="en" />
      {/* HERO */}
