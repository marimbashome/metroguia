...
export async function generateMetadata({ params }) {
  const estacion = estacionesBaltimore.find((e) => e.slug === params.slug);
  if (!estacion) {
    return {
  ...
  return {
    ...
    openGraph: {
      title: estacion.seo_title,
      description: estacion.meta_description,
      url: `https://metroguia.mx/baltimore/station/${estacion.slug}`,
    },
  };
}

export default function StationBaltimorePagePage({ params }) {
  const estacion = estacionesBaltimore.find((e) => e.slug === params.slug);
  ...
  const transitStationSchema = {
    ...
    address: { '@type': 'PostalAddress', addressLocality: 'Baltimore', addressRegion: estacion.municipio || 'Maryland', addressCountry: 'US' },
    geo: { '@type': 'GeoCoordinates', latitude: estacion.lat || 39.2904, longitude: estacion.lng || -76.6122 },
  ...
  const faqItems = [];
  faqItems.push({ '@type': 'Question', name: `How do I get to ${estacion.nombre} station?`, acceptedAnswer: { '@type': 'Answer', text: `${estacion.nombre} is served by the MTA Maryland Metro ${lineaId} Line. Use the MetroGuia trip planner for directions.` } });
  ...
  faqItems.push({ '@type': 'Question', name: `What are the hours and fares?`, acceptedAnswer: { '@type': 'Answer', text: `MTA Maryland Metro operates 5:00 AM to 12:30 AM. Fare: $2.00 with CharmCard.` } });
  ...
  return (
    <main style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
      ...
      {/* HERO */}
      <section style={{ background: `linear-gradient(135deg, ${colorLinea} 0%, ${colorLinea}cc 100%)`, color: colorLinea === '#FFB81C' ? '#000' : '#fff', padding: '60px 24px' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          <div style={{ marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '12px' }}>
            <span style={{ width: '48px', height: '48px', backgroundColor: colorLinea === '#FFB81C' ? 'rgba(0,0,0,0.1)' : 'rgba(255,255,255,0.3)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '28px', fontWeight: '700' }}>
              {lineaId[0]}
            </span>
            <span style={{ fontSize: '1rem', fontWeight: '500' }}>{estacion.municipio || 'Baltimore'}</span>
          </div>
          ...
          <div>
            ...
            <div style={{ backgroundColor: 'var(--surface)', padding: '24px', borderRadius: 'var(--radius)', borderTop: `4px solid var(--warning)` }}>
              ...
              <p style={{ fontSize: '0.9rem', color: 'var(--text-muted)', margin: '0 0 12px 0', lineHeight: '1.5' }}>
                Use CharmCard for fastest boarding or buy a single ticket online.
              </p>
              <Link href="https://www.mta.maryland.gov/" target="_blank" style={{ textDecoration: 'none' }}>
                <button style={{ width: '100%', padding: '12px', backgroundColor: colorLinea, color: colorLinea === '#FFB81C' ? '#000' : '#fff', border: 'none', borderRadius: 'var(--radius)', fontSize: '0.95rem', fontWeight: '700', cursor: 'pointer' }}>
                  Get CharmCard
                </button>
              </Link>
            </div>
