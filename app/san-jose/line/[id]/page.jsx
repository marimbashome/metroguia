...
export async function generateStaticParams() {
  return lineasSanJose.map((linea) => ({
    id: linea.id,
  }));
}
...
export async function generateMetadata({ params }) {
  ...
  return {
    title: `VTA Light Rail ${linea.colorNombre} Line — San Jose | MetroGuia`,
    description: linea.descripcion.substring(0, 160),
    openGraph: {
      title: `VTA Light Rail ${linea.colorNombre} Line`,
      description: linea.descripcion.substring(0, 160),
      url: `https://metroguia.mx/san-jose/line/${linea.id}`,
    },
  };
}

export default function LineSanJosePage({ params }) {
  const linea = lineasSanJose.find((l) => l.id === params.id);
  ...
  return (
    <main style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
      <IntlLineSchema line={(lineasSanJose.find(l => l.id === params.id)) || null} stations={((lineasSanJose.find(l => l.id === params.id) || {}).estaciones || []).map(s => (typeof s === 'string' ? (estacionesSanJose.find(x => x.slug === s) || { slug: s, nombre: s }) : s))} city="San Jose" citySlug="san-jose" linePathPrefix="/san-jose/line" stationPathPrefix="/san-jose/station" country="US" systemName="VTA Light Rail" locale="en" />
      {/* HERO */}
