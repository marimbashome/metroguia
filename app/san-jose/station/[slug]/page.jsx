...
const LINE_COLORS = {"blue":"#0055A4","green":"#00A651","orange":"#ED8B00"};

export async function generateStaticParams() {
  return estacionesSanJose.map((estacion) => ({
    slug: estacion.slug,
  }));
}
...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    openGraph: {
      title: estacion.seo_title,
      description: estacion.meta_description,
      url: `https://metroguia.mx/san-jose/station/${estacion.slug}`,
    },
  };
}

export default function StationSanJosePage({ params }) {
  const estacion = estacionesSanJose.find((e) => e.slug === params.slug);
  ...
  if (!estacion) {
    return (
      <main style={{ padding: '80px 24px', textAlign: 'center', backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
        ...
        <Link href="/san-jose">
          <button style={{ marginTop: '24px', padding: '12px 24px', backgroundColor: '#008752', color: '#fff', border: 'none', borderRadius: 'var(--radius)', fontSize: '1rem', fontWeight: '700', cursor: 'pointer' }}>
            Back to San Jose
          </button>
        </Link>
      </main>
  ...
  const colorLinea = LINE_COLORS[lineaArray[0]] || '#008752';
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: `Line ${lineaId}`, item: `https://metroguia.mx/san-jose/line/${lineaId}/` },
      { '@type': 'ListItem', position: 4, name: estacion.nombre, item: `https://metroguia.mx/san-jose/station/${estacion.slug}/` },
    ]
  };
  ...
  const stationSchema = {
    ...
    address: { '@type': 'PostalAddress', addressLocality: 'San Jose', addressRegion: 'California', addressCountry: 'US' },
  };
      ...
      <section style={{ background: `linear-gradient(135deg, ${colorLinea} 0%, ${colorLinea}cc 100%)`, color: '#fff', padding: '60px 24px' }}>
        <div style={{ maxWidth: '1000px', margin: '0 auto' }}>
          <div style={{ marginBottom: '8px' }}>
            <Link href="/san-jose" style={{ color: 'rgba(255,255,255,0.8)', textDecoration: 'none', fontSize: '0.9rem' }}>
              ← San Jose
            </Link>
          </div>
          <h1 style={{ fontSize: '2.5rem', fontWeight: '800', margin: '0 0 8px 0' }}>{estacion.nombre}</h1>
          <p style={{ fontSize: '1.125rem', opacity: 0.9, margin: 0 }}>
            {lineaArray.map(l => `${l.charAt(0).toUpperCase() + l.slice(1)} Line`).join(' · ')} — VTA Light Rail
          </p>
        </div>
      </section>
      ...
      <section style={{ padding: '60px 24px' }}>
        <div style={{ maxWidth: '800px', margin: '0 auto' }}>
          ...
          <div style={{ marginTop: '48px', textAlign: 'center' }}>
            <Link href={`/san-jose/line/${lineaId}`}>
