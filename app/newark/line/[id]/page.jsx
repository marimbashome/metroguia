...
export async function generateMetadata({ params }) {
  ...
  return {
    title: `NJ Transit ${linea.colorNombre} Line — Newark | MetroGuia`,
    description: linea.descripcion.substring(0, 160),
    openGraph: {
      title: `NJ Transit ${linea.colorNombre} Line`,
      description: linea.descripcion.substring(0, 160),
      url: `https://metroguia.mx/newark/line/${linea.id}`,
    },
  };
}

export default function LineNewarkPage({ params }) {
  const linea = lineasNewark.find((l) => l.id === params.id);
  ...
  if (!linea) {
    return (
      <main style={{ padding: '80px 24px', textAlign: 'center', backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
        ...
        <Link href="/newark">
          <button style={{ marginTop: '24px', padding: '12px 24px', backgroundColor: '#FF3333', color: '#fff', border: 'none', borderRadius: 'var(--radius)', fontSize: '1rem', fontWeight: '700', cursor: 'pointer' }}>
            Back to Newark
          </button>
        </Link>
      </main>
  ...
  return (
    <main style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
      <IntlLineSchema line={(lineasNewark.find(l => l.id === params.id)) || null} stations={((lineasNewark.find(l => l.id === params.id) || {}).estaciones || []).map(s => (typeof s === 'string' ? (estacionesNewark.find(x => x.slug === s) || { slug: s, nombre: s }) : s))} city="Newark" citySlug="newark" linePathPrefix="/newark/line" stationPathPrefix="/newark/station" country="US" systemName="Newark Light Rail" locale="en" />
      <section style={{ background: `linear-gradient(135deg, ${linea.color} 0%, ${linea.color}cc 100%)`, color: '#fff', padding: '60px 24px' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          <div style={{ marginBottom: '8px' }}>
            <Link href="/newark" style={{ color: 'rgba(255,255,255,0.8)', textDecoration: 'none', fontSize: '0.9rem' }}>
              ← Newark
            </Link>
          </div>
