...
const LINE_COLORS = {"red":"#FF3333"};

export async function generateStaticParams() {
  return estacionesNewark.map((estacion) => ({
    slug: estacion.slug,
  }));
}
...
export default function StationNewarkPage({ params }) {
  const estacion = estacionesNewark.find((e) => e.slug === params.slug);
  ...
  const colorLinea = LINE_COLORS[lineaArray[0]] || '#FF3333';
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: `Line ${lineaId}`, item: `https://metroguia.mx/newark/line/${lineaId}/` },
      { '@type': 'ListItem', position: 4, name: estacion.nombre, item: `https://metroguia.mx/newark/station/${estacion.slug}/` },
    ]
  };
  ...
  const stationSchema = {
    ...
    address: { '@type': 'PostalAddress', addressLocality: 'Newark', addressRegion: 'New Jersey', addressCountry: 'US' },
  };
      ...
      <section style={{ background: `linear-gradient(135deg, ${colorLinea} 0%, ${colorLinea}cc 100%)`, color: '#fff', padding: '60px 24px' }}>
        <div style={{ maxWidth: '1000px', margin: '0 auto' }}>
          ...
          <p style={{ fontSize: '1.125rem', opacity: 0.9, margin: 0 }}>
            {lineaArray.map(l => `${l.charAt(0).toUpperCase() + l.slice(1)} Line`).join(' · ')} — NJ Transit
          </p>
        </div>
      </section>
