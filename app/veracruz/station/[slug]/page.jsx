...
export async function generateMetadata({ params }) {
  const estacion = estacionesVeracruz.find((e) => e.slug === params.slug);
  if (!estacion) return { title: 'Station not found' };
  const seoTitle = estacion.seo_title || `${estacion.nombre} Station — Veracruz Transit | MetroGuia`;
  const metaDesc = estacion.meta_description || estacion.descripcion_turistica || `Guide to ${estacion.nombre} station in Veracruz`;
  return {
    title: seoTitle,
    ...
    openGraph: {
      title: seoTitle,
      description: metaDesc,
      url: `https://metroguia.mx/veracruz/station/${estacion.slug}`,
      type: 'website',
    },
...
export default function StationVeracruzPage({ params }) {
  const estacion = estacionesVeracruz.find((e) => e.slug === params.slug);
  if (!estacion) {
    return (
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: `Line ${lineaId}`, item: `https://metroguia.mx/veracruz/line/${lineaId}/` },
      { '@type': 'ListItem', position: 4, name: estacion.h1 || estacion.nombre, item: `https://metroguia.mx/veracruz/station/${estacion.slug}/` }
    ]
  };
    ...
    <main style={{ backgroundColor: '#ffffff'}} >
      ...
      <section style={heroStyles}>
        <div style={containerStyles}>
          <p style={{ fontSize: '14px', marginBottom: '10px', opacity: '0.9'}} >
            Line {lineaId} • {estacion.municipio || 'Veracruz'}
          </p>
      ...
      <section style={{ ...containerStyles, paddingTop: '40px', paddingBottom: '40px', textAlign: 'center'}} >
        <div style={{ display: 'flex', gap: '10px', justifyContent: 'center', flexWrap: 'wrap'}} >
          ...
          {linea && <Link href={`/veracruz/line/${linea.id}`} style={buttonStyles}>View Line {linea.id}</Link>}
        </div>
      </section>
    </main>
  );
