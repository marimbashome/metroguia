...
export async function generateMetadata({ params }) {
  ...
  return {
    title: `O-Train ${linea.colorNombre} Line — Ottawa | MetroGuia`,
    description: linea.descripcion.substring(0, 160),
    openGraph: {
      title: `O-Train ${linea.colorNombre} Line`,
      description: linea.descripcion.substring(0, 160),
      url: `https://metroguia.mx/ottawa/line/${linea.id}`,
    },
  };
}

export default function LineOttawaPage({ params }) {
  const linea = lineasOttawa.find((l) => l.id === params.id);
  ...
  if (!linea) {
    return (
      <main style={{ padding: '80px 24px', textAlign: 'center', backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
        ...
        <Link href="/ottawa">
          <button style={{ marginTop: '24px', padding: '12px 24px', backgroundColor: '#CF142B', color: '#fff', border: 'none', borderRadius: 'var(--radius)', fontSize: '1rem', fontWeight: '700', cursor: 'pointer' }}>
            Back to Ottawa
          </button>
        </Link>
      </main>
  ...
  return (
    <main style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
      <IntlLineSchema line={(lineasOttawa.find(l => l.id === params.id)) || null} stations={((lineasOttawa.find(l => l.id === params.id) || {}).estaciones || []).map(s => (typeof s === 'string' ? (estacionesOttawa.find(x => x.slug === s) || { slug: s, nombre: s }) : s))} city="Ottawa" citySlug="ottawa" linePathPrefix="/ottawa/line" stationPathPrefix="/ottawa/station" country="CA" systemName="OC Transpo O-Train" locale="en" />
      {/* HERO */}
      <section style={{ background: `linear-gradient(135deg, ${linea.color} 0%, ${linea.color}cc 100%)`, color: '#fff', padding: '60px 24px' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          <div style={{ marginBottom: '8px' }}>
            <Link href="/ottawa" style={{ color: 'rgba(255,255,255,0.8)', textDecoration: 'none', fontSize: '0.9rem' }}>
              ← Ottawa
            </Link>
          </div>
