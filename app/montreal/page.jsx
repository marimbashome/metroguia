...
export const metadata = {
  title: 'STM Metro — Montreal Transit | MetroGuia',
  description: 'STM Metro guide: 4 lines, 72+ stations in Montreal. Complete trip planner, schedule & fares.',
  keywords: 'STM Metro, Montreal transit, Green Line, Orange Line, Blue Line, Yellow Line',
  openGraph: {
    title: 'STM Metro — Montreal Transit Guide',
    description: 'Navigate Montreal with STM Metro. Trip planner, fares, and station guide.',
    url: 'https://metroguia.mx/montreal',
    type: 'website',
  },
  robots: { index: true, follow: true },
};

const estacionesDestacadas = estacionesMontreal.filter(e =>
  ['angrignon', 'monk', 'jolicoeur', 'verdun'].includes(e.slug)
);
...
export default function MontrealPage() {
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'Montreal', item: 'https://metroguia.mx/montreal/' }
    ]
  };
  ...
  return (
    <main style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
      ...
      {/* HERO */}
      <section style={{
        background: 'linear-gradient(135deg, #0079C2 0%, #0079C2cc 100%)',
        color: '#FFFFFF',
        padding: '80px 24px',
        textAlign: 'center',
        position: 'relative',
        ...
        <div style={{ maxWidth: '1000px', marginLeft: 'auto', marginRight: 'auto' }}>
          ...
          <p style={{ fontSize: '1.125rem', fontWeight: '400', margin: '0', opacity: '0.9', maxWidth: '600px', marginLeft: 'auto', marginRight: 'auto' }}>
            Montreal's STM Metro — your guide to navigating the city by rail.
          </p>
        </div>
      </section>
      ...
      <section style={{ backgroundColor: 'var(--surface)', padding: '48px 24px', borderBottom: '1px solid var(--border)' }}>
        <div style={{ maxWidth: '1000px', marginLeft: 'auto', marginRight: 'auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '32px', textAlign: 'center' }}>
          <div>
            <p style={{ fontSize: '2.5rem', fontWeight: '800', color: '#0079C2', margin: '0 0 8px 0' }}>72+</p>
            <p style={{ fontSize: '1rem', color: 'var(--text-muted)', margin: '0', fontWeight: '500' }}>Stations</p>
          </div>
          <div>
            <p style={{ fontSize: '2.5rem', fontWeight: '800', color: '#0079C2', margin: '0 0 8px 0' }}>4</p>
            <p style={{ fontSize: '1rem', color: 'var(--text-muted)', margin: '0', fontWeight: '500' }}>Active Lines</p>
          </div>
          <div>
            <p style={{ fontSize: '2.5rem', fontWeight: '800', color: '#0079C2', margin: '0 0 8px 0' }}>3–5 min</p>
      ...
      <section style={{ backgroundColor: 'var(--bg)', padding: '60px 24px', borderBottom: '1px solid var(--border)' }}>
        <div style={{ maxWidth: '700px', marginLeft: 'auto', marginRight: 'auto' }}>
          <h2 style={{ fontSize: '1.75rem', fontWeight: '800', marginBottom: '12px', textAlign: 'center', color: 'var(--text)' }}>
            Plan Your Route in Montreal
          </h2>
          <p style={{ fontSize: '1rem', textAlign: 'center', marginBottom: '24px', color: 'var(--text-muted)' }}>
            Calculate the best route between STM Metro stations
          </p>
          <SearchBar ciudad="montreal" />
        </div>
      </section>
        ...
        <div style={{ maxWidth: '1000px', marginLeft: 'auto', marginRight: 'auto' }}>
          ...
          <div style={{ display: 'grid', gap: '24px' }}>
            {estacionesDestacadas.map((estacion, idx) => (
              <Link href={`/montreal/station/${estacion.slug}`} key={idx} style={{ textDecoration: 'none' }}>
                <div style={{ padding: '28px', backgroundColor: 'var(--surface)', borderRadius: 'var(--radius)', border: '1px solid var(--border)', cursor: 'pointer', transition: 'all 0.3s ease' }}>
                  <h3 style={{ fontSize: '1.25rem', fontWeight: '700', margin: '0 0 8px 0', color: '#0079C2' }}>
                    {estacion.nombre}
                  </h3>
      ...
      <section style={{ backgroundColor: 'var(--bg)', padding: '80px 24px', borderBottom: '1px solid var(--border)' }}>
        <div style={{ maxWidth: '1000px', marginLeft: 'auto', marginRight: 'auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '24px' }}>
            <AffiliateTransportCard icon="🎫" titulo="Single Ride" descripcion="One-way trip on STM Metro." precio="$3.75 CAD" enlace="#" />
      ...
      <section style={{ backgroundColor: 'var(--bg)', padding: '80px 24px' }}>
        <div style={{ maxWidth: '1000px', marginLeft: 'auto', marginRight: 'auto' }}>
          ...
          <div style={{ display: 'grid', gap: '20px' }}>
            {lineasMontreal.map((linea, idx) => (
              <Link href={`/montreal/line/${linea.id}`} key={idx} style={{ textDecoration: 'none' }}>
