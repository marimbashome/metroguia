...
export async function generateStaticParams() {
  return lineasNewOrleans.map((linea) => ({
    id: linea.id,
  }));
}
...
export async function generateMetadata({ params }) {
  ...
  return {
    title: `RTA Streetcar ${linea.colorNombre} Line — New Orleans | MetroGuia`,
    description: linea.descripcion.substring(0, 160),
    openGraph: {
      title: `RTA Streetcar ${linea.colorNombre} Line`,
      description: linea.descripcion.substring(0, 160),
      url: `https://metroguia.mx/new-orleans/line/${linea.id}`,
    },
  };
}

export default function LineNewOrleansPage({ params }) {
  const linea = lineasNewOrleans.find((l) => l.id === params.id);
  ...
  return (
    <main style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
      <IntlLineSchema line={(lineasNewOrleans.find(l => l.id === params.id)) || null} stations={((lineasNewOrleans.find(l => l.id === params.id) || {}).estaciones || []).map(s => (typeof s === 'string' ? (estacionesNewOrleans.find(x => x.slug === s) || { slug: s, nombre: s }) : s))} city="New Orleans" citySlug="new-orleans" linePathPrefix="/new-orleans/line" stationPathPrefix="/new-orleans/station" country="US" systemName="New Orleans Streetcar" locale="en" />
      {/* HERO */}
