...
export const metadata = {
  title: 'CTrain — Calgary Transit | MetroGuia',
  description: 'CTrain guide: 2 lines, 44+ stations in Calgary. Complete trip planner, schedule & fares.',
  keywords: 'CTrain, Calgary transit, Red Line, Blue Line',
  openGraph: {
    title: 'CTrain — Calgary Transit Guide',
    description: 'Navigate Calgary with CTrain. Trip planner, fares, and station guide.',
    url: 'https://metroguia.mx/calgary',
    type: 'website',
  },
  robots: { index: true, follow: true },
};

const estacionesDestacadas = estacionesCalgary.filter(e =>
  ['tuscany', 'crowfoot', 'dalhousie', 'brentwood'].includes(e.slug)
);
...
export default function CalgaryPage() {
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'Calgary', item: 'https://metroguia.mx/calgary/' }
    ]
  };
    ...
    <main style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
      ...
      <section style={{
        ...
        <div style={{ maxWidth: '1000px', marginLeft: 'auto', marginRight: 'auto' }}>
          ...
          <p style={{ fontSize: '1.125rem', fontWeight: '400', margin: '0', opacity: '0.9', maxWidth: '600px', marginLeft: 'auto', marginRight: 'auto' }}>
            Calgary's CTrain — your guide to navigating the city by rail.
          </p>
        </div>
      </section>
      ...
      <section style={{ backgroundColor: 'var(--surface)', padding: '48px 24px', borderBottom: '1px solid var(--border)' }}>
        <div style={{ maxWidth: '1000px', marginLeft: 'auto', marginRight: 'auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '32px', textAlign: 'center' }}>
          <div>
            <p style={{ fontSize: '2.5rem', fontWeight: '800', color: '#E31937', margin: '0 0 8px 0' }}>44+</p>
          ...
          <div>
            <p style={{ fontSize: '2.5rem', fontWeight: '800', color: '#E31937', margin: '0 0 8px 0' }}>5–10 min</p>
      ...
      <section style={{ backgroundColor: 'var(--bg)', padding: '60px 24px', borderBottom: '1px solid var(--border)' }}>
        <div style={{ maxWidth: '700px', marginLeft: 'auto', marginRight: 'auto' }}>
          <h2 style={{ fontSize: '1.75rem', fontWeight: '800', marginBottom: '12px', textAlign: 'center', color: 'var(--text)' }}>
            Plan Your Route in Calgary
          </h2>
          <p style={{ fontSize: '1rem', textAlign: 'center', marginBottom: '24px', color: 'var(--text-muted)' }}>
            Calculate the best route between CTrain stations
          </p>
          <SearchBar ciudad="calgary" />
        </div>
      </section>
        ...
        <div style={{ maxWidth: '1000px', marginLeft: 'auto', marginRight: 'auto' }}>
          ...
          <div style={{ display: 'grid', gap: '24px' }}>
            {estacionesDestacadas.map((estacion, idx) => (
              <Link href={`/calgary/station/${estacion.slug}`} key={idx} style={{ textDecoration: 'none' }}>
      ...
      <section style={{ backgroundColor: 'var(--bg)', padding: '80px 24px', borderBottom: '1px solid var(--border)' }}>
        <div style={{ maxWidth: '1000px', marginLeft: 'auto', marginRight: 'auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '24px' }}>
            <AffiliateTransportCard icon="🎫" titulo="Single Ride" descripcion="One-way trip on CTrain." precio="$3.60 CAD" enlace="#" />
            <AffiliateTransportCard icon="📱" titulo="Mobile Pass" descripcion="Digital ticket via mobile app." precio="$3.60 CAD" enlace="#" />
          </div>
        </div>
      </section>
      ...
      <section style={{ backgroundColor: 'var(--bg)', padding: '80px 24px' }}>
        <div style={{ maxWidth: '1000px', marginLeft: 'auto', marginRight: 'auto' }}>
          ...
          <div style={{ display: 'grid', gap: '20px' }}>
            {lineasCalgary.map((linea, idx) => (
              <Link href={`/calgary/line/${linea.id}`} key={idx} style={{ textDecoration: 'none' }}>
