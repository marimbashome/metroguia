...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    openGraph: {
      title: estacion.seo_title,
      description: estacion.meta_description,
      url: `https://metroguia.mx/tren-maya/estacion/${estacion.slug}`,
      type: 'website',
    },
    robots: {
      index: true,
...
export default function EstacionTrenMayaPage({ params }) {
  const estacion = estacionesTrenMaya.find((e) => e.slug === params.slug);
  if (!estacion) {
    return (
  ...
  const colorLinea = estacion.linea === 'T1' ? '#0EA5E9' : estacion.linea === 'T3' ? '#10B981' : estacion.linea === 'T4' ? '#EF4444' : estacion.linea === 'T5' ? '#F59E0B' : estacion.linea === 'T6' ? '#8B5CF6' : '#EC4899';
  const linea = lineasTrenMaya.find((l) => l.id === estacion.linea);
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      },
      {
        ...
        item: `https://metroguia.mx/tren-maya/linea/${estacion.linea}/`
      },
      {
        '@type': 'ListItem',
        position: 4,
        name: estacion.h1,
        item: `https://metroguia.mx/tren-maya/estacion/${estacion.slug}/`
      }
    ]
  }
    ...
    <main style={{ backgroundColor: '#ffffff' }}>
      ...
      <section style={{ ...containerStyles, paddingTop: '40px', paddingBottom: '40px' }}>
        ...
        <div style={sidebarStyles}>
          ...
          <ul style={{ fontSize: '14px', color: '#374151', lineHeight: '1.8', paddingLeft: '20px' }}>
            ...
            <li><strong>Tramo:</strong> {linea?.colorNombre}</li>
      ...
      <section style={{ ...containerStyles, paddingTop: '40px', paddingBottom: '40px', textAlign: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: '10px', justifyContent: 'center', flexWrap: 'wrap' }}>
          ...
          {linea && (
            <Link href={`/tren-maya/linea/${linea.id}`} style={buttonStyles}>
              Ver Tramo {linea.id}
            </Link>
          )}
        </div>
