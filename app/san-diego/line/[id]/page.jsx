...
export default function LineSanDiegoPage({ params }) {
  ...
  return (
    <main style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
      <IntlLineSchema line={(lineasDetalleSanDiego.find(l => l.id === params.id)) || null} stations={((lineasDetalleSanDiego.find(l => l.id === params.id) || {}).estaciones || []).map(s => (typeof s === 'string' ? (estacionesSanDiego.find(x => x.slug === s) || { slug: s, nombre: s }) : s))} city="San Diego" citySlug="san-diego" linePathPrefix="/san-diego/line" stationPathPrefix="/san-diego/station" country="US" systemName="San Diego Trolley" locale="en" />
      {/* HERO */}
