...
export async function generateMetadata({ params }) {
  ...
  return {
    title: linea.seo_title || `${linea.id} Line — Portland TriMet | MetroGuia`,
    description: linea.meta_description || linea.descripcion,
    openGraph: {
      title: linea.seo_title || `${linea.id} Line — Portland TriMet`,
      description: linea.meta_description || linea.descripcion,
      url: `https://metroguia.mx/portland/line/${linea.id}`,
    },
  };
}

export default function LinePortlandPage({ params }) {
  const linea = lineasPortland.find((l) => l.id === params.id);
  ...
  const estacionesLinea = estacionesPortland.filter((e) => {
    if (Array.isArray(e.linea)) return e.linea.includes(linea.id);
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: `${linea.colorNombre} Line`, item: `https://metroguia.mx/portland/line/${linea.id}` },
    ],
  };
  ...
  const routeSchema = {
    '@context': 'https://schema.org',
    '@type': 'Route',
    name: `${linea.colorNombre} Line — Portland TriMet`,
              ...
              {linea.ruta_1_dia.paradas.map((parada, idx) => {
                ...
                return (
                  <div key={idx} style={{ display: 'grid', gridTemplateColumns: '60px 1fr', gap: '24px', alignItems: 'start' }}>
                    ...
                    <div>
                      <Link href={`/portland/station/${parada.estacion}`}>
        ...
        <div style={{ marginBottom: '60px', marginTop: '40px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '16px' }}>
            {estacionesLinea.map((estacion, idx) => (
              <Link href={`/portland/station/${estacion.slug}`} key={idx} style={{ textDecoration: 'none' }}>
                <div style={{
                  padding: '20px',
