...
export async function generateStaticParams() {
  return estacionesNYC.map((estacion) => ({
    slug: estacion.slug,
  }));
}

export async function generateMetadata({ params }) {
  const estacion = estacionesNYC.find((e) => e.slug === params.slug);
  if (!estacion) {
    return {
  ...
  return {
    ...
    openGraph: {
      title: estacion.seo_title,
      description: estacion.meta_description,
      url: `https://metroguia.mx/nyc/station/${estacion.slug}`,
    },
  };
}

export default function StationNYCPage({ params }) {
  const estacion = estacionesNYC.find((e) => e.slug === params.slug);
  ...
  if (!estacion) {
    return (
      <main style={{ padding: '80px 24px', textAlign: 'center', backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
        ...
        <Link href="/nyc">
          <button style={{ marginTop: '24px', padding: '12px 24px', backgroundColor: '#EE352E', color: '#fff', border: 'none', borderRadius: 'var(--radius)', fontSize: '1rem', fontWeight: '700', cursor: 'pointer' }}>
            Back to NYC
          </button>
        </Link>
      </main>
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'NYC', item: 'https://metroguia.mx/nyc/' },
      { '@type': 'ListItem', position: 3, name: `Lines ${estacion.linea}`, item: `https://metroguia.mx/nyc/line/${lineas[0]}/` },
      { '@type': 'ListItem', position: 4, name: estacion.nombre, item: `https://metroguia.mx/nyc/station/${estacion.slug}` },
    ],
  };
  ...
  const transitStationSchema = {
    ...
    address: { '@type': 'PostalAddress', addressLocality: estacion.borough, addressRegion: 'NY', addressCountry: 'US' },
    geo: { '@type': 'GeoCoordinates', latitude: estacion.lat || 40.7580, longitude: estacion.lng || -73.9855 },
    openingHoursSpecification: { '@type': 'OpeningHoursSpecification', dayOfWeek: ['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday'], opens: '00:00', closes: '23:59' },
  };

  const faqItems = [];
  faqItems.push({ '@type': 'Question', name: `How do I get to ${estacion.nombre} station in NYC?`, acceptedAnswer: { '@type': 'Answer', text: `${estacion.nombre} station is served by Lines ${estacion.linea} of the MTA Subway system. Use MetroGuia.mx route planner for directions.` } });
  if (estacion.pois && estacion.pois.length > 0) {
    const poiList = estacion.pois.slice(0, 5).map(p => typeof p === 'string' ? p : `${p.nombre} (${p.tipo})`).join(', ');
    faqItems.push({ '@type': 'Question', name: `What's near ${estacion.nombre} station?`, acceptedAnswer: { '@type': 'Answer', text: `You'll find: ${poiList}.` } });
  }
  faqItems.push({ '@type': 'Question', name: `What's the fare and hours?`, acceptedAnswer: { '@type': 'Answer', text: `MTA Subway operates 24 hours. Fare: $2.90 USD per ride (OMNY or MetroCard).` } });
    ...
    <main style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
      ...
      <section style={{ background: `linear-gradient(135deg, ${colorLinea} 0%, ${colorLinea}cc 100%)`, color: '#fff', padding: '60px 24px' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          <div style={{ marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <span style={{ fontSize: '1rem', fontWeight: '500' }}>{estacion.borough}</span>
          </div>
      ...
      <section style={{ maxWidth: '1200px', margin: '0 auto', padding: '60px 24px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '48px' }}>
          ...
          <div>
            ...
            {/* WORLD CUP 2026 */}
            {estacion.mundial_relevancia && !estacion.mundial_relevancia.includes('No relevance') && (
          ...
          <div>
            ...
            <div style={{ backgroundColor: 'var(--surface)', padding: '24px', borderRadius: 'var(--radius)', marginBottom: '24px', borderTop: `4px solid ${colorLinea}` }}>
              ...
              <div style={{ display: 'grid', gap: '12px' }}>
                ...
                <div>
                  <p style={{ fontSize: '0.75rem', color: 'var(--text-dim)', margin: '0 0 4px 0' }}>Borough</p>
                  <p style={{ fontSize: '1rem', fontWeight: '700', margin: '0' }}>{estacion.borough}</p>
                </div>
                <div>
                  <p style={{ fontSize: '0.75rem', color: 'var(--text-dim)', margin: '0 0 4px 0' }}>Zone Type</p>
                  <p style={{ fontSize: '1rem', fontWeight: '700', margin: '0' }}>{(estacion.tipo_zona || '').replace('-', ' / ')}</p>
                </div>
                <div>
                  <p style={{ fontSize: '0.75rem', color: 'var(--text-dim)', margin: '0 0 4px 0' }}>Best Time to Visit</p>
                  <p style={{ fontSize: '1rem', fontWeight: '700', margin: '0' }}>{estacion.mejor_horario || 'Off-peak hours'}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p style={{ fontSize: '1rem', fontWeight: '700', margin: '0', color: 'var(--success)' }}>$2.90 USD</p>
                </div>
              </div>
            </div>
            ...
            <div style={{ marginBottom: '24px' }}>
              <AffiliateTransportCard ciudad="NYC" estacion={estacion.nombre} fechas={null} />
            </div>

            {/* NAVIGATION */}
            <Link href={`/nyc/line/${lineas[0]}`}>
      ...
      <section style={{ backgroundColor: 'var(--surface)', padding: '60px 24px', textAlign: 'center' }}>
        ...
        <p style={{ fontSize: '1rem', color: 'var(--text-muted)', margin: '0 0 24px 0', maxWidth: '600px', marginLeft: 'auto', marginRight: 'auto' }}>
          Find accommodations near this station and explore NYC's diverse neighborhoods.
        </p>
        <Link href="/hospedaje">
          <button style={{ padding: '14px 32px', backgroundColor: '#EE352E', color: '#fff', border: 'none', borderRadius: 'var(--radius)', fontSize: '1rem', fontWeight: '700', cursor: 'pointer' }}>
            Explore Lodging
          </button>
        </Link>
      </section>
