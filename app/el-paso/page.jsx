...
export const metadata = {
  title: 'Sun Link — El Paso Transit | MetroGuia',
  description: 'Sun Link guide: 1 line, 14+ stations in El Paso. Complete trip planner, schedule & fares.',
  keywords: 'Sun Link, El Paso transit, Sun Link Streetcar',
  openGraph: {
    title: 'Sun Link — El Paso Transit Guide',
    description: 'Navigate El Paso with Sun Link. Trip planner, fares, and station guide.',
    url: 'https://metroguia.mx/el-paso',
    type: 'website',
  },
  robots: { index: true, follow: true },
};

const estacionesDestacadas = estacionesElPaso.filter(e =>
  ['el-paso-downtown', 'el-paso-convention-center', 'el-paso-union-depot', 'el-paso-east-side'].includes(e.slug)
);
...
export default function ElPasoPage() {
  ...
  return (
    <main style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
      ...
      {/* HERO */}
      <section style={{
        background: 'linear-gradient(135deg, #FFB81C 0%, #FFB81Ccc 100%)',
        color: '#000000',
        padding: '80px 24px',
        textAlign: 'center',
        position: 'relative',
        ...
        <div style={{ maxWidth: '1000px', marginLeft: 'auto', marginRight: 'auto' }}>
          ...
          <p style={{ fontSize: '1.125rem', fontWeight: '400', margin: '0', opacity: '0.9', maxWidth: '600px', marginLeft: 'auto', marginRight: 'auto' }}>
            El Paso's Sun Link — your guide to navigating the city by rail.
          </p>
        </div>
      </section>
      ...
      <section style={{ backgroundColor: 'var(--surface)', padding: '48px 24px', borderBottom: '1px solid var(--border)' }}>
        <div style={{ maxWidth: '1000px', marginLeft: 'auto', marginRight: 'auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '32px', textAlign: 'center' }}>
          <div>
            <p style={{ fontSize: '2.5rem', fontWeight: '800', color: '#FFB81C', margin: '0 0 8px 0' }}>14+</p>
            <p style={{ fontSize: '1rem', color: 'var(--text-muted)', margin: '0', fontWeight: '500' }}>Stations</p>
          </div>
          <div>
            <p style={{ fontSize: '2.5rem', fontWeight: '800', color: '#FFB81C', margin: '0 0 8px 0' }}>1</p>
            <p style={{ fontSize: '1rem', color: 'var(--text-muted)', margin: '0', fontWeight: '500' }}>Active Lines</p>
          </div>
          <div>
            <p style={{ fontSize: '2.5rem', fontWeight: '800', color: '#FFB81C', margin: '0 0 8px 0' }}>10–15 min</p>
      ...
      <section style={{ backgroundColor: 'var(--bg)', padding: '60px 24px', borderBottom: '1px solid var(--border)' }}>
        <div style={{ maxWidth: '700px', marginLeft: 'auto', marginRight: 'auto' }}>
          <h2 style={{ fontSize: '1.75rem', fontWeight: '800', marginBottom: '12px', textAlign: 'center', color: 'var(--text)' }}>
            Plan Your Route in El Paso
          </h2>
          <p style={{ fontSize: '1rem', textAlign: 'center', marginBottom: '24px', color: 'var(--text-muted)' }}>
            Calculate the best route between Sun Link stations
          </p>
          <SearchBar ciudad="el-paso" />
        </div>
      </section>
        ...
        <div style={{ maxWidth: '1000px', marginLeft: 'auto', marginRight: 'auto' }}>
          ...
          <div style={{ display: 'grid', gap: '24px' }}>
            {estacionesDestacadas.map((estacion, idx) => (
              <Link href={`/el-paso/station/${estacion.slug}`} key={idx} style={{ textDecoration: 'none' }}>
                <div style={{ padding: '28px', backgroundColor: 'var(--surface)', borderRadius: 'var(--radius)', border: '1px solid var(--border)', cursor: 'pointer', transition: 'all 0.3s ease' }}>
                  <h3 style={{ fontSize: '1.25rem', fontWeight: '700', margin: '0 0 8px 0', color: '#FFB81C' }}>
                    {estacion.nombre}
                  </h3>
      ...
      <section style={{ backgroundColor: 'var(--bg)', padding: '80px 24px', borderBottom: '1px solid var(--border)' }}>
        <div style={{ maxWidth: '1000px', marginLeft: 'auto', marginRight: 'auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '24px' }}>
            <AffiliateTransportCard icon="🎫" titulo="Single Ride" descripcion="One-way trip on Sun Link." precio="$1.00 USD" enlace="#" />
            <AffiliateTransportCard icon="📱" titulo="Mobile Pass" descripcion="Digital ticket via mobile app." precio="$1.00 USD" enlace="#" />
          </div>
        </div>
      </section>
      ...
      <section style={{ backgroundColor: 'var(--bg)', padding: '80px 24px' }}>
        <div style={{ maxWidth: '1000px', marginLeft: 'auto', marginRight: 'auto' }}>
          ...
          <div style={{ display: 'grid', gap: '20px' }}>
            {lineasElPaso.map((linea, idx) => (
              <Link href={`/el-paso/line/${linea.id}`} key={idx} style={{ textDecoration: 'none' }}>
