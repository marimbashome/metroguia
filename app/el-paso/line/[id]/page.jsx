...
export async function generateStaticParams() {
  return lineasElPaso.map((linea) => ({
    id: linea.id,
  }));
}

export async function generateMetadata({ params }) {
  const linea = lineasElPaso.find((l) => l.id === params.id);
  if (!linea) {
    return { title: 'Line not found', description: 'The requested line does not exist.' };
  }
  return {
    title: `Sun Link ${linea.colorNombre} Line — El Paso | MetroGuia`,
    description: linea.descripcion.substring(0, 160),
    openGraph: {
      title: `Sun Link ${linea.colorNombre} Line`,
      description: linea.descripcion.substring(0, 160),
      url: `https://metroguia.mx/el-paso/line/${linea.id}`,
    },
  };
}

export default function LineElPasoPage({ params }) {
  const linea = lineasElPaso.find((l) => l.id === params.id);
  ...
  const estacionesLinea = estacionesElPaso.filter((e) => {
  ...
  return (
    <main style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
      <IntlLineSchema line={(lineasElPaso.find(l => l.id === params.id)) || null} stations={((lineasElPaso.find(l => l.id === params.id) || {}).estaciones || []).map(s => (typeof s === 'string' ? (estacionesElPaso.find(x => x.slug === s) || { slug: s, nombre: s }) : s))} city="El Paso" citySlug="el-paso" linePathPrefix="/el-paso/line" stationPathPrefix="/el-paso/station" country="US" systemName="El Paso Streetcar" locale="en" />
      <section style={{ background: `linear-gradient(135deg, ${linea.color} 0%, ${linea.color}cc 100%)`, color: '#000', padding: '60px 24px' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '16px' }}>
            <span style={{ width: '60px', height: '60px', backgroundColor: 'rgba(0,0,0,0.1)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '2rem', fontWeight: '800', textTransform: 'uppercase' }}>
              {(linea.id || 'X')[0]}
            </span>
            <div>
              ...
              <Link href={`/el-paso/station/${estacion.slug}`} key={idx} style={{ textDecoration: 'none' }}>
                <div style={{
                  ...
                  <div style={{
                    ...
                    backgroundColor: linea.color, color: '#000',
