...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    openGraph: {
      title: estacion.seo_title,
      description: estacion.meta_description,
      url: `https://metroguia.mx/puebla/estacion/${estacion.slug}`,
      type: 'website',
    },
    robots: {
      index: true,
    ...
    alternates: {
      canonical: `https://metroguia.mx/puebla/estacion/${estacion.slug}`,
      languages: {
        'es': `https://metroguia.mx/puebla/estacion/${estacion.slug}`,
        'en': `https://metroguia.mx/puebla/station/${estacion.slug}`,
      },
    },
  };
}

export default function EstacionPueblaPage({ params }) {
  const estacion = estacionesPuebla.find((e) => e.slug === params.slug);
  if (!estacion) {
    return (
  ...
  const colorLinea = estacion.linea === '1' ? '#8B5CF6' : estacion.linea === '2' ? '#A78BFA' : '#7C3AED';
  const linea = lineasPuebla.find((l) => l.id === estacion.linea);
    ...
    <main style={{ backgroundColor: '#ffffff' }}>
      ...
      <section style={{ ...containerStyles, paddingTop: '40px', paddingBottom: '40px', textAlign: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: '10px', justifyContent: 'center', flexWrap: 'wrap' }}>
          ...
          {linea && (
            <Link href={`/puebla/linea/${linea.id}`} style={buttonStyles}>
