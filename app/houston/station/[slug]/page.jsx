...
export async function generateStaticParams() {
  return estacionesHOU.map((estacion) => ({
    slug: estacion.slug,
  }));
}
...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    openGraph: {
      title: estacion.seo_title,
      description: estacion.meta_description,
      url: `https://metroguia.mx/houston/station/${estacion.slug}`,
    },
  };
}

export default function StationHouston({ params }) {
  const estacion = estacionesHOU.find((e) => e.slug === params.slug);
  ...
  if (!estacion) {
    return (
      <main style={{ padding: '80px 24px', textAlign: 'center', backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
        ...
        <Link href="/houston">
          <button style={{ marginTop: '24px', padding: '12px 24px', backgroundColor: '#FF6600', color: '#fff', border: 'none', borderRadius: 'var(--radius)', fontSize: '1rem', fontWeight: '700', cursor: 'pointer' }}>
            Back to Houston
          </button>
        </Link>
      </main>
    ...
    <main style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
      ...
      <section style={{ maxWidth: '1200px', margin: '0 auto', padding: '60px 24px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '40px', marginBottom: '60px' }}>
          ...
          <div>
            ...
            <div className="card" style={{ padding: '24px', marginBottom: '24px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                ...
                <div style={{ borderTop: '1px solid var(--border)', paddingTop: '12px' }}>
                  ...
                  <p style={{ fontSize: '0.95rem', fontWeight: '600', color: 'var(--success)', margin: '0' }}>$1.50 USD</p>
                </div>
                <div style={{ borderTop: '1px solid var(--border)', paddingTop: '12px' }}>
                  ...
                  <p style={{ fontSize: '0.95rem', fontWeight: '600', margin: '0' }}>5:00 AM – Midnight</p>
                </div>
              </div>
            </div>
            ...
            {/* Affiliate */}
            <AffiliateTransportCard ciudad="Houston" estacion={estacion.nombre} fechas={null} />
          </div>
        </div>
      ...
      <section style={{ backgroundColor: 'var(--surface)', padding: '60px 24px', textAlign: 'center' }}>
        ...
        <p style={{ fontSize: '1rem', color: 'var(--text-muted)', margin: '0 0 24px 0', maxWidth: '600px', marginLeft: 'auto', marginRight: 'auto' }}>
          Discover other stations and lines throughout Houston Metro Rail.
        </p>
        <Link href="/houston">
          <button style={{ padding: '14px 32px', backgroundColor: colorPrincipal, color: '#fff', border: 'none', borderRadius: 'var(--radius)', fontSize: '1rem', fontWeight: '700', cursor: 'pointer' }}>
            Back to Houston
          </button>
        </Link>
      </section>
