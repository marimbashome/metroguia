...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    openGraph: {
      title: linea.seo_title,
      description: linea.meta_description,
      url: `https://metroguia.mx/houston/line/${linea.id}`,
    },
  };
}

export default function LineHouston({ params }) {
  const linea = lineasHOU.find((l) => l.id === params.id);
  ...
  const estacionesLinea = estacionesHOU.filter((e) => Array.isArray(e.linea) ? e.linea.includes(linea.id) : e.linea === linea.id);

  return (
    <main style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
      <LineSchema linea={linea} ciudad="houston" sistema="Metro Rail" />
      ...
      <section style={{ maxWidth: '1200px', margin: '0 auto', padding: '60px 24px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px', marginBottom: '48px' }}>
          ...
          <div className="card" style={{ padding: '20px' }}>
            ...
            <p style={{ fontSize: '1.125rem', fontWeight: '700', margin: '0' }}>5:00 AM – Midnight</p>
          </div>
          <div className="card" style={{ padding: '20px' }}>
            ...
            <p style={{ fontSize: '1.125rem', fontWeight: '700', margin: '0', color: 'var(--success)' }}>$1.50 USD</p>
          </div>
        </div>
