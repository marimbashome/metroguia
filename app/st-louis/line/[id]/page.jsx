...
export async function generateStaticParams() {
  return lineasStLouis.map((linea) => ({
    id: linea.id,
  }));
}
...
export async function generateMetadata({ params }) {
  ...
  return {
    title: `MetroLink ${linea.colorNombre} — St. Louis | MetroGuia`,
    description: linea.descripcion.substring(0, 160),
    openGraph: {
      title: `MetroLink ${linea.colorNombre}`,
      description: linea.descripcion.substring(0, 160),
      url: `https://metroguia.mx/st-louis/line/${linea.id}`,
    },
  };
}

export default function LineStLouisPage({ params }) {
  const linea = lineasStLouis.find((l) => l.id === params.id);
  ...
  return (
    <main style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
      <IntlLineSchema line={(lineasStLouis.find(l => l.id === params.id)) || null} stations={((lineasStLouis.find(l => l.id === params.id) || {}).estaciones || []).map(s => (typeof s === 'string' ? (estacionesStLouis.find(x => x.slug === s) || { slug: s, nombre: s }) : s))} city="St. Louis" citySlug="st-louis" linePathPrefix="/st-louis/line" stationPathPrefix="/st-louis/station" country="US" systemName="MetroLink" locale="en" />
      {/* HERO */}
              ...
              {(linea.ruta_1_dia.paradas || []).slice(0, 6).map((parada, idx) => {
                ...
                return (
                  <div key={idx} style={{ display: 'grid', gridTemplateColumns: '60px 1fr', gap: '24px', alignItems: 'start' }}>
                    ...
                    <div>
                      <Link href={`/st-louis/station/${parada.estacion}`}>
