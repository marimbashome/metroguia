...
export async function generateMetadata({ params }) {
  ...
  const seoTitle = linea.seo_title || `${linea.nombre || linea.h1} — Campeche Transit | MetroGuia`;
  ...
  return {
    ...
    openGraph: {
      title: seoTitle,
      description: metaDesc,
      url: `https://metroguia.mx/campeche/line/${linea.id}`,
      type: 'website',
    },
...
export default function LineCampechePage({ params }) {
    ...
    return (
      <main style={{ padding: '40px 20px', textAlign: 'center' }}>
        ...
        <p style={{ marginTop: '20px' }}>
          <Link href="/campeche" style={{ color: '#059669', textDecoration: 'underline' }}>Back to Campeche</Link>
        </p>
      </main>
    );
  }
  ...
  const infoBoxStyles = { backgroundColor: `${linea.color}15`, borderLeft: `4px solid ${linea.color}`, padding: '20px', borderRadius: '4px', marginBottom: '30px' };  const estacionCardStyles = { backgroundColor: '#ffffff', border: '1px solid #e5e7eb', borderRadius: '6px', padding: '15px', marginBottom: '15px', paddingLeft: '50px', position: 'relative' };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: linea.h1 || linea.nombre, item: `https://metroguia.mx/campeche/line/${linea.id}/` }
    ]
  };
    ...
    <main style={{ backgroundColor: '#ffffff' }}>
        ...
        <section style={{ ...containerStyles, ...sectionStyles, paddingTop: '40px' }}>
          ...
          <div style={{ backgroundColor: '#f9fafb', padding: '30px', borderRadius: '8px' }}>
            {linea.ruta_1_dia.paradas.map((parada, idx) => {
              const estacionInfo = estacionesCampeche.find((e) => e.slug === parada.estacion);
              return (
                <div key={idx} style={rutaParadaStyles}>
                  ...
                  <div style={{ flex: 1, color: '#374151', fontSize: '15px', lineHeight: '1.5' }}>
                    <Link href={`/campeche/station/${parada.estacion}`} style={{ color: linea.color, fontWeight: 'bold', textDecoration: 'underline' }}>
        ...
        <section style={{ ...containerStyles, ...sectionStyles, paddingTop: '40px' }}>
          ...
          <div style={gridStyles}>
            {linea.estaciones_turisticas.map((slugT) => {
              ...
              if (!est) return null;
              return (
                <Link key={est.slug} href={`/campeche/station/${est.slug}`} style={{ textDecoration: 'none' }}>
      ...
      <section style={{ ...containerStyles, paddingTop: '40px', paddingBottom: '40px', textAlign: 'center' }}>
        <div style={{ display: 'flex', gap: '10px', justifyContent: 'center', flexWrap: 'wrap' }}>
          <Link href="/campeche" style={buttonStyles}>Back to Campeche</Link>
          {lineasCampeche.filter(l => l.id !== linea.id).map(otherLine => (
            <Link key={otherLine.id} href={`/campeche/line/${otherLine.id}`} style={{ ...buttonStyles, backgroundColor: otherLine.color }}>
