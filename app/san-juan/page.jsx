...
export const metadata = {
  title: 'Tren Urbano — San Juan Transit | MetroGuia',
  description: 'Tren Urbano guide: 1 line, 16+ stations in San Juan. Complete trip planner, schedule & fares.',
  keywords: 'Tren Urbano, San Juan transit, Tren line',
  openGraph: {
    title: 'Tren Urbano — San Juan Transit Guide',
    description: 'Navigate San Juan with Tren Urbano. Trip planner, fares, and station guide.',
    url: 'https://metroguia.mx/san-juan',
    type: 'website',
  },
  robots: { index: true, follow: true },
};

const estacionesDestacadas = estacionesSanJuan.filter(e =>
  ['san-juan-old-san-juan', 'san-juan-puerta-covarrubias', 'san-juan-baldorioty', 'san-juan-plaza-las-delicias'].includes(e.slug)
);
...
export default function SanJuanPage() {
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'San Juan', item: 'https://metroguia.mx/san-juan/' }
    ]
  };
  ...
  return (
    <main style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
      ...
      {/* HERO */}
      <section style={{
        background: 'linear-gradient(135deg, #009639 0%, #009639cc 100%)',
        color: '#FFFFFF',
        padding: '80px 24px',
        textAlign: 'center',
        position: 'relative',
        ...
        <div style={{ maxWidth: '1000px', marginLeft: 'auto', marginRight: 'auto' }}>
          ...
          <p style={{ fontSize: '1.125rem', fontWeight: '400', margin: '0', opacity: '0.9', maxWidth: '600px', marginLeft: 'auto', marginRight: 'auto' }}>
            San Juan's Tren Urbano — your guide to navigating the city by rail.
          </p>
        </div>
      </section>
      ...
      <section style={{ backgroundColor: 'var(--surface)', padding: '48px 24px', borderBottom: '1px solid var(--border)' }}>
        <div style={{ maxWidth: '1000px', marginLeft: 'auto', marginRight: 'auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '32px', textAlign: 'center' }}>
          <div>
            <p style={{ fontSize: '2.5rem', fontWeight: '800', color: '#009639', margin: '0 0 8px 0' }}>16+</p>
            <p style={{ fontSize: '1rem', color: 'var(--text-muted)', margin: '0', fontWeight: '500' }}>Stations</p>
          </div>
          <div>
            <p style={{ fontSize: '2.5rem', fontWeight: '800', color: '#009639', margin: '0 0 8px 0' }}>1</p>
            <p style={{ fontSize: '1rem', color: 'var(--text-muted)', margin: '0', fontWeight: '500' }}>Active Lines</p>
          </div>
          <div>
            <p style={{ fontSize: '2.5rem', fontWeight: '800', color: '#009639', margin: '0 0 8px 0' }}>5–10 min</p>
      ...
      <section style={{ backgroundColor: 'var(--bg)', padding: '60px 24px', borderBottom: '1px solid var(--border)' }}>
        <div style={{ maxWidth: '700px', marginLeft: 'auto', marginRight: 'auto' }}>
          <h2 style={{ fontSize: '1.75rem', fontWeight: '800', marginBottom: '12px', textAlign: 'center', color: 'var(--text)' }}>
            Plan Your Route in San Juan
          </h2>
          <p style={{ fontSize: '1rem', textAlign: 'center', marginBottom: '24px', color: 'var(--text-muted)' }}>
            Calculate the best route between Tren Urbano stations
          </p>
          <SearchBar ciudad="san-juan" />
        </div>
      </section>
        ...
        <div style={{ maxWidth: '1000px', marginLeft: 'auto', marginRight: 'auto' }}>
          ...
          <div style={{ display: 'grid', gap: '24px' }}>
            {estacionesDestacadas.map((estacion, idx) => (
              <Link href={`/san-juan/station/${estacion.slug}`} key={idx} style={{ textDecoration: 'none' }}>
                <div style={{ padding: '28px', backgroundColor: 'var(--surface)', borderRadius: 'var(--radius)', border: '1px solid var(--border)', cursor: 'pointer', transition: 'all 0.3s ease' }}>
                  <h3 style={{ fontSize: '1.25rem', fontWeight: '700', margin: '0 0 8px 0', color: '#009639' }}>
                    {estacion.nombre}
                  </h3>
      ...
      <section style={{ backgroundColor: 'var(--bg)', padding: '80px 24px', borderBottom: '1px solid var(--border)' }}>
        <div style={{ maxWidth: '1000px', marginLeft: 'auto', marginRight: 'auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '24px' }}>
            <AffiliateTransportCard icon="🎫" titulo="Single Ride" descripcion="One-way trip on Tren Urbano." precio="$2.75 USD" enlace="#" />
            <AffiliateTransportCard icon="📱" titulo="Tarjeta Inteligente" descripcion="Digital card via smart card system." precio="$2.75 USD" enlace="#" />
          </div>
        </div>
      </section>
      ...
      <section style={{ backgroundColor: 'var(--bg)', padding: '80px 24px' }}>
        <div style={{ maxWidth: '1000px', marginLeft: 'auto', marginRight: 'auto' }}>
          ...
          <div style={{ display: 'grid', gap: '20px' }}>
            {lineasSanJuan.map((linea, idx) => (
              <Link href={`/san-juan/line/${linea.id}`} key={idx} style={{ textDecoration: 'none' }}>
