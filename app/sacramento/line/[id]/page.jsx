...
export async function generateStaticParams() {
  return lineasSacramento.map((linea) => ({
    id: linea.id,
  }));
}
...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    openGraph: {
      title: linea.seo_title,
      description: linea.meta_description,
      url: `https://metroguia.mx/sacramento/line/${linea.id}`,
    },
  };
}

export default function LineSacramentoPage({ params }) {
  const linea = lineasSacramento.find((l) => l.id === params.id);
  ...
  const estacionesLinea = estacionesSacramento.filter((e) => e.linea && e.linea.includes(linea.id));

  return (
    <main style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
      <IntlLineSchema line={(lineasSacramento.find(l => l.id === params.id)) || null} stations={((lineasSacramento.find(l => l.id === params.id) || {}).estaciones || []).map(s => (typeof s === 'string' ? (estacionesSacramento.find(x => x.slug === s) || { slug: s, nombre: s }) : s))} city="Sacramento" citySlug="sacramento" linePathPrefix="/sacramento/line" stationPathPrefix="/sacramento/station" country="US" systemName="Sacramento RT Light Rail" locale="en" />
      {/* HERO */}
              ...
              {linea.ruta_1_dia.paradas.map((parada, idx) => {
                ...
                return (
                  <div key={idx} style={{ display: 'grid', gridTemplateColumns: '60px 1fr', gap: '24px', alignItems: 'start' }}>
                    ...
                    <div>
                      <Link href={`/sacramento/station/${parada.estacion}`}>
