...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    description: linea.meta_description || `Line ${linea.id} guide for Toronto TTC.`,
    openGraph: {
      title: linea.seo_title,
      description: linea.meta_description,
      url: `https://metroguia.mx/toronto/line/${linea.id}`,
    },
  };
}
...
export default function LineaTorontoPage({ params }) {
  ...
  return (
    <main style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
      <IntlLineSchema line={(lineasToronto.find(l => l.id === params.id)) || null} stations={((lineasToronto.find(l => l.id === params.id) || {}).estaciones || []).map(s => (typeof s === 'string' ? (estacionesToronto.find(x => x.slug === s) || { slug: s, nombre: s }) : s))} city="Toronto" citySlug="toronto" linePathPrefix="/toronto/line" stationPathPrefix="/toronto/station" country="CA" systemName="TTC Subway" locale="en" />
      {/* HERO */}
      ...
      <section style={{ maxWidth: '1200px', margin: '0 auto', padding: '60px 24px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px', marginBottom: '48px' }}>
          ...
          <div className="card" style={{ padding: '20px' }}>
            ...
            <p style={{ fontSize: '1.125rem', fontWeight: '700', margin: '0' }}>5:30–1:30</p>
          </div>
          <div className="card" style={{ padding: '20px' }}>
            ...
            <p style={{ fontSize: '1.125rem', fontWeight: '700', margin: '0', color: 'var(--success)' }}>CAD $3.25</p>
          </div>
        </div>
        ...
        <div style={{ backgroundColor: 'var(--surface)', padding: '32px', borderLeft: `4px solid ${linea.color}`, borderRadius: 'var(--radius)', marginBottom: '60px' }}>
          ...
          <p style={{ fontSize: '1rem', color: 'var(--text)', margin: '0 0 16px 0', lineHeight: '1.8' }}>
            {linea.descripcion || `Line ${linea.id} is a key part of Toronto TTC, connecting major areas and attractions.`}
          </p>
        </div>
