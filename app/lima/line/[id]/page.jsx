...
export async function generateMetadata({ params }) {
  const linea = lineasLima.find(l => l.id === params.id);
  if (!linea) return { title: 'Línea no encontrada' };
  return {
    title: linea.seo_title,
    description: linea.meta_description,
    openGraph: { title: linea.seo_title, url: `https://metroguia.mx/lima/line/${linea.id}/` },
    alternates: { canonical: `https://metroguia.mx/lima/line/${linea.id}/` },
  };
}

export default function LineLima({ params }) {
  const linea = lineasLima.find(l => l.id === params.id);
  ...
  if (!linea) {
    return (
      <main style={{ padding: '80px 24px', textAlign: 'center', backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
        ...
        <Link href="/lima/"><button style={{ marginTop: 24, padding: '12px 24px', background: '#1F8B47', color: '#fff', border: 'none', borderRadius: 'var(--radius)', fontWeight: 700, cursor: 'pointer' }}>Volver a Lima</button></Link>
      </main>
    );
  }

  const estacionesLinea = estacionesLima.filter(e => {
      ...
      <section style={{ maxWidth: '1200px', margin: '0 auto', padding: '60px 24px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: 16, marginBottom: 48 }}>
          {[
            ...
            { label: 'Tarifa', val: 'PEN S/2.50' },
            { label: 'Horario', val: '06:00 – 22:00' },
          ].map(c => (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {linea.ruta_1_dia.paradas.map((p, i) => (
                <div key={i} style={{ display: 'flex', gap: 16, alignItems: 'flex-start' }}>
                  ...
                  <div>
                    <Link href={`/lima/station/${p.estacion}/`} style={{ color: linea.color, fontWeight: 700, textDecoration: 'none' }}>{p.estacion.replace('baires-','').replace(/-/g,' ').replace(/\b\w/g,c=>c.toUpperCase())}</Link>
