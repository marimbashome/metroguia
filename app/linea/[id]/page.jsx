...
export function generateMetadata({ params }) {
  ...
  return { title: linea.seo_title, description: linea.meta_description }
}
...
export default function LineaPage({ params }) {
  ...
  if (!linea) return (
    <div style={{ padding: '4rem 2rem', textAlign: 'center' }}>
      ...
      <p>La línea que buscas no existe en nuestro sistema.</p>
    </div>
  )
    ...
    <div>
      <section className="hero" style={{ backgroundColor: linea.color }}>
        <div className="container">
          <h1>{linea.h1}</h1>
          <p style={{ fontSize: '1.125rem' }}>Desde {linea.inicio} hasta {linea.fin}</p>
        </div>
      </section>
        ...
        <div className="container" style={{ maxWidth: '800px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem', marginBottom: '3rem' }}>
            {linea.estaciones.map((est) => (
              <a key={est.slug} href={`/estacion/${est.slug}/`} style={{ padding: '1rem', border: `2px solid ${linea.color}`, borderRadius: 'var(--radius)', textDecoration: 'none', color: 'var(--text)', textAlign: 'center' }}>
          ...
          {linea.ruta_1_dia && linea.ruta_1_dia.paradas.length > 0 && (
            <>
              ...
              <div style={{ backgroundColor: 'var(--metro-gray)', padding: '2rem', borderRadius: 'var(--radius)' }}>
                {linea.ruta_1_dia.paradas.map((parada, idx) => (
                  <div key={idx} style={{ marginBottom: idx < linea.ruta_1_dia.paradas.length - 1 ? '2rem' : 0 }}>
                    <h4 style={{ color: linea.color, marginBottom: '0.5rem' }}>{idx + 1}. {parada.estacion}</h4>
      ...
      <section style={{ padding: '3rem 2rem', backgroundColor: 'var(--metro-dark)', color: 'white', textAlign: 'center' }}>
        <div className="container">
          <p style={{ marginBottom: '1.5rem' }}>¿Necesitas llegar al Estadio Azteca para el Mundial 2026?</p>
