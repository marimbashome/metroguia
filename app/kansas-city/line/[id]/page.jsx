...
export async function generateMetadata({ params }) {
  ...
  return {
    title: linea.seo_title || `${linea.colorNombre} — Kansas City Streetcar`,
    description: linea.meta_description || `${linea.colorNombre} guide for Kansas City Streetcar.`,
    openGraph: {
      title: linea.seo_title,
      description: linea.meta_description,
      url: `https://metroguia.mx/kansas-city/line/${linea.id}`,
    },
  };
}

export default function LineaKCPage({ params }) {
  const linea = lineasKansasCity.find((l) => l.id === params.id);
  ...
  const estacionesLinea = estacionesKansasCity.filter(e => linea.estaciones.includes(e.slug));
      ...
      <section style={{ background: `linear-gradient(135deg, ${linea.color} 0%, ${linea.color}cc 100%)`, color: '#fff', padding: '60px 24px' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          <div style={{ marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '12px' }}>
            <span style={{ width: '60px', height: '60px', backgroundColor: 'rgba(255,255,255,0.3)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.75rem', fontWeight: '800' }}>
              🚊
            </span>
          </div>
      ...
      <section style={{ maxWidth: '1200px', margin: '0 auto', padding: '60px 24px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px', marginBottom: '48px' }}>
          ...
          <div className="card" style={{ padding: '20px' }}>
            ...
            <p style={{ fontSize: '1.125rem', fontWeight: '700', margin: '0' }}>6:00–23:00</p>
          </div>
          <div className="card" style={{ padding: '20px' }}>
            ...
            <p style={{ fontSize: '1.125rem', fontWeight: '700', margin: '0', color: 'var(--success)' }}>FREE</p>
          </div>
        </div>
        ...
        <div style={{ backgroundColor: 'var(--surface)', padding: '32px', borderLeft: `4px solid ${linea.color}`, borderRadius: 'var(--radius)', marginBottom: '60px' }}>
          ...
          <p style={{ fontSize: '1rem', color: 'var(--text)', margin: '0 0 16px 0', lineHeight: '1.8' }}>
            {linea.descripcion || `The ${linea.colorNombre} is a key part of Kansas City's modern streetcar system, serving the Midtown and Downtown areas.`}
          </p>
        </div>
