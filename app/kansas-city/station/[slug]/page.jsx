...
const LINE_COLORS = { 'Main': '#0066CC', 'Purple': '#8B008B' };

export async function generateStaticParams() {
  return estacionesKansasCity.map((estacion) => ({ slug: estacion.slug }));
}
...
export async function generateMetadata({ params }) {
  ...
  return {
    title: estacion.seo_title || `${estacion.nombre} Station — Kansas City Streetcar`,
    ...
    openGraph: {
      ...
      url: `https://metroguia.mx/kansas-city/station/${estacion.slug}`,
    },
  };
}

export default function EstacionKCPage({ params }) {
  const estacion = estacionesKansasCity.find((e) => e.slug === params.slug);
  ...
  const lineaData = lineasKansasCity.find(l => l.id === estacion.linea);
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: `${estacion.linea}`, item: `https://metroguia.mx/kansas-city/line/${estacion.linea}/` },
      { '@type': 'ListItem', position: 4, name: estacion.nombre, item: `https://metroguia.mx/kansas-city/station/${estacion.slug}` },
    ],
  };
  ...
  const transitStationSchema = {
    ...
    description: estacion.meta_description || `${estacion.nombre} station on Kansas City Streetcar.`,
    url: `https://metroguia.mx/kansas-city/station/${estacion.slug}/`,
    isAccessibleForFree: true,
    address: { '@type': 'PostalAddress', addressLocality: 'Kansas City', addressRegion: 'Missouri', addressCountry: 'US' },
      ...
      <section style={{ background: `linear-gradient(135deg, ${colorLinea} 0%, ${colorLinea}cc 100%)`, color: '#fff', padding: '60px 24px' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          <div style={{ marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <span style={{ fontSize: '1rem', fontWeight: '500' }}>Kansas City, Missouri</span>
          </div>
          ...
          <p style={{ fontSize: '1.125rem', margin: '0', opacity: '0.95', maxWidth: '700px', lineHeight: '1.6' }}>
            {estacion.intro || `${estacion.nombre} is a key station on Kansas City Streetcar.`}
          </p>
        </div>
      </section>
            ...
            {lineaData && (
              <div style={{ marginBottom: '48px' }}>
                ...
                <div style={{ padding: '20px', backgroundColor: 'var(--surface)', borderRadius: 'var(--radius)' }}>
                  ...
                  <Link href={`/kansas-city/line/${lineaData.id}`}>
