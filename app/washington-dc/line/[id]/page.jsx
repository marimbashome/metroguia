...
export default function LineDCPage({ params }) {
  ...
  return (
    <main style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
      <IntlLineSchema line={(lineasDetalleDC.find(l => l.id === params.id)) || null} stations={((lineasDetalleDC.find(l => l.id === params.id) || {}).estaciones || []).map(s => (typeof s === 'string' ? (estacionesDC.find(x => x.slug === s) || { slug: s, nombre: s }) : s))} city="Washington DC" citySlug="washington-dc" linePathPrefix="/washington-dc/line" stationPathPrefix="/washington-dc/station" country="US" systemName="WMATA Metro" locale="en" />
      {/* HERO */}
