...
const LINE_COLORS = { 'RED': '#BF0D3E', 'BLUE': '#009CDE', 'ORANGE': '#ED8B00', 'SILVER': '#A2A4A1', 'GREEN': '#00B140', 'YELLOW': '#FFD100' };
...
export default function StationDCPage({ params }) {
  const estacion = estacionesDC.find((e) => e.slug === params.slug);
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: `Line ${lineValue}`, item: `https://metroguia.mx/washington-dc/line/${lineValue}/` },
  ...
  const transitStationSchema = {
    ...
    address: { '@type': 'PostalAddress', addressLocality: 'Washington', addressRegion: 'D.C.', addressCountry: 'US' },
  ...
  const faqItems = [];
  faqItems.push({ '@type': 'Question', name: `How do I get to ${estacion.nombre} station?`, acceptedAnswer: { '@type': 'Answer', text: `${estacion.nombre} is served by the Metro ${lineValue} Line. Use the MetroGuia trip planner for directions.` } });
  ...
  faqItems.push({ '@type': 'Question', name: `What are the hours and fares?`, acceptedAnswer: { '@type': 'Answer', text: `Metro operates 5:00 AM to 11:30 PM. Fares: $2.00–$6.00 with SmarTrip Card.` } });
      ...
      <section style={{ background: `linear-gradient(135deg, ${colorLinea} 0%, ${colorLinea}cc 100%)`, color: '#fff', padding: '60px 24px' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          <div style={{ marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <span style={{ fontSize: '1rem', fontWeight: '500' }}>Washington, D.C.</span>
          </div>
          ...
          <div>
            ...
            <div style={{ backgroundColor: 'var(--surface)', padding: '24px', borderRadius: 'var(--radius)', borderTop: `4px solid var(--warning)` }}>
              ...
              <p style={{ fontSize: '0.9rem', color: 'var(--text-muted)', margin: '0 0 12px 0', lineHeight: '1.5' }}>
                Use SmarTrip card for fastest boarding or buy a single ticket at vending machine.
              </p>
              <Link href="https://www.wmata.com/smartrip" target="_blank" style={{ textDecoration: 'none' }}>
