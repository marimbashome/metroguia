...
export async function generateStaticParams() {
  return lineasToluca.map((linea) => ({ id: linea.id }));
}

export async function generateMetadata({ params }) {
  const linea = lineasToluca.find((l) => l.id === params.id);
  if (!linea) return { title: 'Line not found' };
  const seoTitle = linea.seo_title || `${linea.nombre || linea.h1} — Toluca Transit | MetroGuia`;
  ...
  return {
    ...
    openGraph: {
      title: seoTitle,
      description: metaDesc,
      url: `https://metroguia.mx/toluca/line/${linea.id}`,
      type: 'website',
    },
...
export default function LineTolucaPage({ params }) {
  const linea = lineasToluca.find((l) => l.id === params.id);
  if (!linea) {
    return (
      <main style={{ padding: '40px 20px', textAlign: 'center' }}>
        ...
        <p style={{ marginTop: '20px' }}>
          <Link href="/toluca" style={{ color: '#059669', textDecoration: 'underline' }}>Back to Toluca</Link>
        </p>
      </main>
    );
  }
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'Toluca', item: 'https://metroguia.mx/toluca/' },
      { '@type': 'ListItem', position: 3, name: linea.h1 || linea.nombre, item: `https://metroguia.mx/toluca/line/${linea.id}/` }
    ]
  };
    ...
    <main style={{ backgroundColor: '#ffffff' }}>
        ...
        <section style={{ ...containerStyles, ...sectionStyles, paddingTop: '40px' }}>
          ...
          <div style={{ backgroundColor: '#f9fafb', padding: '30px', borderRadius: '8px' }}>
            {linea.ruta_1_dia.paradas.map((parada, idx) => {
              const estacionInfo = estacionesToluca.find((e) => e.slug === parada.estacion);
              return (
                <div key={idx} style={rutaParadaStyles}>
                  ...
                  <div style={{ flex: 1, color: '#374151', fontSize: '15px', lineHeight: '1.5' }}>
                    <Link href={`/toluca/station/${parada.estacion}`} style={{ color: linea.color, fontWeight: 'bold', textDecoration: 'underline' }}>
        ...
        <section style={{ ...containerStyles, ...sectionStyles, paddingTop: '40px' }}>
          ...
          <div style={gridStyles}>
            {linea.estaciones_turisticas.map((slugT) => {
              const est = estacionesToluca.find((e) => e.slug === slugT);
              if (!est) return null;
              return (
                <Link key={est.slug} href={`/toluca/station/${est.slug}`} style={{ textDecoration: 'none' }}>
      ...
      <section style={{ ...containerStyles, paddingTop: '40px', paddingBottom: '40px', textAlign: 'center' }}>
        <div style={{ display: 'flex', gap: '10px', justifyContent: 'center', flexWrap: 'wrap' }}>
          <Link href="/toluca" style={buttonStyles}>Back to Toluca</Link>
          {lineasToluca.filter(l => l.id !== linea.id).map(otherLine => (
            <Link key={otherLine.id} href={`/toluca/line/${otherLine.id}`} style={{ ...buttonStyles, backgroundColor: otherLine.color }}>
