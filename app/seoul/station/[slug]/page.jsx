...
export async function generateMetadata({ params }) {
  const estacion = estacionesSeoul.find((e) => e.slug === params.slug);
  if (!estacion) return { title: 'Station not found' };
  return {
    title: estacion.seo_title,
    description: estacion.meta_description,
    openGraph: { title: estacion.seo_title, description: estacion.meta_description, url: `https://metroguia.mx/seoul/station/${estacion.slug}/` },
    alternates: { canonical: `https://metroguia.mx/seoul/station/${estacion.slug}/` },
  };
}

export default function SeoulStation({ params }) {
  const estacion = estacionesSeoul.find((e) => e.slug === params.slug);
  if (!estacion) {
    return (
      <main style={{ padding: '80px 24px', textAlign: 'center', backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
        <h1>Station not found</h1>
        <Link href="/seoul" style={{ display: 'inline-block', marginTop: 24, padding: '12px 24px', background: '#00A84D', color: '#fff', borderRadius: 'var(--radius)', textDecoration: 'none', fontWeight: 700 }}>Back to Seoul</Link>
      </main>
    );
  }
  const lineaArr = Array.isArray(estacion.linea) ? estacion.linea : [estacion.linea];
  const lineasEstacion = lineaArr.map(id => lineasSeoul.find(l => l.id === id)).filter(Boolean);
  const colorPrincipal = lineasEstacion.length > 0 ? lineasEstacion[0].color : '#00A84D';
    ...
    <main style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
      <section style={{ background: `linear-gradient(135deg, ${colorPrincipal} 0%, ${colorPrincipal}cc 100%)`, color: '#fff', padding: '64px 24px' }}>
        <div style={{ maxWidth: 1000, margin: '0 auto' }}>
          <nav style={{ fontSize: '0.8rem', opacity: 0.85, marginBottom: 16 }}>
            <Link href="/seoul/" style={{ color: '#fff', textDecoration: 'none' }}>Seoul Metro</Link> → {estacion.nombre}
          </nav>
      ...
      <section style={{ maxWidth: 1000, margin: '0 auto', padding: '48px 24px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr minmax(240px, 300px)', gap: 40 }}>
          <div>
            {estacion.descripcion_turistica && <div style={{ marginBottom: 32 }}><h2 style={{ fontWeight: 700, fontSize: '1.2rem', marginBottom: 12 }}>About this Station</h2><p style={{ lineHeight: 1.7, margin: 0 }}>{estacion.descripcion_turistica}</p></div>}
            ...
            {estacion.lugares_cercanos && estacion.lugares_cercanos.length > 0 && (
              <div><h2 style={{ fontWeight: 700, fontSize: '1.2rem', marginBottom: 12 }}>Nearby</h2>
              <div style={{ display: 'grid', gap: 10 }}>
                {estacion.lugares_cercanos.map((l, i) => (
                  <div key={i} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: '0.875rem' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}><p style={{ fontWeight: 700, margin: 0, fontSize: '0.9rem' }}>{l.nombre}</p><span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>{l.distancia}</span></div>
                    {l.descripcion && <p style={{ fontSize: '0.82rem', color: 'var(--text-muted)', margin: 0, lineHeight: 1.5 }}>{l.descripcion}</p>}
                  </div>
                ))}
              </div></div>
            )}
          ...
          <div>
            <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: '1.25rem', marginBottom: 16 }}>
              ...
              <div style={{ display: 'grid', gap: 10 }}>
                <div><p style={{ fontSize: '0.7rem', color: 'var(--text-muted)', textTransform: 'uppercase', margin: '0 0 2px', fontWeight: 600 }}>Municipality</p><p style={{ margin: 0, fontSize: '0.9rem' }}>{estacion.municipio}</p></div>
            ...
            {estacion.transferencias && estacion.transferencias.length > 0 && (
              <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: '1.25rem' }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                  {estacion.transferencias.map((t, i) => <span key={i} style={{ padding: '4px 10px', background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 20, fontSize: '0.78rem' }}>{t}</span>)}
                </div>
              </div>
            )}
          </div>
