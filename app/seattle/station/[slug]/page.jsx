...
const LINE_COLORS = { 'RED': '#E31937', 'GREEN': '#00A651', 'BLUE': '#0066CC' };

export async function generateStaticParams() {
  return estacionesSeattle.map((estacion) => ({
    slug: estacion.slug,
  }));
}

export async function generateMetadata({ params }) {
  const estacion = estacionesSeattle.find((e) => e.slug === params.slug);
  if (!estacion) {
    return {
  ...
  return {
    ...
    openGraph: {
      title: estacion.seo_title,
      description: estacion.meta_description,
      url: `https://metroguia.mx/seattle/station/${estacion.slug}`,
    },
  };
}

export default function StationSeattlePage({ params }) {
  const estacion = estacionesSeattle.find((e) => e.slug === params.slug);
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: `Line ${estacion.linea}`, item: `https://metroguia.mx/seattle/line/${estacion.linea}/` },
      { '@type': 'ListItem', position: 4, name: estacion.nombre, item: `https://metroguia.mx/seattle/station/${estacion.slug}` },
    ],
  };
  ...
  const transitStationSchema = {
    ...
    address: { '@type': 'PostalAddress', addressLocality: 'Seattle', addressRegion: estacion.municipio || 'Washington', addressCountry: 'US' },
    geo: { '@type': 'GeoCoordinates', latitude: estacion.lat || 47.6062, longitude: estacion.lng || -122.3321 },
    openingHoursSpecification: { '@type': 'OpeningHoursSpecification', dayOfWeek: ['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday'], opens: '05:00', closes: '00:30' },
  };

  const faqItems = [];
  faqItems.push({ '@type': 'Question', name: `How do I get to ${estacion.nombre} station?`, acceptedAnswer: { '@type': 'Answer', text: `${estacion.nombre} is served by the Link ${estacion.linea} Line. Use the MetroGuia trip planner for directions.` } });
  ...
  faqItems.push({ '@type': 'Question', name: `What are the hours and fares?`, acceptedAnswer: { '@type': 'Answer', text: `Link operates 5:00 AM to 12:30 AM. Fares: $2.75–$3.25 with ORCA Card.` } });
          ...
          <div>
            ...
            <div style={{ backgroundColor: 'var(--surface)', padding: '24px', borderRadius: 'var(--radius)', borderTop: `4px solid var(--warning)` }}>
              ...
              <p style={{ fontSize: '0.9rem', color: 'var(--text-muted)', margin: '0 0 12px 0', lineHeight: '1.5' }}>
                Use ORCA card for fastest boarding or buy a single ticket online.
              </p>
              <Link href="https://www.orca.cards/" target="_blank" style={{ textDecoration: 'none' }}>
