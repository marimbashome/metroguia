...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    openGraph: {
      title: linea.seo_title,
      description: linea.meta_description,
      url: `https://metroguia.mx/seattle/line/${linea.id}`,
    },
  };
}

export default function LineSeattlePage({ params }) {
  const linea = lineasSeattle.find((l) => l.id === params.id);
  ...
  const estacionesLinea = estacionesSeattle.filter((e) => e.linea === linea.id);
      ...
      <section style={{ maxWidth: '1200px', margin: '0 auto', padding: '60px 24px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px', marginBottom: '48px' }}>
          ...
          <div style={{ padding: '20px', backgroundColor: 'var(--surface)', borderRadius: 'var(--radius)', border: '1px solid var(--border)' }}>
            ...
            <p style={{ fontSize: '1.125rem', fontWeight: '700', margin: '0', color: 'var(--success)' }}>$2.75–$3.25</p>
          </div>
        </div>
              ...
              {linea.ruta_1_dia.paradas.map((parada, idx) => {
                ...
                return (
                  <div key={idx} style={{ display: 'grid', gridTemplateColumns: '60px 1fr', gap: '24px', alignItems: 'start' }}>
                    ...
                    <div>
                      <Link href={`/seattle/station/${parada.estacion}`}>
