...
export async function generateMetadata({ params }) {
  ...
  const seoTitle = estacion.seo_title || `${estacion.nombre} Station — Mérida Transit | MetroGuia`;
  const metaDesc = estacion.meta_description || estacion.descripcion_turistica || `Guide to ${estacion.nombre} station in Mérida`;
  return {
    title: seoTitle,
    ...
    openGraph: {
      title: seoTitle,
      description: metaDesc,
      url: `https://metroguia.mx/merida/station/${estacion.slug}`,
      type: 'website',
    },
    robots: { index: true, follow: true },
    alternates: {
      canonical: `https://metroguia.mx/merida/station/${estacion.slug}`,
      languages: {
        'es': `https://metroguia.mx/merida/estacion/${estacion.slug}`,
        'en': `https://metroguia.mx/merida/station/${estacion.slug}`,
      },
    },
  };
}

export default function StationMeridaPage({ params }) {
  const estacion = estacionesMerida.find((e) => e.slug === params.slug);
  if (!estacion) {
    return (
  ...
  const linea = lineasMerida.find((l) => l.id === lineaId);
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: `Line ${lineaId}`, item: `https://metroguia.mx/merida/line/${lineaId}/` },
      { '@type': 'ListItem', position: 4, name: estacion.h1 || estacion.nombre, item: `https://metroguia.mx/merida/station/${estacion.slug}/` }
    ]
  };
    ...
    <main style={{ backgroundColor: '#ffffff'}} >
      ...
      <section style={heroStyles}>
        <div style={containerStyles}>
          <p style={{ fontSize: '14px', marginBottom: '10px', opacity: '0.9'}} >
            Line {lineaId} • {estacion.municipio || 'Mérida'}
          </p>
      ...
      <section style={{ ...containerStyles, paddingTop: '40px', paddingBottom: '40px', textAlign: 'center'}} >
        <div style={{ display: 'flex', gap: '10px', justifyContent: 'center', flexWrap: 'wrap'}} >
          ...
          {linea && <Link href={`/merida/line/${linea.id}`} style={buttonStyles}>View Line {linea.id}</Link>}
        </div>
      </section>
    </main>
  );
