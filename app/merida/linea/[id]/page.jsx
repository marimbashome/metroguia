...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    openGraph: {
      title: linea.seo_title,
      description: linea.meta_description,
      url: `https://metroguia.mx/merida/linea/${linea.id}`,
      type: 'website',
    },
    robots: {
      index: true,
...
export default function LineaMeridaPage({ params }) {
  const linea = lineasMerida.find((l) => l.id === params.id);
  if (!linea) {
    return (
  ...
  const estacionesLinea = estacionesMerida.filter((e) => e.linea === linea.id);
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      },
      {
        ...
        position: 3,
        name: linea.nombre,
        item: `https://metroguia.mx/merida/linea/${linea.id}/`
      }
    ]
  }
  ...
  return (
    <main style={{ backgroundColor: '#ffffff' }}>
            ...
            return (
              <div key={idx} style={rutaParadaStyles}>
                ...
                <div style={rutaActivityStyles}>
                  <Link
                    href={`/merida/estacion/${parada.estacion}`}
        ...
        <section style={{ ...containerStyles, ...sectionStyles, paddingTop: '40px' }}>
          ...
          <div style={gridStyles}>
            {linea.estaciones_turisticas.map((slugTuristico) => {
              const estacion = estacionesMerida.find((e) => e.slug === slugTuristico);
        ...
        <div style={gridStyles}>
          <div style={{ backgroundColor: '#f9fafb', padding: '20px', borderRadius: '8px', border: `1px solid #e5e7eb` }}>
            ...
            <p style={{ fontSize: '14px', color: '#374151', lineHeight: '1.6', margin: 0 }}>
              Operan de 5:00 AM a 11:00 PM. Frecuencia cada 5-15 minutos según hora.
            </p>
          </div>
          <div style={{ backgroundColor: '#f9fafb', padding: '20px', borderRadius: '8px', border: `1px solid #e5e7eb` }}>
            <h3 style={{ fontSize: '16px', fontWeight: 'bold', color: linea.color, marginBottom: '10px' }}>
            ...
            <p style={{ fontSize: '14px', color: '#374151', lineHeight: '1.6', margin: 0 }}>
              Tarjeta recargable del SIT. Transbordo gratuito entre corredores.
            </p>
          </div>
          <div style={{ backgroundColor: '#f9fafb', padding: '20px', borderRadius: '8px', border: `1px solid #e5e7eb` }}>
            <h3 style={{ fontSize: '16px', fontWeight: 'bold', color: linea.color, marginBottom: '10px' }}>
            ...
            <p style={{ fontSize: '14px', color: '#374151', lineHeight: '1.6', margin: 0 }}>
              Estaciones accesibles. Asientos prioritarios para adultos mayores y embarazadas.
            </p>
          </div>
        </div>
      </section>
      ...
      <section style={{ ...containerStyles, paddingTop: '40px', paddingBottom: '40px', textAlign: 'center' }}>
        <h2 style={sectionTitleStyles}>Explora Más de Mérida</h2>
        <div style={{ display: 'flex', gap: '10px', justifyContent: 'center', flexWrap: 'wrap' }}>
          <Link href="/merida" style={buttonStyles}>
            Volver a Mérida
          </Link>
          {linea.id !== 'D1' && (
            <Link href="/merida/linea/D1" style={buttonStyles}>
              Corredor Dorado
            </Link>
          )}
          {linea.id !== 'R2' && (
            <Link href="/merida/linea/R2" style={buttonStyles}>
              Corredor Rojo
            </Link>
          )}
          {linea.id !== 'V3' && (
            <Link href="/merida/linea/V3" style={buttonStyles}>
              Corredor Verde
            </Link>
          )}
          {linea.id !== 'A4' && (
            <Link href="/merida/linea/A4" style={buttonStyles}>
              Corredor Azul
            </Link>
          )}
        </div>
