...
export async function generateMetadata({ params }) {
  ...
  const seoTitle = estacion.seo_title || `${estacion.nombre} Station — Tijuana Transit | MetroGuia`;
  const metaDesc = estacion.meta_description || estacion.descripcion_turistica || `Guide to ${estacion.nombre} station in Tijuana`;
  return {
    title: seoTitle,
    ...
    openGraph: {
      title: seoTitle,
      description: metaDesc,
      url: `https://metroguia.mx/tijuana/station/${estacion.slug}`,
      type: 'website',
    },
    robots: { index: true, follow: true },
    alternates: {
      canonical: `https://metroguia.mx/tijuana/station/${estacion.slug}`,
      languages: {
...
export default function StationTijuanaPage({ params }) {
  const estacion = estacionesTijuana.find((e) => e.slug === params.slug);
  if (!estacion) {
    return (
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: `Line ${lineaId}`, item: `https://metroguia.mx/tijuana/line/${lineaId}/` },
      { '@type': 'ListItem', position: 4, name: estacion.h1 || estacion.nombre, item: `https://metroguia.mx/tijuana/station/${estacion.slug}/` }
    ]
  };
    ...
    <main style={{ backgroundColor: '#ffffff'}} >
      ...
      <section style={heroStyles}>
        <div style={containerStyles}>
          <p style={{ fontSize: '14px', marginBottom: '10px', opacity: '0.9'}} >
            Line {lineaId} • {estacion.municipio || 'Tijuana'}
          </p>
      ...
      <section style={{ ...containerStyles, paddingTop: '40px', paddingBottom: '40px', textAlign: 'center'}} >
        <div style={{ display: 'flex', gap: '10px', justifyContent: 'center', flexWrap: 'wrap'}} >
          ...
          {linea && <Link href={`/tijuana/line/${linea.id}`} style={buttonStyles}>View Line {linea.id}</Link>}
        </div>
      </section>
    </main>
  );
