...
export async function generateMetadata({ params }) {
  ...
  const seoTitle = linea.seo_title || `${linea.nombre || linea.h1} — Tijuana Transit | MetroGuia`;
  ...
  return {
    ...
    openGraph: {
      title: seoTitle,
      description: metaDesc,
      url: `https://metroguia.mx/tijuana/line/${linea.id}`,
      type: 'website',
    },
...
export default function LineTijuanaPage({ params }) {
  const linea = lineasTijuana.find((l) => l.id === params.id);
  if (!linea) {
    return (
      <main style={{ padding: '40px 20px', textAlign: 'center' }}>
        ...
        <p style={{ marginTop: '20px' }}>
          <Link href="/tijuana" style={{ color: '#059669', textDecoration: 'underline' }}>Back to Tijuana</Link>
        </p>
      </main>
    );
  }

  const estacionesLinea = estacionesTijuana.filter((e) =>
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'Tijuana', item: 'https://metroguia.mx/tijuana/' },
      { '@type': 'ListItem', position: 3, name: linea.h1 || linea.nombre, item: `https://metroguia.mx/tijuana/line/${linea.id}/` }
    ]
  };
    ...
    <main style={{ backgroundColor: '#ffffff' }}>
        ...
        <section style={{ ...containerStyles, ...sectionStyles, paddingTop: '40px' }}>
          ...
          <div style={{ backgroundColor: '#f9fafb', padding: '30px', borderRadius: '8px' }}>
            {linea.ruta_1_dia.paradas.map((parada, idx) => {
              const estacionInfo = estacionesTijuana.find((e) => e.slug === parada.estacion);
              return (
                <div key={idx} style={rutaParadaStyles}>
                  ...
                  <div style={{ flex: 1, color: '#374151', fontSize: '15px', lineHeight: '1.5' }}>
                    <Link href={`/tijuana/station/${parada.estacion}`} style={{ color: linea.color, fontWeight: 'bold', textDecoration: 'underline' }}>
        ...
        <section style={{ ...containerStyles, ...sectionStyles, paddingTop: '40px' }}>
          ...
          <div style={gridStyles}>
            {linea.estaciones_turisticas.map((slugT) => {
              const est = estacionesTijuana.find((e) => e.slug === slugT);
              if (!est) return null;
              return (
                <Link key={est.slug} href={`/tijuana/station/${est.slug}`} style={{ textDecoration: 'none' }}>
      ...
      <section style={{ ...containerStyles, paddingTop: '40px', paddingBottom: '40px', textAlign: 'center' }}>
        <div style={{ display: 'flex', gap: '10px', justifyContent: 'center', flexWrap: 'wrap' }}>
          <Link href="/tijuana" style={buttonStyles}>Back to Tijuana</Link>
          {lineasTijuana.filter(l => l.id !== linea.id).map(otherLine => (
            <Link key={otherLine.id} href={`/tijuana/line/${otherLine.id}`} style={{ ...buttonStyles, backgroundColor: otherLine.color }}>
