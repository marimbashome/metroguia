...
const LINEA_COLORS = {
  NQS: '#EF3D33', CARACAS: '#FFCD00', AUTOPISTA: '#1A6FB4', AMERICAS: '#00A859',
};

export async function generateStaticParams() {
  return estacionesBogota.map(e => ({ slug: e.slug }));
}

export async function generateMetadata({ params }) {
  const estacion = estacionesBogota.find(e => e.slug === params.slug);
  if (!estacion) return { title: 'Estación no encontrada' };
  return {
    title: estacion.seo_title,
    description: estacion.meta_description,
    openGraph: { title: estacion.seo_title, url: `https://metroguia.mx/bogota/station/${estacion.slug}/` },
    alternates: { canonical: `https://metroguia.mx/bogota/station/${estacion.slug}/` },
  };
}

export default function StationBogota({ params }) {
  const estacion = estacionesBogota.find(e => e.slug === params.slug);
  ...
  if (!estacion) {
    return (
      <main style={{ padding: '80px 24px', textAlign: 'center', backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
        ...
        <Link href="/bogota"><button style={{ marginTop: 24, padding: '12px 24px', background: '#1E5FA6', color: '#fff', border: 'none', borderRadius: 'var(--radius)', fontWeight: 700, cursor: 'pointer' }}>Volver a Bogotá</button></Link>
      </main>
    );
  }

  const lineaArr = Array.isArray(estacion.linea) ? estacion.linea : [estacion.linea];
  const lineasEst = lineaArr.map(id => lineasBogota.find(l => l.id === id)).filter(Boolean);
  ...
  const breadcrumb = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'Bogotá', item: 'https://metroguia.mx/bogota/' },
      { '@type': 'ListItem', position: 3, name: estacion.nombre, item: `https://metroguia.mx/bogota/station/${estacion.slug}/` },
    ],
  };
