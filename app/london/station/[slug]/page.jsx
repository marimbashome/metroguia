...
export async function generateStaticParams() {
  return estacionesLondon.map(e => ({ slug: e.slug }));
}

export async function generateMetadata({ params }) {
  const e = estacionesLondon.find(e => e.slug === params.slug);
  ...
  return {
    ...
    openGraph: {
      title: e.seo_title,
      description: e.meta_description,
      url: `https://metroguia.mx/london/station/${e.slug}/`,
    },
    alternates: { canonical: `https://metroguia.mx/london/station/${e.slug}/` },
  };
}

export default function LondonStation({ params }) {
  const e = estacionesLondon.find(s => s.slug === params.slug);
  if (!e) {
    return (
      <main style={{ padding: '80px 24px', textAlign: 'center', backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
        ...
        <Link href="/london/" style={{ color: 'var(--primary)' }}>← Back to London</Link>
      </main>
    );
  }
  ...
  return (
    <main style={{ backgroundColor: 'var(--bg)', color: 'var(--text)', padding: '5rem 1.5rem 3rem', maxWidth: 800, margin: '0 auto' }}>
      <IntlStationSchema station={(estacionesLondon.find(s => s.slug === params.slug)) || null} city="Londres" citySlug="london" stationPathPrefix="/london/station" linePathPrefix="/london/line" country="GB" systemName="London Underground" locale="es" />
      {/* BREADCRUMB */}
      <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginBottom: '0.5rem' }}>
        <Link href="/london/" style={{ color: 'var(--text-muted)', textDecoration: 'none' }}>London</Link>
        {' '} › {' '}
        <Link href="/london/" style={{ color: 'var(--text-muted)', textDecoration: 'none' }}>Estaciones</Link>
        {' '} › {e.nombre}
      </p>
      ...
      <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginBottom: '1.5rem' }}>
        {lineaArr.map(l => {
          const linea = lineasLondon.find(x => x.id === l);
          return (
            <Link key={l} href={`/london/line/${l}/`} style={{ background: linea?.color || 'var(--primary)', color: '#fff', padding: '0.3rem 0.8rem', borderRadius: 999, fontWeight: 700, fontSize: '0.85rem', textDecoration: 'none' }}>
              L{l}
            </Link>
          );
        })}
