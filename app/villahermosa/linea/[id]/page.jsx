...
export async function generateStaticParams() {
  return lineasVillahermosa.map((linea) => ({ id: linea.id }));
}

export async function generateMetadata({ params }) {
  const linea = lineasVillahermosa.find((l) => l.id === params.id);
  ...
  return {
    ...
    openGraph: {
      title: linea.seo_title,
      description: linea.meta_description,
      url: `https://metroguia.mx/villahermosa/linea/${linea.id}`,
      type: 'website',
    },
...
export default function LineaVillahermosaPage({ params }) {
  const linea = lineasVillahermosa.find((l) => l.id === params.id);
  if (!linea) {
    return (
      <main style={{ padding: '40px 20px', textAlign: 'center' }}>
        ...
        <p style={{ marginTop: '20px' }}>
          <Link href="/villahermosa" style={{ color: '#F59E0B', textDecoration: 'underline' }}>Volver a Villahermosa</Link>
        </p>
      </main>
    );
  }

  const estacionesLinea = estacionesVillahermosa.filter((e) =>
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: linea.h1, item: `https://metroguia.mx/villahermosa/linea/${linea.id}/` }
    ]
  };
    ...
    <main style={{ backgroundColor: '#ffffff' }}>
            ...
            return (
              <div key={idx} style={rutaParadaStyles}>
                ...
                <div style={{ flex: 1, color: '#374151', fontSize: '15px', lineHeight: '1.5' }}>
                  <Link href={`/villahermosa/estacion/${parada.estacion}`} style={{ color: linea.color, fontWeight: 'bold', textDecoration: 'underline' }}>
        ...
        <section style={{ ...containerStyles, ...sectionStyles, paddingTop: '40px' }}>
          ...
          <div style={gridStyles}>
            {linea.estaciones_turisticas.map((slugT) => {
              const est = estacionesVillahermosa.find((e) => e.slug === slugT);
              if (!est) return null;
              return (
                <Link key={est.slug} href={`/villahermosa/estacion/${est.slug}`} style={{ textDecoration: 'none' }}>
      ...
      <section style={{ ...containerStyles, paddingTop: '40px', paddingBottom: '40px', textAlign: 'center' }}>
        <div style={{ display: 'flex', gap: '10px', justifyContent: 'center', flexWrap: 'wrap' }}>
          <Link href="/villahermosa" style={buttonStyles}>Volver a Villahermosa</Link>
          {lineasVillahermosa.filter(l => l.id !== linea.id).map(otherLine => (
            <Link key={otherLine.id} href={`/villahermosa/linea/${otherLine.id}`} style={{ ...buttonStyles, backgroundColor: otherLine.color }}>
              {otherLine.h1}
            </Link>
          ))}
        </div>
