...
export async function generateMetadata({ params }) {
  const estacion = estacionesVillahermosa.find((e) => e.slug === params.slug);
  if (!estacion) return { title: 'Station not found' };
  const seoTitle = estacion.seo_title || `${estacion.nombre} Station — Villahermosa Transit | MetroGuia`;
  const metaDesc = estacion.meta_description || estacion.descripcion_turistica || `Guide to ${estacion.nombre} station in Villahermosa`;
  return {
    title: seoTitle,
    ...
    openGraph: {
      title: seoTitle,
      description: metaDesc,
      url: `https://metroguia.mx/villahermosa/station/${estacion.slug}`,
      type: 'website',
    },
    robots: { index: true, follow: true },
    alternates: {
      canonical: `https://metroguia.mx/villahermosa/station/${estacion.slug}`,
      languages: {
        'es': `https://metroguia.mx/villahermosa/estacion/${estacion.slug}`,
        'en': `https://metroguia.mx/villahermosa/station/${estacion.slug}`,
      },
    },
  };
}

export default function StationVillahermosaPage({ params }) {
  const estacion = estacionesVillahermosa.find((e) => e.slug === params.slug);
  if (!estacion) {
    return (
  ...
  const linea = lineasVillahermosa.find((l) => l.id === lineaId);
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: `Line ${lineaId}`, item: `https://metroguia.mx/villahermosa/line/${lineaId}/` },
      { '@type': 'ListItem', position: 4, name: estacion.h1 || estacion.nombre, item: `https://metroguia.mx/villahermosa/station/${estacion.slug}/` }
    ]
  };
    ...
    <main style={{ backgroundColor: '#ffffff'}} >
      ...
      <section style={heroStyles}>
        <div style={containerStyles}>
          <p style={{ fontSize: '14px', marginBottom: '10px', opacity: '0.9'}} >
            Line {lineaId} • {estacion.municipio || 'Villahermosa'}
          </p>
      ...
      <section style={{ ...containerStyles, paddingTop: '40px', paddingBottom: '40px', textAlign: 'center'}} >
        <div style={{ display: 'flex', gap: '10px', justifyContent: 'center', flexWrap: 'wrap'}} >
          ...
          {linea && <Link href={`/villahermosa/line/${linea.id}`} style={buttonStyles}>View Line {linea.id}</Link>}
        </div>
      </section>
    </main>
  );
