...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    openGraph: {
      title: seoTitle,
      description: metaDesc,
      url: `https://metroguia.mx/villahermosa/estacion/${estacion.slug}`,
      type: 'website',
    },
    robots: { index: true, follow: true },
    alternates: {
      canonical: `https://metroguia.mx/villahermosa/estacion/${estacion.slug}`,
      languages: {
...
export default function EstacionVillahermosaPage({ params }) {
  const estacion = estacionesVillahermosa.find((e) => e.slug === params.slug);
  if (!estacion) {
    return (
  ...
  const colorLinea = linea?.color || '#F59E0B';
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: `Línea ${lineaId}`, item: `https://metroguia.mx/villahermosa/linea/${lineaId}/` },
      { '@type': 'ListItem', position: 4, name: estacion.h1, item: `https://metroguia.mx/villahermosa/estacion/${estacion.slug}/` }
    ]
  };
    ...
    <main style={{ backgroundColor: '#ffffff' }}>
      ...
      <section style={{ ...containerStyles, paddingTop: '40px', paddingBottom: '40px', textAlign: 'center' }}>
        <div style={{ display: 'flex', gap: '10px', justifyContent: 'center', flexWrap: 'wrap' }}>
          ...
          {linea && <Link href={`/villahermosa/linea/${linea.id}`} style={buttonStyles}>Ver Línea {linea.id}</Link>}
        </div>
      </section>
    </main>
  );
