...
export async function generateStaticParams() {
  return lineasBuffalo.map((linea) => ({
    id: linea.id,
  }));
}
...
export async function generateMetadata({ params }) {
  ...
  return {
    title: `NFTA Metro Rail ${linea.colorNombre} Line — Buffalo | MetroGuia`,
    description: linea.descripcion.substring(0, 160),
    openGraph: {
      title: `NFTA Metro Rail ${linea.colorNombre} Line`,
      description: linea.descripcion.substring(0, 160),
      url: `https://metroguia.mx/buffalo/line/${linea.id}`,
    },
  };
}

export default function LineBuffaloPage({ params }) {
  const linea = lineasBuffalo.find((l) => l.id === params.id);
  ...
  return (
    <main style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
      <IntlLineSchema line={(lineasBuffalo.find(l => l.id === params.id)) || null} stations={((lineasBuffalo.find(l => l.id === params.id) || {}).estaciones || []).map(s => (typeof s === 'string' ? (estacionesBuffalo.find(x => x.slug === s) || { slug: s, nombre: s }) : s))} city="Buffalo" citySlug="buffalo" linePathPrefix="/buffalo/line" stationPathPrefix="/buffalo/station" country="US" systemName="NFTA Metro Rail" locale="en" />
      {/* HERO */}
