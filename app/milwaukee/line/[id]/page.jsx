...
export async function generateStaticParams() {
  return lineasMilwaukee.map((linea) => ({
    id: linea.id,
  }));
}

export async function generateMetadata({ params }) {
  const linea = lineasMilwaukee.find((l) => l.id === params.id);
  if (!linea) {
    return { title: 'Line not found', description: 'The requested line does not exist.' };
  }
  return {
    title: `${linea.colorNombre} Line — Milwaukee Transit | MetroGuia`,
    description: linea.descripcion.substring(0, 160),
    openGraph: {
      title: `${linea.colorNombre} Line`,
      description: linea.descripcion.substring(0, 160),
      url: `https://metroguia.mx/milwaukee/line/${linea.id}`,
    },
  };
}

export default function LineMilwaukeeePage({ params }) {
  const linea = lineasMilwaukee.find((l) => l.id === params.id);
  ...
  if (!linea) {
    return (
      <main style={{ padding: '80px 24px', textAlign: 'center', backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
        ...
        <Link href="/milwaukee">
          <button style={{ marginTop: '24px', padding: '12px 24px', backgroundColor: '#FFC72C', color: '#000', border: 'none', borderRadius: 'var(--radius)', fontSize: '1rem', fontWeight: '700', cursor: 'pointer' }}>
            Back to Milwaukee
          </button>
        </Link>
      </main>
  ...
  const estacionesLinea = estacionesMilwaukee.filter((e) => {
  ...
  return (
    <main style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
      <IntlLineSchema line={(lineasMilwaukee.find(l => l.id === params.id)) || null} stations={((lineasMilwaukee.find(l => l.id === params.id) || {}).estaciones || []).map(s => (typeof s === 'string' ? (estacionesMilwaukee.find(x => x.slug === s) || { slug: s, nombre: s }) : s))} city="Milwaukee" citySlug="milwaukee" linePathPrefix="/milwaukee/line" stationPathPrefix="/milwaukee/station" country="US" systemName="The Hop" locale="en" />
      {/* HERO */}
      <section style={{ background: `linear-gradient(135deg, ${linea.color} 0%, ${linea.color}cc 100%)`, color: linea.color === '#FFC72C' ? '#000' : '#fff', padding: '60px 24px' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          <div style={{ marginBottom: '8px' }}>
            <Link href="/milwaukee" style={{ color: linea.color === '#FFC72C' ? 'rgba(0,0,0,0.8)' : 'rgba(255,255,255,0.8)', textDecoration: 'none', fontSize: '0.9rem' }}>
              ← Milwaukee
            </Link>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '16px' }}>
            <span style={{ width: '60px', height: '60px', backgroundColor: linea.color === '#FFC72C' ? 'rgba(0,0,0,0.3)' : 'rgba(255,255,255,0.3)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '2rem', fontWeight: '800', textTransform: 'uppercase' }}>
              {(linea.id || 'X')[0]}
            </span>
            <div>
              ...
              <Link href={`/milwaukee/station/${estacion.slug}`} key={idx} style={{ textDecoration: 'none' }}>
                <div style={{
                  ...
                  <div style={{
                    ...
                    backgroundColor: linea.color, color: linea.color === '#FFC72C' ? '#000' : '#fff',
