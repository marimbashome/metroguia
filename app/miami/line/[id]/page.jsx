...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    openGraph: {
      ...
      url: `https://metroguia.mx/miami/line/${linea.id}`,
    },
  };
}

export default function LineaMiamiPage({ params }) {
  const linea = lineasMiami.find((l) => l.id === params.id);
  ...
  const estacionesLinea = estacionesMiami.filter(e => linea.estaciones.includes(e.slug));

  return (
    <main style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
      <IntlLineSchema line={(lineasMiami.find(l => l.id === params.id)) || null} stations={((lineasMiami.find(l => l.id === params.id) || {}).estaciones || []).map(s => (typeof s === 'string' ? (estacionesMiami.find(x => x.slug === s) || { slug: s, nombre: s }) : s))} city="Miami" citySlug="miami" linePathPrefix="/miami/line" stationPathPrefix="/miami/station" country="US" systemName="Miami Metrorail" locale="en" />
      {/* HERO */}
      ...
      <section style={{ maxWidth: '1200px', margin: '0 auto', padding: '60px 24px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px', marginBottom: '48px' }}>
          ...
          <div className="card" style={{ padding: '20px' }}>
            ...
            <p style={{ fontSize: '1.125rem', fontWeight: '700', margin: '0', color: 'var(--success)' }}>
              {linea.id === 'LOOP' || linea.id === 'OMNI' ? 'FREE' : '$2.75 USD'}
            </p>
          </div>
        </div>
        ...
        <div style={{ backgroundColor: 'var(--surface)', padding: '32px', borderLeft: `4px solid ${linea.color}`, borderRadius: 'var(--radius)', marginBottom: '60px' }}>
          ...
          <p style={{ fontSize: '1rem', color: 'var(--text)', margin: '0 0 16px 0', lineHeight: '1.8' }}>
            {linea.descripcion || `Line ${linea.id} is a key part of Miami's transit system, connecting major areas and attractions.`}
          </p>
        </div>
