...
export async function generateMetadata({ params }) {
  const l = lineasSaoPaulo.find(l => l.id === params.id);
  if (!l) return { title: 'Linha não encontrada' };
  return {
    title: l.seo_title, description: l.meta_description,
    openGraph: { title: l.seo_title, url: `https://metroguia.mx/sao-paulo/line/${l.id}/` },
    alternates: { canonical: `https://metroguia.mx/sao-paulo/line/${l.id}/` },
  };
}

export default function LineSaoPaulo({ params }) {
  const linea = lineasSaoPaulo.find(l => l.id === params.id);
  if (!linea) {
    return <main style={{ padding: '80px 24px', textAlign: 'center', backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
      <h1>Linha não encontrada</h1>
      <Link href="/sao-paulo/"><button style={{ marginTop: 24, padding: '12px 24px', background: '#0052A5', color: '#fff', border: 'none', borderRadius: 'var(--radius)', fontWeight: 700, cursor: 'pointer' }}>Voltar</button></Link>
    </main>;
  }

  const ests = estacionesSaoPaulo.filter(e => (Array.isArray(e.linea) ? e.linea : [e.linea]).includes(linea.id));
    ...
    <main style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
      ...
      <section style={{ background: `linear-gradient(135deg, ${linea.color} 0%, ${linea.color}cc 100%)`, color: '#fff', padding: '60px 24px' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          ...
          <p style={{ opacity: 0.85, margin: 0 }}>{linea.total} estações</p>
        </div>
      </section>

      <section style={{ maxWidth: '1200px', margin: '0 auto', padding: '60px 24px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: 16, marginBottom: 48 }}>
          {[{ label:'Terminais', val:`${linea.inicio} → ${linea.fin}`},{label:'Estações',val:linea.total},{label:'Tarifa',val:'R$ 5,00'},{label:'Horário',val:'04:40–00:00'}].map(c=>(
        ...
        <div style={{ background:'var(--surface)', padding:32, borderLeft:`4px solid ${linea.color}`, borderRadius:'var(--radius)', marginBottom:48 }}>
          <h2 style={{ fontSize:'1.4rem', fontWeight:700, margin:'0 0 16px 0' }}>Sobre a {linea.colorNombre}</h2>
        ...
        <div style={{ marginTop:48 }}>
          <h2 style={{ fontSize:'1.5rem', fontWeight:700, margin:'0 0 24px 0', borderBottom:`3px solid ${linea.color}`, paddingBottom:16 }}>Estações ({ests.length})</h2>
      ...
      <section style={{ backgroundColor:'var(--surface)', padding:'60px 24px', textAlign:'center' }}>
        <Link href="/sao-paulo/"><button style={{ padding:'14px 32px', background:linea.color, color:'#fff', border:'none', borderRadius:'var(--radius)', fontWeight:700, cursor:'pointer' }}>Ver todas as linhas</button></Link>
      </section>
    </main>
  );
}
