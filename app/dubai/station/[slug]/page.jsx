...
export async function generateMetadata({ params }) {
  ...
  return { title: e.seo_title, description: e.meta_description, openGraph: { title: e.seo_title, url: `https://metroguia.mx/dubai/station/${e.slug}/` }, alternates: { canonical: `https://metroguia.mx/dubai/station/${e.slug}/` } };
}

export default function DubaiStation({ params }) {
  const estacion = estacionesDubai.find((e) => e.slug === params.slug);
  if (!estacion) return <main style={{ padding: '80px 24px', textAlign: 'center' }}><h1>Station not found</h1><Link href="/dubai">Back to Dubai</Link></main>;
  const lineaArr = Array.isArray(estacion.linea) ? estacion.linea : [estacion.linea];
  const lineasEstacion = lineaArr.map(id => lineasDubai.find(l => l.id === id)).filter(Boolean);
          ...
          <div>
            <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: '1.25rem', marginBottom: 16 }}>
              ...
              <div style={{ display: 'grid', gap: 10 }}>
                ...
                {estacion.horarios && <div><p style={{ fontSize: '0.7rem', color: 'var(--text-muted)', textTransform: 'uppercase', margin: '0 0 2px', fontWeight: 600 }}>Hours</p><p style={{ margin: 0, fontSize: '0.85rem' }}>{estacion.horarios.notas}</p></div>}
              </div>
            </div>
