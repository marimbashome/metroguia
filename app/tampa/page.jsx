...
export const metadata = {
  title: 'TECO Line Streetcar — Tampa Transit | MetroGuia',
  description: 'TECO Line Streetcar guide: 1 line, 11 stations in Tampa. Complete trip planner, schedule & fares.',
  keywords: 'TECO Line Streetcar, Tampa transit, Streetcar',
  openGraph: {
    title: 'TECO Line Streetcar — Tampa Transit Guide',
    description: 'Navigate Tampa with TECO Line Streetcar. Trip planner, fares, and station guide.',
    url: 'https://metroguia.mx/tampa',
    type: 'website',
  },
  robots: { index: true, follow: true },
};

const estacionesDestacadas = estacionesTampa.filter(e =>
  ['tampa-channelside', 'tampa-tampa-convention', 'tampa-florida-ave', 'tampa-ybor-city'].includes(e.slug)
);
...
export default function TampaPage() {
  ...
  return (
    <main style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
      ...
      {/* HERO */}
      <section style={{
        background: 'linear-gradient(135deg, #0033A0 0%, #0033A0cc 100%)',
        color: '#FFFFFF',
        padding: '80px 24px',
        textAlign: 'center',
        position: 'relative',
        ...
        <div style={{ maxWidth: '1000px', marginLeft: 'auto', marginRight: 'auto' }}>
          ...
          <p style={{ fontSize: '1.125rem', fontWeight: '400', margin: '0', opacity: '0.9', maxWidth: '600px', marginLeft: 'auto', marginRight: 'auto' }}>
            Tampa's TECO Line Streetcar — your guide to navigating the city by rail.
          </p>
        </div>
      </section>
      ...
      <section style={{ backgroundColor: 'var(--surface)', padding: '48px 24px', borderBottom: '1px solid var(--border)' }}>
        <div style={{ maxWidth: '1000px', marginLeft: 'auto', marginRight: 'auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '32px', textAlign: 'center' }}>
          <div>
            <p style={{ fontSize: '2.5rem', fontWeight: '800', color: '#0033A0', margin: '0 0 8px 0' }}>11</p>
            <p style={{ fontSize: '1rem', color: 'var(--text-muted)', margin: '0', fontWeight: '500' }}>Stations</p>
          </div>
          <div>
            <p style={{ fontSize: '2.5rem', fontWeight: '800', color: '#0033A0', margin: '0 0 8px 0' }}>1</p>
            <p style={{ fontSize: '1rem', color: 'var(--text-muted)', margin: '0', fontWeight: '500' }}>Active Lines</p>
          </div>
          <div>
            <p style={{ fontSize: '2.5rem', fontWeight: '800', color: '#0033A0', margin: '0 0 8px 0' }}>20 min</p>
      ...
      <section style={{ backgroundColor: 'var(--bg)', padding: '60px 24px', borderBottom: '1px solid var(--border)' }}>
        <div style={{ maxWidth: '700px', marginLeft: 'auto', marginRight: 'auto' }}>
          <h2 style={{ fontSize: '1.75rem', fontWeight: '800', marginBottom: '12px', textAlign: 'center', color: 'var(--text)' }}>
            Plan Your Route in Tampa
          </h2>
          <p style={{ fontSize: '1rem', textAlign: 'center', marginBottom: '24px', color: 'var(--text-muted)' }}>
            Calculate the best route between TECO Line Streetcar stations
          </p>
          <SearchBar ciudad="tampa" />
        </div>
      </section>
            ...
            {estacionesDestacadas.map((estacion, idx) => (
              <Link href={`/tampa/station/${estacion.slug}`} key={idx} style={{ textDecoration: 'none' }}>
                <div style={{ padding: '28px', backgroundColor: 'var(--surface)', borderRadius: 'var(--radius)', border: '1px solid var(--border)', cursor: 'pointer', transition: 'all 0.3s ease' }}>
                  <h3 style={{ fontSize: '1.25rem', fontWeight: '700', margin: '0 0 8px 0', color: '#0033A0' }}>
                    {estacion.nombre}
                  </h3>
      ...
      <section style={{ backgroundColor: 'var(--bg)', padding: '80px 24px' }}>
        <div style={{ maxWidth: '1000px', marginLeft: 'auto', marginRight: 'auto' }}>
          <h2 style={{ fontSize: '2rem', fontWeight: '800', margin: '0 0 48px 0', color: 'var(--text)', textAlign: 'center' }}>
            TECO Line Streetcar Lines
          </h2>
          <div style={{ display: 'grid', gap: '20px' }}>
            {lineasTampa.map((linea, idx) => (
              <Link href={`/tampa/line/${linea.id}`} key={idx} style={{ textDecoration: 'none' }}>
