...
export default function LineTampaPage({ params }) {
  ...
  return (
    <main style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
      <IntlLineSchema line={(lineasTampa.find(l => l.id === params.id)) || null} stations={((lineasTampa.find(l => l.id === params.id) || {}).estaciones || []).map(s => (typeof s === 'string' ? (estacionesTampa.find(x => x.slug === s) || { slug: s, nombre: s }) : s))} city="Tampa" citySlug="tampa" linePathPrefix="/tampa/line" stationPathPrefix="/tampa/station" country="US" systemName="TECO Line Streetcar" locale="en" />
      {/* HERO */}
