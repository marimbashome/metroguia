...
export async function generateStaticParams() {
  return estacionesQueretaro.map((estacion) => ({ slug: estacion.slug }));
}

export async function generateMetadata({ params }) {
  const estacion = estacionesQueretaro.find((e) => e.slug === params.slug);
  if (!estacion) return { title: 'Station not found' };
  const seoTitle = estacion.seo_title || `${estacion.nombre} Station — Querétaro Transit | MetroGuia`;
  const metaDesc = estacion.meta_description || estacion.descripcion_turistica || `Guide to ${estacion.nombre} station in Querétaro`;
  return {
    title: seoTitle,
    ...
    openGraph: {
      title: seoTitle,
      description: metaDesc,
      url: `https://metroguia.mx/queretaro/station/${estacion.slug}`,
      type: 'website',
    },
    robots: { index: true, follow: true },
    alternates: {
      canonical: `https://metroguia.mx/queretaro/station/${estacion.slug}`,
      languages: {
        'es': `https://metroguia.mx/queretaro/estacion/${estacion.slug}`,
        'en': `https://metroguia.mx/queretaro/station/${estacion.slug}`,
      },
    },
  };
}

export default function StationQueretaroPage({ params }) {
  const estacion = estacionesQueretaro.find((e) => e.slug === params.slug);
  if (!estacion) {
    return (
      <main style={{ padding: '40px 20px', textAlign: 'center'}} >
        ...
        <p style={{ marginTop: '20px'}} >
          <Link href="/queretaro" style={{ color: '#059669', textDecoration: 'underline'}} >Back to Querétaro</Link>
        </p>
      </main>
    );
  }
  ...
  const linea = lineasQueretaro.find((l) => l.id === lineaId);
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'Querétaro', item: 'https://metroguia.mx/queretaro/' },
      { '@type': 'ListItem', position: 3, name: `Line ${lineaId}`, item: `https://metroguia.mx/queretaro/line/${lineaId}/` },
      { '@type': 'ListItem', position: 4, name: estacion.h1 || estacion.nombre, item: `https://metroguia.mx/queretaro/station/${estacion.slug}/` }
    ]
  };
    ...
    <main style={{ backgroundColor: '#ffffff'}} >
      ...
      <section style={heroStyles}>
        <div style={containerStyles}>
          <p style={{ fontSize: '14px', marginBottom: '10px', opacity: '0.9'}} >
            Line {lineaId} • {estacion.municipio || 'Querétaro'}
          </p>
      ...
      <section style={{ ...containerStyles, paddingTop: '40px', paddingBottom: '40px', textAlign: 'center'}} >
        <div style={{ display: 'flex', gap: '10px', justifyContent: 'center', flexWrap: 'wrap'}} >
          <Link href="/queretaro" style={buttonStyles}>Back to Querétaro</Link>
          {linea && <Link href={`/queretaro/line/${linea.id}`} style={buttonStyles}>View Line {linea.id}</Link>}
        </div>
      </section>
    </main>
  );
