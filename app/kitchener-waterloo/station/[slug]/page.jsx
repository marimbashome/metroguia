...
const LINE_COLORS = {"ion":"#0066CC"};

export async function generateStaticParams() {
  return estacionesKitchenerWaterloo.map((estacion) => ({
    slug: estacion.slug,
  }));
}

export async function generateMetadata({ params }) {
  const estacion = estacionesKitchenerWaterloo.find((e) => e.slug === params.slug);
  if (!estacion) {
  ...
  return {
    ...
    openGraph: {
      title: estacion.seo_title,
      description: estacion.meta_description,
      url: `https://metroguia.mx/kitchener-waterloo/station/${estacion.slug}`,
    },
  };
}

export default function StationKitchenerWaterlooPage({ params }) {
  const estacion = estacionesKitchenerWaterloo.find((e) => e.slug === params.slug);
  ...
  const colorLinea = LINE_COLORS[lineaArray[0]] || '#0066CC';
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: `Line ${lineaId}`, item: `https://metroguia.mx/kitchener-waterloo/line/${lineaId}/` },
      { '@type': 'ListItem', position: 4, name: estacion.nombre, item: `https://metroguia.mx/kitchener-waterloo/station/${estacion.slug}/` },
    ]
  };
  ...
  const stationSchema = {
    ...
    address: { '@type': 'PostalAddress', addressLocality: 'Kitchener-Waterloo', addressRegion: 'Ontario', addressCountry: 'CA' },
  };
      ...
      <section style={{ background: `linear-gradient(135deg, ${colorLinea} 0%, ${colorLinea}cc 100%)`, color: '#fff', padding: '60px 24px' }}>
        <div style={{ maxWidth: '1000px', margin: '0 auto' }}>
          <div style={{ marginBottom: '8px' }}>
            <Link href="/kitchener-waterloo" style={{ color: 'rgba(255,255,255,0.8)', textDecoration: 'none', fontSize: '0.9rem' }}>
              ← Kitchener-Waterloo
            </Link>
          </div>
          <h1 style={{ fontSize: '2.5rem', fontWeight: '800', margin: '0 0 8px 0' }}>{estacion.nombre}</h1>
          <p style={{ fontSize: '1.125rem', opacity: 0.9, margin: 0 }}>
            {lineaArray.map(l => `${l.charAt(0).toUpperCase() + l.slice(1)} Line`).join(' · ')} — Ion
          </p>
        </div>
      </section>
      ...
      <section style={{ padding: '60px 24px' }}>
        <div style={{ maxWidth: '800px', margin: '0 auto' }}>
          ...
          <div style={{ marginTop: '48px', textAlign: 'center' }}>
            <Link href={`/kitchener-waterloo/line/${lineaId}`}>
