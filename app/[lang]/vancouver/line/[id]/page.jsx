...
export async function generateMetadata({ params }) {
  ...
  return buildMetadata({
    lang: params.lang,
    title: `${linea.nombre || linea.name || 'Line ' + linea.id} — Vancouver Transit | MetroGuia`,
    description: linea.meta_description || linea.descripcion || `${linea.nombre || linea.name || 'Line ' + linea.id} in Vancouver`,
    path: `/vancouver/line/${linea.id}/`,
    keywords: ['Vancouver', linea.nombre || linea.name || linea.id, 'transit line'],
  });
}

export default function LineVancouverPageLang({ params }) {
  ...
  const lineColor = linea.color || '#0061A8';
  const lineName = linea.nombre || linea.name || `Line ${linea.id}`;
  const estacionesLinea = estacionesVANCOUVER.filter(e => {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: lineName, item: `https://metroguia.mx/${lang}/vancouver/line/${linea.id}` },
    ],
  };
        ...
        <div style={{ marginBottom: '60px' }}>
          ...
          <div style={{ display: 'grid', gap: '16px' }}>
            {estacionesLinea.map((estacion, idx) => (
              <Link href={`/${lang}/vancouver/station/${estacion.slug}`} key={idx} style={{ textDecoration: 'none' }}>
