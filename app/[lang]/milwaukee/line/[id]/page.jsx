...
export async function generateMetadata({ params }) {
  ...
  return buildMetadata({
    lang: params.lang,
    title: `${linea.nombre || linea.name || 'Line ' + linea.id} — Milwaukee Transit | MetroGuia`,
    description: linea.meta_description || linea.descripcion || `${linea.nombre || linea.name || 'Line ' + linea.id} in Milwaukee`,
    path: `/milwaukee/line/${linea.id}/`,
    keywords: ['Milwaukee', linea.nombre || linea.name || linea.id, 'transit line'],
  });
}

export default function LineMilwaukeePageLang({ params }) {
  ...
  const lineColor = linea.color || '#0A3161';
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: lineName, item: `https://metroguia.mx/${lang}/milwaukee/line/${linea.id}` },
    ],
  };
        ...
        <div style={{ marginBottom: '60px' }}>
          ...
          <div style={{ display: 'grid', gap: '16px' }}>
            {estacionesLinea.map((estacion, idx) => (
              <Link href={`/${lang}/milwaukee/station/${estacion.slug}`} key={idx} style={{ textDecoration: 'none' }}>
