...
export async function generateMetadata({ params }) {
  const linea = lineasHonolulu.find(l => l.id === params.id);
  if (!linea) return { title: 'Line not found — MetroGuia' };
  return buildMetadata({
    lang: params.lang,
    title: `${linea.nombre || linea.name || 'Line ' + linea.id} — Honolulu Transit | MetroGuia`,
    description: linea.meta_description || linea.descripcion || `${linea.nombre || linea.name || 'Line ' + linea.id} in Honolulu`,
    path: `/honolulu/line/${linea.id}/`,
    keywords: ['Honolulu', linea.nombre || linea.name || linea.id, 'transit line'],
  });
}

export default function LineHonoluluPageLang({ params }) {
  ...
  const lineColor = linea.color || '#0077BE';
  const lineName = linea.nombre || linea.name || `Line ${linea.id}`;
  const estacionesLinea = estacionesHonolulu.filter(e => {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: lineName, item: `https://metroguia.mx/${lang}/honolulu/line/${linea.id}` },
    ],
  };
        ...
        <div style={{ marginBottom: '60px' }}>
          ...
          <div style={{ display: 'grid', gap: '16px' }}>
            {estacionesLinea.map((estacion, idx) => (
              <Link href={`/${lang}/honolulu/station/${estacion.slug}`} key={idx} style={{ textDecoration: 'none' }}>
