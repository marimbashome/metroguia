...
export async function generateMetadata({ params }) {
  const linea = lineasBuffalo.find(l => l.id === params.id);
  if (!linea) return { title: 'Line not found — MetroGuia' };
  return buildMetadata({
    lang: params.lang,
    title: `${linea.nombre || linea.name || 'Line ' + linea.id} — Buffalo Transit | MetroGuia`,
    description: linea.meta_description || linea.descripcion || `${linea.nombre || linea.name || 'Line ' + linea.id} in Buffalo`,
    path: `/buffalo/line/${linea.id}/`,
    keywords: ['Buffalo', linea.nombre || linea.name || linea.id, 'transit line'],
  });
}

export default function LineBuffaloPageLang({ params }) {
  ...
  const lineColor = linea.color || '#003DA5';
  const lineName = linea.nombre || linea.name || `Line ${linea.id}`;
  const estacionesLinea = estacionesBuffalo.filter(e => {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: lineName, item: `https://metroguia.mx/${lang}/buffalo/line/${linea.id}` },
    ],
  };
        ...
        <div style={{ marginBottom: '60px' }}>
          ...
          <div style={{ display: 'grid', gap: '16px' }}>
            {estacionesLinea.map((estacion, idx) => (
              <Link href={`/${lang}/buffalo/station/${estacion.slug}`} key={idx} style={{ textDecoration: 'none' }}>
