...
export async function generateMetadata({ params }) {
  const linea = lineasOrlando.find(l => l.id === params.id);
  if (!linea) return { title: 'Line not found — MetroGuia' };
  return buildMetadata({
    lang: params.lang,
    title: `${linea.nombre || linea.name || 'Line ' + linea.id} — Orlando Transit | MetroGuia`,
    description: linea.meta_description || linea.descripcion || `${linea.nombre || linea.name || 'Line ' + linea.id} in Orlando`,
    path: `/orlando/line/${linea.id}/`,
    keywords: ['Orlando', linea.nombre || linea.name || linea.id, 'transit line'],
  });
}

export default function LineOrlandoPageLang({ params }) {
  ...
  if (!linea) {
    return (
      <main style={{ padding: '80px 24px', textAlign: 'center', backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
        ...
        <Link href={`/${lang}/orlando`}>
          <button style={{ marginTop: '24px', padding: '12px 24px', backgroundColor: '#0057B7', color: '#fff', border: 'none', borderRadius: 'var(--radius)', fontSize: '1rem', fontWeight: '700', cursor: 'pointer' }}>
            ← Orlando
          </button>
        </Link>
      </main>
  ...
  const lineColor = linea.color || '#0057B7';
  const lineName = linea.nombre || linea.name || `Line ${linea.id}`;
  const estacionesLinea = estacionesOrlando.filter(e => {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: lineName, item: `https://metroguia.mx/${lang}/orlando/line/${linea.id}` },
    ],
  };
        ...
        <div style={{ marginBottom: '60px' }}>
          ...
          <div style={{ display: 'grid', gap: '16px' }}>
            {estacionesLinea.map((estacion, idx) => (
              <Link href={`/${lang}/orlando/station/${estacion.slug}`} key={idx} style={{ textDecoration: 'none' }}>
