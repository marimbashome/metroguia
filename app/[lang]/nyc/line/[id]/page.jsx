...
export async function generateMetadata({ params }) {
  const linea = lineasNYC.find(l => l.id === params.id);
  if (!linea) return { title: 'Line not found — MetroGuia' };
  return buildMetadata({
    lang: params.lang,
    title: `${linea.nombre || linea.name || 'Line ' + linea.id} — New York City Transit | MetroGuia`,
    description: linea.meta_description || linea.descripcion || `${linea.nombre || linea.name || 'Line ' + linea.id} in New York City`,
    path: `/nyc/line/${linea.id}/`,
    keywords: ['New York City', linea.nombre || linea.name || linea.id, 'transit line'],
  });
}

export default function LineNycPageLang({ params }) {
  ...
  const lineColor = linea.color || '#FF4F5E';
  const lineName = linea.nombre || linea.name || `Line ${linea.id}`;
  const estacionesLinea = estacionesNYC.filter(e => {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: lineName, item: `https://metroguia.mx/${lang}/nyc/line/${linea.id}` },
    ],
  };
        ...
        <div style={{ marginBottom: '60px' }}>
          ...
          <div style={{ display: 'grid', gap: '16px' }}>
            {estacionesLinea.map((estacion, idx) => (
              <Link href={`/${lang}/nyc/station/${estacion.slug}`} key={idx} style={{ textDecoration: 'none' }}>
