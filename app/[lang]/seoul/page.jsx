...
export function generateStaticParams() {
  const defaultLang = getDefaultLangForCity('seoul') || 'en';
...
export async function generateMetadata({ params }) {
  return buildMetadata({
    lang: params.lang,
    title: 'Seoul Metro Guide | MetroGuia',
    description: 'South Korea · 9 lines · 300+ stations. Complete metro guide for tourists and commuters.',
    path: '/seoul/',
    keywords: ['seoul metro', 'seoul transit', 'seoul subway'],
  });
}

export default function SeoulPageLang({ params }) {
  const lang = params.lang;
  return (
    <main style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
      <section style={{ background: 'linear-gradient(135deg, #00A84D 0%, #00A84Dcc 100%)', color: '#fff', padding: '80px 24px' }}>
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
          <p style={{ fontSize: '0.75rem', color: 'rgba(255,255,255,0.85)', letterSpacing: '0.15em', textTransform: 'uppercase', fontWeight: 600, marginBottom: '0.75rem' }}>🇰🇷 Seoul Metro · South Korea</p>
          <h1 style={{ fontSize: 'clamp(1.75rem, 5vw, 2.75rem)', fontWeight: 800, marginBottom: '1rem' }}>Seoul Metro</h1>
          <p style={{ fontSize: '1.1rem', color: 'rgba(255,255,255,0.9)', marginBottom: '2rem' }}>9 lines · 300+ stations</p>
          <SearchBar />
        </div>
      </section>
      <section style={{ padding: '3rem 1.5rem', maxWidth: 900, margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '1rem' }}>
          {lineasSeoul.map(linea => (
            <Link key={linea.id} href={`/${lang}/seoul/line/${linea.id}/`} style={{ textDecoration: 'none' }}>
      ...
      <section style={{ padding: '2rem 1.5rem', maxWidth: 900, margin: '0 auto', borderTop: '1px solid var(--border)' }}>
        <Link href="/seoul/" style={{ color: 'var(--text-muted)', textDecoration: 'none', fontSize: '0.9rem' }}>View in English →</Link>
      </section>
    </main>
  );
}
