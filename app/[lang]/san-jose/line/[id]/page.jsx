...
export async function generateMetadata({ params }) {
  const linea = lineasSanJose.find(l => l.id === params.id);
  if (!linea) return { title: 'Line not found — MetroGuia' };
  return buildMetadata({
    lang: params.lang,
    title: `${linea.nombre || linea.name || 'Line ' + linea.id} — San Jose Transit | MetroGuia`,
    description: linea.meta_description || linea.descripcion || `${linea.nombre || linea.name || 'Line ' + linea.id} in San Jose`,
    path: `/san-jose/line/${linea.id}/`,
    keywords: ['San Jose', linea.nombre || linea.name || linea.id, 'transit line'],
  });
}

export default function LineSanJosePageLang({ params }) {
  ...
  const lineColor = linea.color || '#0070C0';
  const lineName = linea.nombre || linea.name || `Line ${linea.id}`;
  const estacionesLinea = estacionesSanJose.filter(e => {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: lineName, item: `https://metroguia.mx/${lang}/san-jose/line/${linea.id}` },
    ],
  };
        ...
        <div style={{ marginBottom: '60px' }}>
          ...
          <div style={{ display: 'grid', gap: '16px' }}>
            {estacionesLinea.map((estacion, idx) => (
              <Link href={`/${lang}/san-jose/station/${estacion.slug}`} key={idx} style={{ textDecoration: 'none' }}>
