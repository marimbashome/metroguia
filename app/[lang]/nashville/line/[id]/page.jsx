...
export async function generateMetadata({ params }) {
  const linea = lineasNashville.find(l => l.id === params.id);
  if (!linea) return { title: 'Line not found — MetroGuia' };
  return buildMetadata({
    lang: params.lang,
    title: `${linea.nombre || linea.name || 'Line ' + linea.id} — Nashville Transit | MetroGuia`,
    description: linea.meta_description || linea.descripcion || `${linea.nombre || linea.name || 'Line ' + linea.id} in Nashville`,
    path: `/nashville/line/${linea.id}/`,
    keywords: ['Nashville', linea.nombre || linea.name || linea.id, 'transit line'],
  });
}

export default function LineNashvillePageLang({ params }) {
  ...
  const lineColor = linea.color || '#C8102E';
  const lineName = linea.nombre || linea.name || `Line ${linea.id}`;
  const estacionesLinea = estacionesNashville.filter(e => {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: lineName, item: `https://metroguia.mx/${lang}/nashville/line/${linea.id}` },
    ],
  };
        ...
        <div style={{ marginBottom: '60px' }}>
          ...
          <div style={{ display: 'grid', gap: '16px' }}>
            {estacionesLinea.map((estacion, idx) => (
              <Link href={`/${lang}/nashville/station/${estacion.slug}`} key={idx} style={{ textDecoration: 'none' }}>
