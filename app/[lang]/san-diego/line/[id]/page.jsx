...
export function generateStaticParams() {
  ...
  LANGUAGES.filter(l => l !== defaultLang).forEach(lang => {
    lineasDetalleSanDiego.forEach(linea => {
...
export async function generateMetadata({ params }) {
  ...
  return buildMetadata({
    lang: params.lang,
    title: `${linea.nombre || linea.name || 'Line ' + linea.id} — San Diego Transit | MetroGuia`,
    description: linea.meta_description || linea.descripcion || `${linea.nombre || linea.name || 'Line ' + linea.id} in San Diego`,
    path: `/san-diego/line/${linea.id}/`,
    keywords: ['San Diego', linea.nombre || linea.name || linea.id, 'transit line'],
  });
}

export default function LineSanDiegoPageLang({ params }) {
  ...
  const lineColor = linea.color || '#E31837';
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: lineName, item: `https://metroguia.mx/${lang}/san-diego/line/${linea.id}` },
    ],
  };
        ...
        <div style={{ marginBottom: '60px' }}>
          ...
          <div style={{ display: 'grid', gap: '16px' }}>
            {estacionesLinea.map((estacion, idx) => (
              <Link href={`/${lang}/san-diego/station/${estacion.slug}`} key={idx} style={{ textDecoration: 'none' }}>
