...
export async function generateMetadata({ params }) {
  const linea = lineasTucson.find(l => l.id === params.id);
  if (!linea) return { title: 'Line not found — MetroGuia' };
  return buildMetadata({
    lang: params.lang,
    title: `${linea.nombre || linea.name || 'Line ' + linea.id} — Tucson Transit | MetroGuia`,
    description: linea.meta_description || linea.descripcion || `${linea.nombre || linea.name || 'Line ' + linea.id} in Tucson`,
    path: `/tucson/line/${linea.id}/`,
    keywords: ['Tucson', linea.nombre || linea.name || linea.id, 'transit line'],
  });
}

export default function LineTucsonPageLang({ params }) {
  ...
  const lineColor = linea.color || '#CC0033';
  const lineName = linea.nombre || linea.name || `Line ${linea.id}`;
  const estacionesLinea = estacionesTucson.filter(e => {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: lineName, item: `https://metroguia.mx/${lang}/tucson/line/${linea.id}` },
    ],
  };
        ...
        <div style={{ marginBottom: '60px' }}>
          ...
          <div style={{ display: 'grid', gap: '16px' }}>
            {estacionesLinea.map((estacion, idx) => (
              <Link href={`/${lang}/tucson/station/${estacion.slug}`} key={idx} style={{ textDecoration: 'none' }}>
