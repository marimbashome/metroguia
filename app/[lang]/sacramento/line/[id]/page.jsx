...
export function generateStaticParams() {
  ...
  LANGUAGES.filter(l => l !== defaultLang).forEach(lang => {
    lineasSacramento.forEach(linea => {
...
export async function generateMetadata({ params }) {
  const linea = lineasSacramento.find(l => l.id === params.id);
  if (!linea) return { title: 'Line not found — MetroGuia' };
  return buildMetadata({
    lang: params.lang,
    title: `${linea.nombre || linea.name || 'Line ' + linea.id} — Sacramento Transit | MetroGuia`,
    description: linea.meta_description || linea.descripcion || `${linea.nombre || linea.name || 'Line ' + linea.id} in Sacramento`,
    path: `/sacramento/line/${linea.id}/`,
    keywords: ['Sacramento', linea.nombre || linea.name || linea.id, 'transit line'],
  });
}

export default function LineSacramentoPageLang({ params }) {
  ...
  if (!linea) {
    return (
      <main style={{ padding: '80px 24px', textAlign: 'center', backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
        ...
        <Link href={`/${lang}/sacramento`}>
          <button style={{ marginTop: '24px', padding: '12px 24px', backgroundColor: '#0068B3', color: '#fff', border: 'none', borderRadius: 'var(--radius)', fontSize: '1rem', fontWeight: '700', cursor: 'pointer' }}>
            ← Sacramento
          </button>
        </Link>
      </main>
  ...
  const lineColor = linea.color || '#0068B3';
  const lineName = linea.nombre || linea.name || `Line ${linea.id}`;
  const estacionesLinea = estacionesSacramento.filter(e => {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: lineName, item: `https://metroguia.mx/${lang}/sacramento/line/${linea.id}` },
    ],
  };
        ...
        <div style={{ marginBottom: '60px' }}>
          ...
          <div style={{ display: 'grid', gap: '16px' }}>
            {estacionesLinea.map((estacion, idx) => (
              <Link href={`/${lang}/sacramento/station/${estacion.slug}`} key={idx} style={{ textDecoration: 'none' }}>
