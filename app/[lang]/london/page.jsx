...
export function generateStaticParams() {
  const defaultLang = getDefaultLangForCity('london');
...
export async function generateMetadata({ params }) {
  return buildMetadata({
    ...
    description: 'Madrid Metro guide: 11 lines, 272 stations. Sol, Gran Vía, Atocha, Retiro, Ópera. Tourist route planner.',
    path: '/london/',
...
export default function MadridPageLang({ params }) {
  ...
  const estacionesDestacadas = estacionesLondon.filter(e =>
    ['london-sol', 'london-gran-via', 'london-atocha', 'london-opera', 'london-callao', 'london-retiro'].includes(e.slug)
  );
    ...
    <main style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
      ...
      <section style={{ background: 'linear-gradient(135deg, var(--surface) 0%, rgba(0,170,220,0.08) 100%)', padding: '5rem 1.5rem 3rem', borderBottom: '1px solid var(--border)' }}>
        <div className="container" style={{ maxWidth: 900, margin: '0 auto' }}>
          ...
          <p style={{ fontSize: '1.1rem', color: 'var(--text-muted)', marginBottom: '2rem' }}>11 lines · 272 stations · 402 km — Spain's largest london network</p>
          <SearchBar />
        </div>
      </section>
      ...
      <section style={{ padding: '3rem 1.5rem', maxWidth: 900, margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(200px,1fr))', gap: '1rem' }}>
          {lineasLondon.map(linea => (
            <Link key={linea.id} href={`/${lang}/london/line/${linea.id}/`} style={{ textDecoration: 'none' }}>
      ...
      <section style={{ padding: '2rem 1.5rem', maxWidth: 900, margin: '0 auto', borderTop: '1px solid var(--border)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(220px,1fr))', gap: '1rem' }}>
          {estacionesDestacadas.map(e => {
            ...
            const linea = lineasLondon.find(l => l.id === lineaArr[0]);
            return (
              <Link key={e.slug} href={`/${lang}/london/station/${e.slug}/`} style={{ textDecoration: 'none' }}>
      ...
      <section style={{ padding: '1.5rem', maxWidth: 900, margin: '0 auto', borderTop: '1px solid var(--border)' }}>
        <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>
          También disponible en: <Link href="/london/" style={{ color: 'var(--primary)', textDecoration: 'none' }}>Español</Link>
        </p>
      </section>
