...
export function generateStaticParams() {
  ...
  LANGUAGES.filter(l => l !== defaultLang).forEach(lang => {
    estacionesOttawa.forEach(estacion => {
...
export async function generateMetadata({ params }) {
  const estacion = estacionesOttawa.find(e => e.slug === params.slug);
  if (!estacion) return { title: 'Station not found — MetroGuia' };
  return buildMetadata({
    lang: params.lang,
    title: `${estacion.nombre} — Ottawa Transit | MetroGuia`,
    description: estacion.meta_description || estacion.descripcion || estacion.intro || `${estacion.nombre} station in Ottawa`,
    path: `/ottawa/station/${estacion.slug}/`,
    keywords: ['Ottawa', estacion.nombre, 'transit', 'station'],
  });
}

export default function StationOttawaPageLang({ params }) {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: estacion.nombre, item: `https://metroguia.mx/${lang}/ottawa/station/${estacion.slug}` },
    ],
  };
              ...
              <div style={{ marginBottom: '48px' }}>
                ...
                <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                  {lineas.map((lineId, idx) => (
                    <Link href={`/${lang}/ottawa/line/${lineId}`} key={idx} style={{ textDecoration: 'none' }}>
