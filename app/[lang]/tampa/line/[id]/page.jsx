...
export async function generateMetadata({ params }) {
  ...
  return buildMetadata({
    lang: params.lang,
    title: `${linea.nombre || linea.name || 'Line ' + linea.id} — Tampa Transit | MetroGuia`,
    description: linea.meta_description || linea.descripcion || `${linea.nombre || linea.name || 'Line ' + linea.id} in Tampa`,
    path: `/tampa/line/${linea.id}/`,
    keywords: ['Tampa', linea.nombre || linea.name || linea.id, 'transit line'],
  });
}

export default function LineTampaPageLang({ params }) {
  ...
  const lineColor = linea.color || '#005C5C';
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: lineName, item: `https://metroguia.mx/${lang}/tampa/line/${linea.id}` },
    ],
  };
        ...
        <div style={{ marginBottom: '60px' }}>
          ...
          <div style={{ display: 'grid', gap: '16px' }}>
            {estacionesLinea.map((estacion, idx) => (
              <Link href={`/${lang}/tampa/station/${estacion.slug}`} key={idx} style={{ textDecoration: 'none' }}>
