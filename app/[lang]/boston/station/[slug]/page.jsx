...
export function generateStaticParams() {
  ...
  LANGUAGES.filter(l => l !== defaultLang).forEach(lang => {
    estacionesBoston.forEach(estacion => {
...
export async function generateMetadata({ params }) {
  ...
  return buildMetadata({
    lang: params.lang,
    title: `${estacion.nombre} — Boston Transit | MetroGuia`,
    description: estacion.meta_description || estacion.descripcion || estacion.intro || `${estacion.nombre} station in Boston`,
    path: `/boston/station/${estacion.slug}/`,
    keywords: ['Boston', estacion.nombre, 'transit', 'station'],
  });
}

export default function StationBostonPageLang({ params }) {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: estacion.nombre, item: `https://metroguia.mx/${lang}/boston/station/${estacion.slug}` },
    ],
  };
  ...
  const borough = estacion.borough || estacion.municipio || estacion.zona || 'Massachusetts';
              ...
              <div style={{ marginBottom: '48px' }}>
                ...
                <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                  {lineas.map((lineId, idx) => (
                    <Link href={`/${lang}/boston/line/${lineId}`} key={idx} style={{ textDecoration: 'none' }}>
