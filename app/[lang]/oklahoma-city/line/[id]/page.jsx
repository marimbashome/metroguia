...
export function generateStaticParams() {
  ...
  LANGUAGES.filter(l => l !== defaultLang).forEach(lang => {
    lineasOklahomaCity.forEach(linea => {
...
export async function generateMetadata({ params }) {
  ...
  return buildMetadata({
    lang: params.lang,
    title: `${linea.nombre || linea.name || 'Line ' + linea.id} — Oklahoma City Transit | MetroGuia`,
    description: linea.meta_description || linea.descripcion || `${linea.nombre || linea.name || 'Line ' + linea.id} in Oklahoma City`,
    path: `/oklahoma-city/line/${linea.id}/`,
    keywords: ['Oklahoma City', linea.nombre || linea.name || linea.id, 'transit line'],
  });
}

export default function LineOklahomaCityPageLang({ params }) {
  ...
  const lineColor = linea.color || '#DC143C';
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: lineName, item: `https://metroguia.mx/${lang}/oklahoma-city/line/${linea.id}` },
    ],
  };
        ...
        <div style={{ marginBottom: '60px' }}>
          ...
          <div style={{ display: 'grid', gap: '16px' }}>
            {estacionesLinea.map((estacion, idx) => (
              <Link href={`/${lang}/oklahoma-city/station/${estacion.slug}`} key={idx} style={{ textDecoration: 'none' }}>
