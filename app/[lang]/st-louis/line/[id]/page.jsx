...
export async function generateMetadata({ params }) {
  const linea = lineasStLouis.find(l => l.id === params.id);
  if (!linea) return { title: 'Line not found — MetroGuia' };
  return buildMetadata({
    lang: params.lang,
    title: `${linea.nombre || linea.name || 'Line ' + linea.id} — St. Louis Transit | MetroGuia`,
    description: linea.meta_description || linea.descripcion || `${linea.nombre || linea.name || 'Line ' + linea.id} in St. Louis`,
    path: `/st-louis/line/${linea.id}/`,
    keywords: ['St. Louis', linea.nombre || linea.name || linea.id, 'transit line'],
  });
}

export default function LineStLouisPageLang({ params }) {
  ...
  const estacionesLinea = estacionesStLouis.filter(e => {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: lineName, item: `https://metroguia.mx/${lang}/st-louis/line/${linea.id}` },
    ],
  };
        ...
        <div style={{ marginBottom: '60px' }}>
          ...
          <div style={{ display: 'grid', gap: '16px' }}>
            {estacionesLinea.map((estacion, idx) => (
              <Link href={`/${lang}/st-louis/station/${estacion.slug}`} key={idx} style={{ textDecoration: 'none' }}>
