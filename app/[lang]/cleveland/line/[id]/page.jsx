...
export async function generateMetadata({ params }) {
  const linea = lineasCleveland.find(l => l.id === params.id);
  if (!linea) return { title: 'Line not found — MetroGuia' };
  return buildMetadata({
    lang: params.lang,
    title: `${linea.nombre || linea.name || 'Line ' + linea.id} — Cleveland Transit | MetroGuia`,
    description: linea.meta_description || linea.descripcion || `${linea.nombre || linea.name || 'Line ' + linea.id} in Cleveland`,
    path: `/cleveland/line/${linea.id}/`,
    keywords: ['Cleveland', linea.nombre || linea.name || linea.id, 'transit line'],
  });
}

export default function LineClevelandPageLang({ params }) {
  ...
  if (!linea) {
    return (
      <main style={{ padding: '80px 24px', textAlign: 'center', backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
        ...
        <Link href={`/${lang}/cleveland`}>
          <button style={{ marginTop: '24px', padding: '12px 24px', backgroundColor: '#BA0C2F', color: '#fff', border: 'none', borderRadius: 'var(--radius)', fontSize: '1rem', fontWeight: '700', cursor: 'pointer' }}>
            ← Cleveland
          </button>
        </Link>
      </main>
  ...
  const lineColor = linea.color || '#BA0C2F';
  const lineName = linea.nombre || linea.name || `Line ${linea.id}`;
  const estacionesLinea = estacionesCleveland.filter(e => {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: lineName, item: `https://metroguia.mx/${lang}/cleveland/line/${linea.id}` },
    ],
  };
        ...
        <div style={{ marginBottom: '60px' }}>
          ...
          <div style={{ display: 'grid', gap: '16px' }}>
            {estacionesLinea.map((estacion, idx) => (
              <Link href={`/${lang}/cleveland/station/${estacion.slug}`} key={idx} style={{ textDecoration: 'none' }}>
