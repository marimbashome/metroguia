...
export function generateStaticParams() {
  ...
  LANGUAGES.filter(l => l !== defaultLang).forEach(lang => {
    estacionesCleveland.forEach(estacion => {
...
export async function generateMetadata({ params }) {
  ...
  return buildMetadata({
    lang: params.lang,
    title: `${estacion.nombre} — Cleveland Transit | MetroGuia`,
    description: estacion.meta_description || estacion.descripcion || estacion.intro || `${estacion.nombre} station in Cleveland`,
    path: `/cleveland/station/${estacion.slug}/`,
    keywords: ['Cleveland', estacion.nombre, 'transit', 'station'],
  });
}

export default function StationClevelandPageLang({ params }) {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: estacion.nombre, item: `https://metroguia.mx/${lang}/cleveland/station/${estacion.slug}` },
    ],
  };
  ...
  const borough = estacion.borough || estacion.municipio || estacion.zona || 'Ohio';
              ...
              <div style={{ marginBottom: '48px' }}>
                ...
                <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                  {lineas.map((lineId, idx) => (
                    <Link href={`/${lang}/cleveland/line/${lineId}`} key={idx} style={{ textDecoration: 'none' }}>
