...
export function generateStaticParams() {
  const defaultLang = getDefaultLangForCity('singapore') || 'en';
...
export async function generateMetadata({ params }) {
  return buildMetadata({
    lang: params.lang,
    title: 'Singapore Metro Guide | MetroGuia',
    description: 'Singapore · 6 lines · 131 stations. Complete metro guide for tourists and commuters.',
    path: '/singapore/',
    keywords: ['singapore metro', 'singapore transit', 'singapore subway'],
  });
}

export default function SingaporePageLang({ params }) {
  const lang = params.lang;
  return (
    <main style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
      <section style={{ background: 'linear-gradient(135deg, #D42E12 0%, #D42E12cc 100%)', color: '#fff', padding: '80px 24px' }}>
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
          <p style={{ fontSize: '0.75rem', color: 'rgba(255,255,255,0.85)', letterSpacing: '0.15em', textTransform: 'uppercase', fontWeight: 600, marginBottom: '0.75rem' }}>🇸🇬 Singapore Metro · Singapore</p>
          <h1 style={{ fontSize: 'clamp(1.75rem, 5vw, 2.75rem)', fontWeight: 800, marginBottom: '1rem' }}>Singapore Metro</h1>
          <p style={{ fontSize: '1.1rem', color: 'rgba(255,255,255,0.9)', marginBottom: '2rem' }}>6 lines · 131 stations</p>
          <SearchBar />
        </div>
      </section>
      <section style={{ padding: '3rem 1.5rem', maxWidth: 900, margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '1rem' }}>
          {lineasSingapore.map(linea => (
            <Link key={linea.id} href={`/${lang}/singapore/line/${linea.id}/`} style={{ textDecoration: 'none' }}>
      ...
      <section style={{ padding: '2rem 1.5rem', maxWidth: 900, margin: '0 auto', borderTop: '1px solid var(--border)' }}>
        <Link href="/singapore/" style={{ color: 'var(--text-muted)', textDecoration: 'none', fontSize: '0.9rem' }}>View in English →</Link>
      </section>
    </main>
  );
}
