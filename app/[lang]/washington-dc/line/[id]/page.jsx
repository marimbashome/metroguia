...
export async function generateMetadata({ params }) {
  ...
  return buildMetadata({
    lang: params.lang,
    title: `${linea.nombre || linea.name || 'Line ' + linea.id} — Washington D.C. Transit | MetroGuia`,
    description: linea.meta_description || linea.descripcion || `${linea.nombre || linea.name || 'Line ' + linea.id} in Washington D.C.`,
    path: `/washington-dc/line/${linea.id}/`,
    keywords: ['Washington D.C.', linea.nombre || linea.name || linea.id, 'transit line'],
  });
}

export default function LineWashingtonDcPageLang({ params }) {
  ...
  const lineColor = linea.color || '#004B87';
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: lineName, item: `https://metroguia.mx/${lang}/washington-dc/line/${linea.id}` },
    ],
  };
        ...
        <div style={{ marginBottom: '60px' }}>
          ...
          <div style={{ display: 'grid', gap: '16px' }}>
            {estacionesLinea.map((estacion, idx) => (
              <Link href={`/${lang}/washington-dc/station/${estacion.slug}`} key={idx} style={{ textDecoration: 'none' }}>
