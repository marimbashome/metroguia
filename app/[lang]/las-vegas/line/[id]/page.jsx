...
export async function generateMetadata({ params }) {
  const linea = lineasLasVegas.find(l => l.id === params.id);
  if (!linea) return { title: 'Line not found — MetroGuia' };
  return buildMetadata({
    lang: params.lang,
    title: `${linea.nombre || linea.name || 'Line ' + linea.id} — Las Vegas Transit | MetroGuia`,
    description: linea.meta_description || linea.descripcion || `${linea.nombre || linea.name || 'Line ' + linea.id} in Las Vegas`,
    path: `/las-vegas/line/${linea.id}/`,
    keywords: ['Las Vegas', linea.nombre || linea.name || linea.id, 'transit line'],
  });
}

export default function LineLasVegasPageLang({ params }) {
  ...
  if (!linea) {
    return (
      <main style={{ padding: '80px 24px', textAlign: 'center', backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
        ...
        <Link href={`/${lang}/las-vegas`}>
          <button style={{ marginTop: '24px', padding: '12px 24px', backgroundColor: '#C5B358', color: '#fff', border: 'none', borderRadius: 'var(--radius)', fontSize: '1rem', fontWeight: '700', cursor: 'pointer' }}>
            ← Las Vegas
          </button>
        </Link>
      </main>
  ...
  const lineColor = linea.color || '#C5B358';
  const lineName = linea.nombre || linea.name || `Line ${linea.id}`;
  const estacionesLinea = estacionesLasVegas.filter(e => {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: lineName, item: `https://metroguia.mx/${lang}/las-vegas/line/${linea.id}` },
    ],
  };
        ...
        <div style={{ marginBottom: '60px' }}>
          ...
          <div style={{ display: 'grid', gap: '16px' }}>
            {estacionesLinea.map((estacion, idx) => (
              <Link href={`/${lang}/las-vegas/station/${estacion.slug}`} key={idx} style={{ textDecoration: 'none' }}>
