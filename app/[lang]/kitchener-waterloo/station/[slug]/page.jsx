...
export function generateStaticParams() {
  ...
  LANGUAGES.filter(l => l !== defaultLang).forEach(lang => {
    estacionesKitchenerWaterloo.forEach(estacion => {
...
export async function generateMetadata({ params }) {
  ...
  return buildMetadata({
    lang: params.lang,
    title: `${estacion.nombre} — Kitchener-Waterloo Transit | MetroGuia`,
    description: estacion.meta_description || estacion.descripcion || estacion.intro || `${estacion.nombre} station in Kitchener-Waterloo`,
    path: `/kitchener-waterloo/station/${estacion.slug}/`,
    keywords: ['Kitchener-Waterloo', estacion.nombre, 'transit', 'station'],
  });
}

export default function StationKitchenerWaterlooPageLang({ params }) {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: estacion.nombre, item: `https://metroguia.mx/${lang}/kitchener-waterloo/station/${estacion.slug}` },
    ],
  };
              ...
              <div style={{ marginBottom: '48px' }}>
                ...
                <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                  {lineas.map((lineId, idx) => (
                    <Link href={`/${lang}/kitchener-waterloo/line/${lineId}`} key={idx} style={{ textDecoration: 'none' }}>
