...
export function generateStaticParams() {
  ...
  LANGUAGES.filter(l => l !== defaultLang).forEach(lang => {
    lineasKitchenerWaterloo.forEach(linea => {
...
export async function generateMetadata({ params }) {
  const linea = lineasKitchenerWaterloo.find(l => l.id === params.id);
  if (!linea) return { title: 'Line not found — MetroGuia' };
  return buildMetadata({
    lang: params.lang,
    title: `${linea.nombre || linea.name || 'Line ' + linea.id} — Kitchener-Waterloo Transit | MetroGuia`,
    description: linea.meta_description || linea.descripcion || `${linea.nombre || linea.name || 'Line ' + linea.id} in Kitchener-Waterloo`,
    path: `/kitchener-waterloo/line/${linea.id}/`,
    keywords: ['Kitchener-Waterloo', linea.nombre || linea.name || linea.id, 'transit line'],
  });
}

export default function LineKitchenerWaterlooPageLang({ params }) {
  ...
  if (!linea) {
    return (
      <main style={{ padding: '80px 24px', textAlign: 'center', backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
        ...
        <Link href={`/${lang}/kitchener-waterloo`}>
  ...
  const lineColor = linea.color || '#0066CC';
  const lineName = linea.nombre || linea.name || `Line ${linea.id}`;
  const estacionesLinea = estacionesKitchenerWaterloo.filter(e => {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: lineName, item: `https://metroguia.mx/${lang}/kitchener-waterloo/line/${linea.id}` },
    ],
  };
        ...
        <div style={{ marginBottom: '60px' }}>
          ...
          <div style={{ display: 'grid', gap: '16px' }}>
            {estacionesLinea.map((estacion, idx) => (
              <Link href={`/${lang}/kitchener-waterloo/station/${estacion.slug}`} key={idx} style={{ textDecoration: 'none' }}>
