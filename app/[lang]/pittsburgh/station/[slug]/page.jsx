...
export function generateStaticParams() {
  ...
  LANGUAGES.filter(l => l !== defaultLang).forEach(lang => {
    estacionesPittsburgh.forEach(estacion => {
...
export async function generateMetadata({ params }) {
  ...
  return buildMetadata({
    lang: params.lang,
    title: `${estacion.nombre} — Pittsburgh Transit | MetroGuia`,
    description: estacion.meta_description || estacion.descripcion || estacion.intro || `${estacion.nombre} station in Pittsburgh`,
    path: `/pittsburgh/station/${estacion.slug}/`,
    keywords: ['Pittsburgh', estacion.nombre, 'transit', 'station'],
  });
}

export default function StationPittsburghPageLang({ params }) {
  ...
  if (!estacion) {
    return (
      <main style={{ padding: '80px 24px', textAlign: 'center', backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
        ...
        <Link href={`/${lang}/pittsburgh`}>
          <button style={{ marginTop: '24px', padding: '12px 24px', backgroundColor: '#EEB211', color: '#fff', border: 'none', borderRadius: 'var(--radius)', fontSize: '1rem', fontWeight: '700', cursor: 'pointer' }}>
            ← Pittsburgh
          </button>
        </Link>
      </main>
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: estacion.nombre, item: `https://metroguia.mx/${lang}/pittsburgh/station/${estacion.slug}` },
    ],
  };
  ...
  const borough = estacion.borough || estacion.municipio || estacion.zona || 'Pennsylvania';
              ...
              <div style={{ marginBottom: '48px' }}>
                ...
                <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                  {lineas.map((lineId, idx) => (
                    <Link href={`/${lang}/pittsburgh/line/${lineId}`} key={idx} style={{ textDecoration: 'none' }}>
