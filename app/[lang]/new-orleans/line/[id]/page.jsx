...
export function generateStaticParams() {
  ...
  LANGUAGES.filter(l => l !== defaultLang).forEach(lang => {
    lineasNewOrleans.forEach(linea => {
...
export async function generateMetadata({ params }) {
  const linea = lineasNewOrleans.find(l => l.id === params.id);
  if (!linea) return { title: 'Line not found — MetroGuia' };
  return buildMetadata({
    lang: params.lang,
    title: `${linea.nombre || linea.name || 'Line ' + linea.id} — New Orleans Transit | MetroGuia`,
    description: linea.meta_description || linea.descripcion || `${linea.nombre || linea.name || 'Line ' + linea.id} in New Orleans`,
    path: `/new-orleans/line/${linea.id}/`,
    keywords: ['New Orleans', linea.nombre || linea.name || linea.id, 'transit line'],
  });
}

export default function LineNewOrleansPageLang({ params }) {
  ...
  const lineColor = linea.color || '#8B7355';
  const lineName = linea.nombre || linea.name || `Line ${linea.id}`;
  const estacionesLinea = estacionesNewOrleans.filter(e => {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: lineName, item: `https://metroguia.mx/${lang}/new-orleans/line/${linea.id}` },
    ],
  };
        ...
        <div style={{ marginBottom: '60px' }}>
          ...
          <div style={{ display: 'grid', gap: '16px' }}>
            {estacionesLinea.map((estacion, idx) => (
              <Link href={`/${lang}/new-orleans/station/${estacion.slug}`} key={idx} style={{ textDecoration: 'none' }}>
