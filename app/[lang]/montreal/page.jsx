...
export async function generateMetadata({ params }) {
  ...
  return buildMetadata({
    lang: params.lang,
    title: t(translations, 'montreal.title', 'Montreal Transit Guide') + ' — MetroGuia',
    description: t(translations, 'montreal.heroDesc', 'Montreal transit guide with 5 lines and 68 stations'),
    path: '/montreal/',
    keywords: ['Montreal', 'transit', 'metro', 'Quebec', 'public transport', 'transit guide'],
  });
}

export default function MontrealPageLang({ params }) {
  const lang = params.lang;
  const translations = {}; // fallback: t() uses defaults

  const estaciones = estacionesMontreal;
  const lineas = lineasMontreal;
      ...
      <section className="hero" style={{ background: 'linear-gradient(135deg, #0072CE 0%, #0072CEcc 100%)', color: '#FFFFFF', padding: '80px 24px', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        <div style={{ maxWidth: '1000px', marginLeft: 'auto', marginRight: 'auto' }}>
          <h1 style={{ fontSize: '3.5rem', fontWeight: '800', margin: '0 0 16px 0', lineHeight: '1.2', letterSpacing: '-0.02em' }}>
            {t(translations, 'montreal.title', 'Montreal Transit Guide')}
          </h1>
          <p style={{ fontSize: '1.5rem', fontWeight: '500', margin: '0 0 24px 0', maxWidth: '700px', marginLeft: 'auto', marginRight: 'auto', opacity: '0.95', lineHeight: '1.4' }}>
            {t(translations, 'montreal.subtitle', 'Quebec, Canada')}
          </p>
          <p style={{ fontSize: '1.125rem', fontWeight: '400', margin: '0', opacity: '0.9', maxWidth: '600px', marginLeft: 'auto', marginRight: 'auto' }}>
            {t(translations, 'montreal.heroDesc', 'Complete transit guide for Montreal')}
          </p>
        </div>
      </section>
      ...
      <section style={{ backgroundColor: 'var(--surface)', padding: '48px 24px', borderBottom: '1px solid var(--border)' }}>
        <div style={{ maxWidth: '1000px', marginLeft: 'auto', marginRight: 'auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '32px', textAlign: 'center' }}>
          <div>
            ...
            <p style={{ fontSize: '1rem', color: 'var(--text-muted)', margin: '0', fontWeight: '500' }}>{t(translations, 'montreal.stations', t(translations, 'city.stations', 'Stations'))}</p>
          </div>
          <div>
            <p style={{ fontSize: '2.5rem', fontWeight: '800', color: '#0072CE', margin: '0 0 8px 0' }}>{lineas.length}</p>
            <p style={{ fontSize: '1rem', color: 'var(--text-muted)', margin: '0', fontWeight: '500' }}>{t(translations, 'montreal.operativeLines', t(translations, 'city.lines', 'Lines'))}</p>
          </div>
          <div>
            <p style={{ fontSize: '2.5rem', fontWeight: '800', color: '#0072CE', margin: '0 0 8px 0' }}>~71 km</p>
            <p style={{ fontSize: '1rem', color: 'var(--text-muted)', margin: '0', fontWeight: '500' }}>{t(translations, 'montreal.kilometers', t(translations, 'city.networkLength', 'Network'))}</p>
          </div>
        </div>
      </section>
      ...
      <section style={{ backgroundColor: 'var(--bg)', padding: '60px 24px', borderBottom: '1px solid var(--border)' }}>
        <div style={{ maxWidth: '700px', marginLeft: 'auto', marginRight: 'auto' }}>
          <h2 style={{ fontSize: '1.75rem', fontWeight: '800', marginBottom: '12px', textAlign: 'center', color: 'var(--text)' }}>
            {t(translations, 'montreal.planRoute', t(translations, 'city.planRoute', 'Plan your Route'))}
          </h2>
          <p style={{ fontSize: '1rem', textAlign: 'center', marginBottom: '24px', color: 'var(--text-muted)' }}>
            {t(translations, 'montreal.planRouteDesc', t(translations, 'city.planRouteDesc', 'Find the best route between stations'))}
          </p>
          <SearchBar ciudad="montreal" lang={lang} />
        </div>
      </section>
        ...
        <div style={{ maxWidth: '1000px', marginLeft: 'auto', marginRight: 'auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '24px' }}>
            {lineas.slice(0, 12).map((linea) => (
              <Link href={`/${lang}/montreal/line/${linea.id}`} key={linea.id}>
      ...
      <section style={{ backgroundColor: 'var(--bg)', padding: '80px 24px' }}>
        <div style={{ maxWidth: '1000px', marginLeft: 'auto', marginRight: 'auto' }}>
          ...
          <div style={{ maxWidth: '1000px', margin: 'auto', padding: '2rem 1rem 0 1rem' }}>
            <AffiliateBookingCard citySlug="montreal" />
          </div>
          </div>
          <h2 style={{ fontSize: '2rem', fontWeight: '800', marginBottom: '48px', textAlign: 'center', color: 'var(--text)', letterSpacing: '-0.01em' }}>
            {t(translations, 'montreal.featuredStations', t(translations, 'city.featuredStations', 'Featured Stations'))}
          </h2>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '24px' }}>
            {estacionesDestacadas.map((estacion) => (
              <Link href={`/${lang}/montreal/station/${estacion.slug}`} key={estacion.slug}>
