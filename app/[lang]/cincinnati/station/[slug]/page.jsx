...
export function generateStaticParams() {
  ...
  LANGUAGES.filter(l => l !== defaultLang).forEach(lang => {
    estacionesCincinnati.forEach(estacion => {
...
export async function generateMetadata({ params }) {
  ...
  return buildMetadata({
    lang: params.lang,
    title: `${estacion.nombre} — Cincinnati Transit | MetroGuia`,
    description: estacion.meta_description || estacion.descripcion || estacion.intro || `${estacion.nombre} station in Cincinnati`,
    path: `/cincinnati/station/${estacion.slug}/`,
    keywords: ['Cincinnati', estacion.nombre, 'transit', 'station'],
  });
}

export default function StationCincinnatiPageLang({ params }) {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: estacion.nombre, item: `https://metroguia.mx/${lang}/cincinnati/station/${estacion.slug}` },
    ],
  };
              ...
              <div style={{ marginBottom: '48px' }}>
                ...
                <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                  {lineas.map((lineId, idx) => (
                    <Link href={`/${lang}/cincinnati/line/${lineId}`} key={idx} style={{ textDecoration: 'none' }}>
