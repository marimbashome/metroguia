...
export async function generateMetadata({ params }) {
  const linea = lineasSF.find(l => l.id === params.id);
  if (!linea) return { title: 'Line not found — MetroGuia' };
  return buildMetadata({
    lang: params.lang,
    title: `${linea.nombre || linea.name || 'Line ' + linea.id} — San Francisco Transit | MetroGuia`,
    description: linea.meta_description || linea.descripcion || `${linea.nombre || linea.name || 'Line ' + linea.id} in San Francisco`,
    path: `/san-francisco/line/${linea.id}/`,
    keywords: ['San Francisco', linea.nombre || linea.name || linea.id, 'transit line'],
  });
}

export default function LineSanFranciscoPageLang({ params }) {
  ...
  if (!linea) {
    return (
      <main style={{ padding: '80px 24px', textAlign: 'center', backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
        ...
        <Link href={`/${lang}/san-francisco`}>
          <button style={{ marginTop: '24px', padding: '12px 24px', backgroundColor: '#0099CC', color: '#fff', border: 'none', borderRadius: 'var(--radius)', fontSize: '1rem', fontWeight: '700', cursor: 'pointer' }}>
            ← San Francisco
          </button>
        </Link>
      </main>
  ...
  const lineColor = linea.color || '#0099CC';
  const lineName = linea.nombre || linea.name || `Line ${linea.id}`;
  const estacionesLinea = estacionesSF.filter(e => {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: lineName, item: `https://metroguia.mx/${lang}/san-francisco/line/${linea.id}` },
    ],
  };
        ...
        <div style={{ marginBottom: '60px' }}>
          ...
          <div style={{ display: 'grid', gap: '16px' }}>
            {estacionesLinea.map((estacion, idx) => (
              <Link href={`/${lang}/san-francisco/station/${estacion.slug}`} key={idx} style={{ textDecoration: 'none' }}>
