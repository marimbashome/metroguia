...
export async function generateMetadata({ params }) {
  ...
  return buildMetadata({
    lang: params.lang,
    title: `${linea.nombre || linea.name || 'Line ' + linea.id} — Phoenix Transit | MetroGuia`,
    description: linea.meta_description || linea.descripcion || `${linea.nombre || linea.name || 'Line ' + linea.id} in Phoenix`,
    path: `/phoenix/line/${linea.id}/`,
    keywords: ['Phoenix', linea.nombre || linea.name || linea.id, 'transit line'],
  });
}

export default function LinePhoenixPageLang({ params }) {
  ...
  const lineColor = linea.color || '#8B4513';
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: lineName, item: `https://metroguia.mx/${lang}/phoenix/line/${linea.id}` },
    ],
  };
        ...
        <div style={{ marginBottom: '60px' }}>
          ...
          <div style={{ display: 'grid', gap: '16px' }}>
            {estacionesLinea.map((estacion, idx) => (
              <Link href={`/${lang}/phoenix/station/${estacion.slug}`} key={idx} style={{ textDecoration: 'none' }}>
