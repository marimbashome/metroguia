...
export function generateStaticParams() {
  ...
  LANGUAGES.filter(l => l !== defaultLang).forEach(lang => {
    estacionesPhoenix.forEach(estacion => {
...
export async function generateMetadata({ params }) {
  ...
  return buildMetadata({
    lang: params.lang,
    title: `${estacion.nombre} — Phoenix Transit | MetroGuia`,
    description: estacion.meta_description || estacion.descripcion || estacion.intro || `${estacion.nombre} station in Phoenix`,
    path: `/phoenix/station/${estacion.slug}/`,
    keywords: ['Phoenix', estacion.nombre, 'transit', 'station'],
  });
}

export default function StationPhoenixPageLang({ params }) {
  ...
  if (!estacion) {
    return (
      <main style={{ padding: '80px 24px', textAlign: 'center', backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
        ...
        <Link href={`/${lang}/phoenix`}>
          <button style={{ marginTop: '24px', padding: '12px 24px', backgroundColor: '#8B4513', color: '#fff', border: 'none', borderRadius: 'var(--radius)', fontSize: '1rem', fontWeight: '700', cursor: 'pointer' }}>
            ← Phoenix
          </button>
        </Link>
      </main>
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: estacion.nombre, item: `https://metroguia.mx/${lang}/phoenix/station/${estacion.slug}` },
    ],
  };
              ...
              <div style={{ marginBottom: '48px' }}>
                ...
                <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                  {lineas.map((lineId, idx) => (
                    <Link href={`/${lang}/phoenix/line/${lineId}`} key={idx} style={{ textDecoration: 'none' }}>
