...
export async function generateMetadata({ params }) {
  const linea = lineasHOU.find(l => l.id === params.id);
  if (!linea) return { title: 'Line not found — MetroGuia' };
  return buildMetadata({
    lang: params.lang,
    title: `${linea.nombre || linea.name || 'Line ' + linea.id} — Houston Transit | MetroGuia`,
    description: linea.meta_description || linea.descripcion || `${linea.nombre || linea.name || 'Line ' + linea.id} in Houston`,
    path: `/houston/line/${linea.id}/`,
    keywords: ['Houston', linea.nombre || linea.name || linea.id, 'transit line'],
  });
}

export default function LineHoustonPageLang({ params }) {
  ...
  const lineColor = linea.color || '#8A2BE2';
  const lineName = linea.nombre || linea.name || `Line ${linea.id}`;
  const estacionesLinea = estacionesHOU.filter(e => {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: lineName, item: `https://metroguia.mx/${lang}/houston/line/${linea.id}` },
    ],
  };
        ...
        <div style={{ marginBottom: '60px' }}>
          ...
          <div style={{ display: 'grid', gap: '16px' }}>
            {estacionesLinea.map((estacion, idx) => (
              <Link href={`/${lang}/houston/station/${estacion.slug}`} key={idx} style={{ textDecoration: 'none' }}>
