...
export function generateStaticParams() {
  ...
  LANGUAGES.filter(l => l !== defaultLang).forEach(lang => {
    estacionesHOU.forEach(estacion => {
...
export async function generateMetadata({ params }) {
  const estacion = estacionesHOU.find(e => e.slug === params.slug);
  if (!estacion) return { title: 'Station not found — MetroGuia' };
  return buildMetadata({
    lang: params.lang,
    title: `${estacion.nombre} — Houston Transit | MetroGuia`,
    description: estacion.meta_description || estacion.descripcion || estacion.intro || `${estacion.nombre} station in Houston`,
    path: `/houston/station/${estacion.slug}/`,
    keywords: ['Houston', estacion.nombre, 'transit', 'station'],
  });
}

export default function StationHoustonPageLang({ params }) {
  ...
  if (!estacion) {
    return (
      <main style={{ padding: '80px 24px', textAlign: 'center', backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
        ...
        <Link href={`/${lang}/houston`}>
          <button style={{ marginTop: '24px', padding: '12px 24px', backgroundColor: '#8A2BE2', color: '#fff', border: 'none', borderRadius: 'var(--radius)', fontSize: '1rem', fontWeight: '700', cursor: 'pointer' }}>
            ← Houston
          </button>
        </Link>
      </main>
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: estacion.nombre, item: `https://metroguia.mx/${lang}/houston/station/${estacion.slug}` },
    ],
  };
              ...
              <div style={{ marginBottom: '48px' }}>
                ...
                <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                  {lineas.map((lineId, idx) => (
                    <Link href={`/${lang}/houston/line/${lineId}`} key={idx} style={{ textDecoration: 'none' }}>
