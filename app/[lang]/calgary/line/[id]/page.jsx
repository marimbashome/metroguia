...
export async function generateMetadata({ params }) {
  const linea = lineasCalgary.find(l => l.id === params.id);
  if (!linea) return { title: 'Line not found — MetroGuia' };
  return buildMetadata({
    lang: params.lang,
    title: `${linea.nombre || linea.name || 'Line ' + linea.id} — Calgary Transit | MetroGuia`,
    description: linea.meta_description || linea.descripcion || `${linea.nombre || linea.name || 'Line ' + linea.id} in Calgary`,
    path: `/calgary/line/${linea.id}/`,
    keywords: ['Calgary', linea.nombre || linea.name || linea.id, 'transit line'],
  });
}

export default function LineCalgaryPageLang({ params }) {
  ...
  const lineColor = linea.color || '#D93F3C';
  const lineName = linea.nombre || linea.name || `Line ${linea.id}`;
  const estacionesLinea = estacionesCalgary.filter(e => {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: lineName, item: `https://metroguia.mx/${lang}/calgary/line/${linea.id}` },
    ],
  };
        ...
        <div style={{ marginBottom: '60px' }}>
          ...
          <div style={{ display: 'grid', gap: '16px' }}>
            {estacionesLinea.map((estacion, idx) => (
              <Link href={`/${lang}/calgary/station/${estacion.slug}`} key={idx} style={{ textDecoration: 'none' }}>
