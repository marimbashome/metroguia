...
export async function generateMetadata({ params }) {
  const linea = lineasNorfolk.find(l => l.id === params.id);
  if (!linea) return { title: 'Line not found — MetroGuia' };
  return buildMetadata({
    lang: params.lang,
    title: `${linea.nombre || linea.name || 'Line ' + linea.id} — Norfolk Transit | MetroGuia`,
    description: linea.meta_description || linea.descripcion || `${linea.nombre || linea.name || 'Line ' + linea.id} in Norfolk`,
    path: `/norfolk/line/${linea.id}/`,
    keywords: ['Norfolk', linea.nombre || linea.name || linea.id, 'transit line'],
  });
}

export default function LineNorfolkPageLang({ params }) {
  ...
  const estacionesLinea = estacionesNorfolk.filter(e => {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: lineName, item: `https://metroguia.mx/${lang}/norfolk/line/${linea.id}` },
    ],
  };
        ...
        <div style={{ marginBottom: '60px' }}>
          ...
          <div style={{ display: 'grid', gap: '16px' }}>
            {estacionesLinea.map((estacion, idx) => (
              <Link href={`/${lang}/norfolk/station/${estacion.slug}`} key={idx} style={{ textDecoration: 'none' }}>
