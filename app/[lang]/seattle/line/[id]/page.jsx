...
export async function generateMetadata({ params }) {
  const linea = lineasSeattle.find(l => l.id === params.id);
  if (!linea) return { title: 'Line not found — MetroGuia' };
  return buildMetadata({
    lang: params.lang,
    title: `${linea.nombre || linea.name || 'Line ' + linea.id} — Seattle Transit | MetroGuia`,
    description: linea.meta_description || linea.descripcion || `${linea.nombre || linea.name || 'Line ' + linea.id} in Seattle`,
    path: `/seattle/line/${linea.id}/`,
    keywords: ['Seattle', linea.nombre || linea.name || linea.id, 'transit line'],
  });
}

export default function LineSeattlePageLang({ params }) {
  ...
  const lineColor = linea.color || '#00A651';
  const lineName = linea.nombre || linea.name || `Line ${linea.id}`;
  const estacionesLinea = estacionesSeattle.filter(e => {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: lineName, item: `https://metroguia.mx/${lang}/seattle/line/${linea.id}` },
    ],
  };
        ...
        <div style={{ marginBottom: '60px' }}>
          ...
          <div style={{ display: 'grid', gap: '16px' }}>
            {estacionesLinea.map((estacion, idx) => (
              <Link href={`/${lang}/seattle/station/${estacion.slug}`} key={idx} style={{ textDecoration: 'none' }}>
