...
export function generateStaticParams() {
  ...
  LANGUAGES.filter(l => l !== defaultLang).forEach(lang => {
    lineasDetalleBaltimore.forEach(linea => {
...
export async function generateMetadata({ params }) {
  ...
  return buildMetadata({
    lang: params.lang,
    title: `${linea.nombre || linea.name || 'Line ' + linea.id} — Baltimore Transit | MetroGuia`,
    description: linea.meta_description || linea.descripcion || `${linea.nombre || linea.name || 'Line ' + linea.id} in Baltimore`,
    path: `/baltimore/line/${linea.id}/`,
    keywords: ['Baltimore', linea.nombre || linea.name || linea.id, 'transit line'],
  });
}

export default function LineBaltimorePageLang({ params }) {
  ...
  const lineColor = linea.color || '#6F2C91';
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: lineName, item: `https://metroguia.mx/${lang}/baltimore/line/${linea.id}` },
    ],
  };
        ...
        <div style={{ marginBottom: '60px' }}>
          ...
          <div style={{ display: 'grid', gap: '16px' }}>
            {estacionesLinea.map((estacion, idx) => (
              <Link href={`/${lang}/baltimore/station/${estacion.slug}`} key={idx} style={{ textDecoration: 'none' }}>
