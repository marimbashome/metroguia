...
export function generateStaticParams() {
  ...
  LANGUAGES.filter(l => l !== defaultLang).forEach(lang => {
    estacionesBaltimore.forEach(estacion => {
...
export async function generateMetadata({ params }) {
  ...
  return buildMetadata({
    lang: params.lang,
    title: `${estacion.nombre} — Baltimore Transit | MetroGuia`,
    description: estacion.meta_description || estacion.descripcion || estacion.intro || `${estacion.nombre} station in Baltimore`,
    path: `/baltimore/station/${estacion.slug}/`,
    keywords: ['Baltimore', estacion.nombre, 'transit', 'station'],
  });
}

export default function StationBaltimorePageLang({ params }) {
  ...
  if (!estacion) {
    return (
      <main style={{ padding: '80px 24px', textAlign: 'center', backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
        ...
        <Link href={`/${lang}/baltimore`}>
          <button style={{ marginTop: '24px', padding: '12px 24px', backgroundColor: '#6F2C91', color: '#fff', border: 'none', borderRadius: 'var(--radius)', fontSize: '1rem', fontWeight: '700', cursor: 'pointer' }}>
            ← Baltimore
          </button>
        </Link>
      </main>
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: estacion.nombre, item: `https://metroguia.mx/${lang}/baltimore/station/${estacion.slug}` },
    ],
  };
  ...
  const borough = estacion.borough || estacion.municipio || estacion.zona || 'Maryland';
              ...
              <div style={{ marginBottom: '48px' }}>
                ...
                <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                  {lineas.map((lineId, idx) => (
                    <Link href={`/${lang}/baltimore/line/${lineId}`} key={idx} style={{ textDecoration: 'none' }}>
