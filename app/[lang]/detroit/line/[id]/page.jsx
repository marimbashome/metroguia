...
export async function generateMetadata({ params }) {
  ...
  return buildMetadata({
    lang: params.lang,
    title: `${linea.nombre || linea.name || 'Line ' + linea.id} — Detroit Transit | MetroGuia`,
    description: linea.meta_description || linea.descripcion || `${linea.nombre || linea.name || 'Line ' + linea.id} in Detroit`,
    path: `/detroit/line/${linea.id}/`,
    keywords: ['Detroit', linea.nombre || linea.name || linea.id, 'transit line'],
  });
}

export default function LineDetroitPageLang({ params }) {
  ...
  const lineColor = linea.color || '#00274C';
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: lineName, item: `https://metroguia.mx/${lang}/detroit/line/${linea.id}` },
    ],
  };
        ...
        <div style={{ marginBottom: '60px' }}>
          ...
          <div style={{ display: 'grid', gap: '16px' }}>
            {estacionesLinea.map((estacion, idx) => (
              <Link href={`/${lang}/detroit/station/${estacion.slug}`} key={idx} style={{ textDecoration: 'none' }}>
