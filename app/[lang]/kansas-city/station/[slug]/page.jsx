...
export function generateStaticParams() {
  ...
  LANGUAGES.filter(l => l !== defaultLang).forEach(lang => {
    estacionesKansasCity.forEach(estacion => {
...
export async function generateMetadata({ params }) {
  const estacion = estacionesKansasCity.find(e => e.slug === params.slug);
  if (!estacion) return { title: 'Station not found — MetroGuia' };
  return buildMetadata({
    lang: params.lang,
    title: `${estacion.nombre} — Kansas City Transit | MetroGuia`,
    description: estacion.meta_description || estacion.descripcion || estacion.intro || `${estacion.nombre} station in Kansas City`,
    path: `/kansas-city/station/${estacion.slug}/`,
    keywords: ['Kansas City', estacion.nombre, 'transit', 'station'],
  });
}

export default function StationKansasCityPageLang({ params }) {
  ...
  if (!estacion) {
    return (
      <main style={{ padding: '80px 24px', textAlign: 'center', backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
        ...
        <Link href={`/${lang}/kansas-city`}>
          <button style={{ marginTop: '24px', padding: '12px 24px', backgroundColor: '#CE1141', color: '#fff', border: 'none', borderRadius: 'var(--radius)', fontSize: '1rem', fontWeight: '700', cursor: 'pointer' }}>
            ← Kansas City
          </button>
        </Link>
      </main>
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: estacion.nombre, item: `https://metroguia.mx/${lang}/kansas-city/station/${estacion.slug}` },
    ],
  };
  ...
  const borough = estacion.borough || estacion.municipio || estacion.zona || 'Missouri';
              ...
              <div style={{ marginBottom: '48px' }}>
                ...
                <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                  {lineas.map((lineId, idx) => (
                    <Link href={`/${lang}/kansas-city/line/${lineId}`} key={idx} style={{ textDecoration: 'none' }}>
