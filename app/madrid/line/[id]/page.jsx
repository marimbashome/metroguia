...
export async function generateStaticParams() {
  return lineasMadrid.map(l => ({ id: l.id }));
}

export async function generateMetadata({ params }) {
  const l = lineasMadrid.find(l => l.id === params.id);
  if (!l) return { title: 'Line not found | MetroGuia' };
  return {
    title: l.seo_title,
    description: l.meta_description,
    alternates: { canonical: `https://metroguia.mx/madrid/line/${params.id}/` },
  };
}

export default function MadridLine({ params }) {
  const linea = lineasMadrid.find(l => l.id === params.id);
  if (!linea) {
    return (
      <main style={{ padding: '80px 24px', textAlign: 'center', backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
        ...
        <Link href="/madrid/" style={{ color: 'var(--primary)' }}>← Madrid</Link>
      </main>
    );
  }

  const estacionesData = (linea.estaciones || []).map(slug => estacionesMadrid.find(e => e.slug === slug)).filter(Boolean);

  return (
    <main style={{ backgroundColor: 'var(--bg)', color: 'var(--text)', padding: '5rem 1.5rem 3rem', maxWidth: 800, margin: '0 auto' }}>
      <IntlLineSchema line={(lineasMadrid.find(l => l.id === params.id)) || null} stations={((lineasMadrid.find(l => l.id === params.id) || {}).estaciones || []).map(s => (typeof s === 'string' ? (estacionesMadrid.find(x => x.slug === s) || { slug: s, nombre: s }) : s))} city="Madrid" citySlug="madrid" linePathPrefix="/madrid/line" stationPathPrefix="/madrid/station" country="ES" systemName="Metro de Madrid" locale="es" />
      {/* BREADCRUMB */}
      ...
      {linea.ruta_1_dia && (
        <div style={{ background: 'var(--surface)', border: `2px solid ${linea.color}`, borderRadius: 'var(--radius)', padding: '1.25rem', marginBottom: '1.5rem' }}>
          ...
          {linea.ruta_1_dia.paradas.map((parada, i) => {
            const est = estacionesMadrid.find(e => e.slug === parada.estacion);
            return (
              <div key={i} style={{ display: 'flex', gap: '0.75rem', alignItems: 'flex-start', marginBottom: '0.75rem' }}>
                ...
                <div>
                  {est ? (
                    <Link href={`/madrid/station/${parada.estacion}/`} style={{ fontWeight: 600, color: 'var(--primary)', textDecoration: 'none', fontSize: '0.9rem' }}>
                      {est.nombre}
                    </Link>
                  ) : (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(180px,1fr))', gap: '0.5rem', marginBottom: '2rem' }}>
        {estacionesData.length > 0 ? estacionesData.map(e => (
          <Link key={e.slug} href={`/madrid/station/${e.slug}/`} style={{ textDecoration: 'none', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: '0.5rem 0.75rem', fontSize: '0.85rem', fontWeight: 500, color: 'var(--text)', display: 'block' }}>
            {e.nombre}
          </Link>
