import { cablebusLineas, cablebusEstaciones } from '@/data/cdmx/cablebus'
import AdBannerLazy from '@/app/components/AdBannerLazy'

export function generateStaticParams() {
  return Object.keys(cablebusLineas).map((id) => ({ id }))
}

export function generateMetadata({ params }) {
  const linea = cablebusLineas[params.id]
  if (!linea) return { title: 'Línea no encontrada' }
  return {
    title: linea.seo_title,
    description: linea.meta_description,
    alternates: { canonical: `https://metroguia.mx/cdmx/cablebus/linea/${linea.id}/` },
  }
}

export default function CablebusLineaPage({ params }) {
  const linea = cablebusLineas[params.id]
  if (!linea) return <div style={{ padding: '4rem 2rem', textAlign: 'center' }}><h1>Línea no encontrada</h1></div>

  const estacionesLinea = cablebusEstaciones.filter(e => e.lineaId === linea.id)
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 4, name: linea.nombre, item: `https://metroguia.mx/cdmx/cablebus/linea/${linea.id}/` },
    ]
  }
