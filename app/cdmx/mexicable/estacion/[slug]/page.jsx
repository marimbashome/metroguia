...
export function generateMetadata({ params }) {
  const estacion = mexicableEstaciones.find((e) => e.slug === params.slug)
  if (!estacion) return { title: 'Estación no encontrada' }
  return {
    title: estacion.seo_title,
    description: estacion.meta_description,
    alternates: { canonical: `https://metroguia.mx/cdmx/mexicable/estacion/${estacion.slug}/` },
  }
}
...
export default function MexicableEstacionPage({ params }) {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 4, name: linea?.nombre || 'Línea', item: `https://metroguia.mx/cdmx/mexicable/linea/${estacion.lineaId}/` },
      { '@type': 'ListItem', position: 5, name: estacion.nombre, item: `https://metroguia.mx/cdmx/mexicable/estacion/${estacion.slug}/` },
    ]
  }
      ...
      <section style={{
        ...
        <div className="container">
          <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginBottom: '1rem', letterSpacing: '0.15em', textTransform: 'uppercase', fontWeight: 600 }}>
            ...
            {' '}→{' '}
            <a href={`/cdmx/mexicable/linea/${estacion.lineaId}/`} style={{ color: 'var(--primary)', textDecoration: 'none' }}>{linea?.nombre}</a>
            {' '}→ {estacion.nombre}
          </p>
