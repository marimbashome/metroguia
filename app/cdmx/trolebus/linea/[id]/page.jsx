...
export async function generateMetadata(props) {
  ...
  return {
    ...
    alternates: { canonical: `/cdmx/trolebus/linea/${linea.id}/` },
  }
}

export default async function TrolebusLineaPage(props) {
  const params = await props.params;
  const linea = sistema.lineas[params.id]
  if (!linea) {
    return (
      <section className="section">
        <div className="container-narrow" style={{ textAlign: 'center' }}>
          ...
          <p><a href="/cdmx/trolebus/">Ver todas las líneas de {sistema.nombre}</a></p>
        </div>
      </section>
    )
  }
