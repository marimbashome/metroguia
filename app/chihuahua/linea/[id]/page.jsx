...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    openGraph: {
      title: linea.seo_title,
      description: linea.meta_description,
      url: `https://metroguia.mx/chihuahua/linea/${linea.id}`,
      type: 'website',
    },
    robots: {
      index: true,
...
export default function LineaChihuahuaPage({ params }) {
  const linea = lineasChihuahua.find((l) => l.id === params.id);
  if (!linea) {
    return (
      <main style={{ padding: '40px 20px', textAlign: 'center' }}>
        ...
        <p style={{ marginTop: '20px' }}>
          <Link href="/chihuahua" style={{ color: '#D97706', textDecoration: 'underline' }}>
            Volver a Chihuahua
          </Link>
        </p>
      </main>
  ...
  const estacionesLinea = estacionesChihuahua.filter((e) => e.linea === linea.id);
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      },
      {
        ...
        position: 3,
        name: linea.nombre,
        item: `https://metroguia.mx/chihuahua/linea/${linea.id}/`
      }
    ]
  }
  ...
  return (
    <main style={{ backgroundColor: '#ffffff' }}>
      ...
      <section style={{ ...containerStyles, ...sectionStyles, paddingTop: '40px' }}>
        <h2 style={sectionTitleStyles}>Descripción de la Ruta</h2>
            ...
            return (
              <div key={idx} style={rutaParadaStyles}>
                ...
                <div style={rutaActivityStyles}>
                  <Link
                    href={`/chihuahua/estacion/${parada.estacion}`}
        ...
        <section style={{ ...containerStyles, ...sectionStyles, paddingTop: '40px' }}>
          ...
          <div style={gridStyles}>
            {linea.estaciones_turisticas.map((slugTuristico) => {
              const estacion = estacionesChihuahua.find((e) => e.slug === slugTuristico);
      ...
      <section style={{ ...containerStyles, paddingTop: '40px', paddingBottom: '40px', textAlign: 'center' }}>
        <h2 style={sectionTitleStyles}>Explora Más</h2>
        <div style={{ display: 'flex', gap: '10px', justifyContent: 'center', flexWrap: 'wrap' }}>
          <Link href="/chihuahua" style={buttonStyles}>
            Volver a Chihuahua
          </Link>
          {linea.id !== '1' && (
            <Link href="/chihuahua/linea/1" style={buttonStyles}>
              Ver Chepe Express
            </Link>
          )}
          {linea.id !== '2' && (
            <Link href="/chihuahua/linea/2" style={buttonStyles}>
