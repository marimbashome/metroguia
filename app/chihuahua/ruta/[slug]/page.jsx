import { chihuahuaStations, STATION_DISPLAY_NAMES } from '@/data/rutas-engine'
...
export function generateStaticParams() {
  const hubs = ['los-mochis', 'creel', 'divisadero', 'el-fuerte']
  const params = []
  for (const hub of hubs) {
    for (const station of chihuahuaStations) {
      if (station !== hub) {
...
function isValidRouteSlug(slug) {
  ...
  const stationSet = new Set(chihuahuaStations)
...
export default function RutaChihuahuaPage({ params }) {
  return <RutaClient slug={params.slug} ciudad="chihuahua" />
}
