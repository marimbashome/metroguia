...
export async function generateStaticParams() {
  return lineasBuenosAires.map(l => ({ id: l.id }));
}

export async function generateMetadata({ params }) {
  const linea = lineasBuenosAires.find(l => l.id === params.id);
  if (!linea) return { title: 'Línea no encontrada' };
  return {
    title: linea.seo_title,
    description: linea.meta_description,
    openGraph: { title: linea.seo_title, url: `https://metroguia.mx/buenos-aires/line/${linea.id}/` },
    alternates: { canonical: `https://metroguia.mx/buenos-aires/line/${linea.id}/` },
  };
}

export default function LineBuenosAires({ params }) {
  const linea = lineasBuenosAires.find(l => l.id === params.id);
  ...
  if (!linea) {
    return (
      <main style={{ padding: '80px 24px', textAlign: 'center', backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
        ...
        <Link href="/buenos-aires/"><button style={{ marginTop: 24, padding: '12px 24px', background: '#1E5FA6', color: '#fff', border: 'none', borderRadius: 'var(--radius)', fontWeight: 700, cursor: 'pointer' }}>Volver a Buenos Aires</button></Link>
      </main>
    );
  }

  const estacionesLinea = estacionesBuenosAires.filter(e => {
  ...
  return (
    <main style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
      <IntlLineSchema line={(lineasBuenosAires.find(l => l.id === params.id)) || null} stations={((lineasBuenosAires.find(l => l.id === params.id) || {}).estaciones || []).map(s => (typeof s === 'string' ? (estacionesBuenosAires.find(x => x.slug === s) || { slug: s, nombre: s }) : s))} city="Buenos Aires" citySlug="buenos-aires" linePathPrefix="/buenos-aires/line" stationPathPrefix="/buenos-aires/station" country="AR" systemName="Subte" locale="es" />
      {/* HERO */}
      ...
      <section style={{ maxWidth: '1200px', margin: '0 auto', padding: '60px 24px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: 16, marginBottom: 48 }}>
          {[
            ...
            { label: 'Tarifa', val: 'ARS $270' },
            { label: 'Horario', val: '05:30 – 22:30' },
          ].map(c => (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {linea.ruta_1_dia.paradas.map((p, i) => (
                <div key={i} style={{ display: 'flex', gap: 16, alignItems: 'flex-start' }}>
                  ...
                  <div>
                    <Link href={`/buenos-aires/station/${p.estacion}/`} style={{ color: linea.color, fontWeight: 700, textDecoration: 'none' }}>{p.estacion.replace('baires-','').replace(/-/g,' ').replace(/\b\w/g,c=>c.toUpperCase())}</Link>
