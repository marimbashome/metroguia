...
const LINEA_COLORS = {
  A: '#7AC5E5', B: '#EF3A3C', C: '#1E5FA6', D: '#159748', E: '#9B297D', H: '#F9BC15',
};

export async function generateStaticParams() {
  return estacionesBuenosAires.map(e => ({ slug: e.slug }));
}

export async function generateMetadata({ params }) {
  const estacion = estacionesBuenosAires.find(e => e.slug === params.slug);
  if (!estacion) return { title: 'Estación no encontrada' };
  return {
    title: estacion.seo_title,
    description: estacion.meta_description,
    openGraph: { title: estacion.seo_title, url: `https://metroguia.mx/buenos-aires/station/${estacion.slug}/` },
    alternates: { canonical: `https://metroguia.mx/buenos-aires/station/${estacion.slug}/` },
  };
}

export default function StationBuenosAires({ params }) {
  const estacion = estacionesBuenosAires.find(e => e.slug === params.slug);
  ...
  const lineasEst = lineaArr.map(id => lineasBuenosAires.find(l => l.id === id)).filter(Boolean);
  const colorPrincipal = LINEA_COLORS[lineaArr[0]] || '#1E5FA6';

  const breadcrumb = {
    '@context': 'https://schema.org', '@type': 'BreadcrumbList',
    itemListElement: [
      { '@type': 'ListItem', position: 1, name: 'MetroGuia', item: 'https://metroguia.mx' },
      { '@type': 'ListItem', position: 2, name: 'Buenos Aires', item: 'https://metroguia.mx/buenos-aires/' },
      { '@type': 'ListItem', position: 3, name: estacion.nombre, item: `https://metroguia.mx/buenos-aires/station/${estacion.slug}/` },
    ],
  };
