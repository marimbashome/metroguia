...
export function buildEstacionesMap(estaciones) {
  ...
  estaciones.forEach((est) => {
    ...
    // data/estaciones.js guarda un array `lineas` (estaciones de transbordo
    // tienen varias); se acepta también `linea` singular por compatibilidad
    // con otros datasets que puedan usar ese shape.
    const lineasEst = Array.isArray(est.lineas) ? est.lineas : est.linea ? [est.linea] : [];
    lineasEst.forEach((l) => {
      if (!map[est.slug].lineas.includes(l)) {
        map[est.slug].lineas.push(l);
      }
    });
  });
  return map;
