 ...
 * Search index is lazy-loaded on first search to reduce initial bundle size.
 */

// searchIndex will be lazily loaded in searchEstaciones()
 ...
 * @returns {Promise<Object>} The searchIndex object
 */
async function loadSearchIndex() {
  ...
  try {
    const searchMod = await import('@/data/search-index');
    searchIndexCache = searchMod.searchIndex;
    return searchIndexCache;
  } catch (error) {
 ...
 * @returns {Promise<Array>} Array of matching stations with scores, including 'ciudad' and 'url' fields
 */
export async function searchEstaciones(query, options = {}) {
