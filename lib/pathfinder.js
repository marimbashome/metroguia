...
export function findRoute(origen, destino) {
  ...
  if (origenCiudad && destinoCiudad && origenCiudad !== destinoCiudad) {
    return {
      ...
      costo: 0,
      lineas_usadas: [],
      alertas: [`No hay servicio directo entre ${origenCiudad} y ${destinoCiudad}`]
    };
  }
  ...
  while (queue.length > 0) {
    ...
    if (!estacionData || !estacionData.conexiones) continue;

    for (const conexion of estacionData.conexiones) {
      const { estacion: proxima, tiempo: tiempoConexion, linea } = conexion;
...
export function findAlternativeRoute(origen, destino) {
  ...
  if (!estacionOrigen || !estacionOrigen.conexiones) {
    return {
      encontrada: false,
      pasos: [],
      tiempo_total: 0,
  ...
  for (const conexion of estacionOrigen.conexiones) {
    const proximaEstacion = conexion.estacion;
