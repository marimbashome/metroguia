...
// Sin DSN, Sentry.init() no fallaba: se volvia un no-op silencioso que
// instalaba los manejadores globales y no reportaba un solo evento. Con el
// guard la ausencia es explicita, igual que en sentry.server.config y
// sentry.edge.config, que ya lo traian.
if (dsn) {
  Sentry.init({
    dsn,
  ...
  console.warn('[sentry] falta NEXT_PUBLIC_SENTRY_DSN; no se reportan errores')
}
