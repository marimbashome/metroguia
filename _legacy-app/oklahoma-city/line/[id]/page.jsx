...
export async function generateMetadata({ params }) {
  ...
  return {
    title: `The Rapid Transit Network ${linea.colorNombre} Line — Oklahoma City | MetroGuia`,
    description: linea.descripcion.substring(0, 160),
    openGraph: {
      title: `The Rapid Transit Network ${linea.colorNombre} Line`,
      description: linea.descripcion.substring(0, 160),
      url: `https://metroguia.mx/oklahoma-city/line/${linea.id}`,
    },
  };
}
...
export default function LineOklahomaCityPage({ params }) {
  ...
  return (
    <main style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
      <IntlLineSchema line={(lineasOklahomaCity.find(l => l.id === params.id)) || null} stations={((lineasOklahomaCity.find(l => l.id === params.id) || {}).estaciones || []).map(s => (typeof s === 'string' ? (estacionesOklahomaCity.find(x => x.slug === s) || { slug: s, nombre: s }) : s))} city="Oklahoma City" citySlug="oklahoma-city" linePathPrefix="/oklahoma-city/line" stationPathPrefix="/oklahoma-city/station" country="US" systemName="OKC Streetcar" locale="en" />
      {/* HERO */}
