...
export async function generateMetadata({ params }) {
  ...
  const seoTitle = estacion.seo_title || `${estacion.nombre} Station — Morelia Transit | MetroGuia`;
  const metaDesc = estacion.meta_description || estacion.descripcion_turistica || `Guide to ${estacion.nombre} station in Morelia`;
  return {
    title: seoTitle,
    ...
    openGraph: {
      title: seoTitle,
      description: metaDesc,
      url: `https://metroguia.mx/morelia/station/${estacion.slug}`,
      type: 'website',
    },
    robots: { index: true, follow: true },
    alternates: {
      canonical: `https://metroguia.mx/morelia/station/${estacion.slug}`,
      languages: {
        'es': `https://metroguia.mx/morelia/estacion/${estacion.slug}`,
        'en': `https://metroguia.mx/morelia/station/${estacion.slug}`,
      },
    },
  };
}

export default function StationMoreliaPage({ params }) {
  const estacion = estacionesMorelia.find((e) => e.slug === params.slug);
  if (!estacion) {
    return (
  ...
  const linea = lineasMorelia.find((l) => l.id === lineaId);
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: `Line ${lineaId}`, item: `https://metroguia.mx/morelia/line/${lineaId}/` },
      { '@type': 'ListItem', position: 4, name: estacion.h1 || estacion.nombre, item: `https://metroguia.mx/morelia/station/${estacion.slug}/` }
    ]
  };
