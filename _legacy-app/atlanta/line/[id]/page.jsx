...
export async function generateStaticParams() {
  return lineasAtlanta.map((linea) => ({
    id: linea.id,
  }));
}

export async function generateMetadata({ params }) {
  const linea = lineasAtlanta.find((l) => l.id === params.id);
  if (!linea) {
    return {
  ...
  return {
    ...
    openGraph: {
      title: linea.seo_title,
      description: linea.meta_description,
      url: `https://metroguia.mx/atlanta/line/${linea.id}`,
    },
  };
}

export default function LineAtlanta({ params }) {
  const linea = lineasAtlanta.find((l) => l.id === params.id);
  ...
  const estacionesLinea = estacionesAtlanta.filter((e) => Array.isArray(e.linea) ? e.linea.includes(linea.id) : e.linea === linea.id);

  return (
    <main style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
      <LineSchema linea={linea} ciudad="atlanta" sistema="MARTA" />

      {/* HERO */}
      <section style={{ background: `linear-gradient(135deg, ${linea.color} 0%, ${linea.color}cc 100%)`, color: linea.id === 'gold' ? '#000' : '#fff', padding: '60px 24px' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          <div style={{ marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '12px' }}>
            <span style={{ width: '60px', height: '60px', backgroundColor: 'rgba(255,255,255,0.3)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '2rem', fontWeight: '800', color: linea.id === 'gold' ? '#000' : '#fff' }}>
          ...
          <p style={{ fontSize: '1rem', margin: '0', opacity: '0.9', color: linea.id === 'gold' ? '#000' : '#fff' }}>
      ...
      <section style={{ maxWidth: '1200px', margin: '0 auto', padding: '60px 24px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px', marginBottom: '48px' }}>
          ...
          <div className="card" style={{ padding: '20px' }}>
            ...
            <p style={{ fontSize: '1.125rem', fontWeight: '700', margin: '0' }}>24/7</p>
          </div>
          <div className="card" style={{ padding: '20px' }}>
              ...
              <Link href={`/atlanta/station/${estacion.slug}`} key={estacion.slug}>
                <div style={{ padding: '20px', backgroundColor: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', cursor: 'pointer', transition: 'all 0.2s ease' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '12px' }}>
                    <span style={{ width: '36px', height: '36px', backgroundColor: linea.color, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: linea.id === 'gold' ? '#000' : '#fff', fontSize: '0.9rem', fontWeight: '700', flexShrink: 0 }}>
                      {idx + 1}
                    </span>
      ...
      <section style={{ backgroundColor: 'var(--surface)', padding: '60px 24px', textAlign: 'center' }}>
        <h2 style={{ fontSize: '1.5rem', fontWeight: '700', margin: '0 0 16px 0', color: 'var(--text)' }}>
          Explore Atlanta MARTA
        </h2>
        <p style={{ fontSize: '1rem', color: 'var(--text-muted)', margin: '0 0 24px 0', maxWidth: '600px', marginLeft: 'auto', marginRight: 'auto' }}>
          Discover more lines, stations, and attractions throughout Atlanta.
        </p>
        <Link href="/atlanta">
          <button style={{ padding: '14px 32px', backgroundColor: linea.color, color: linea.id === 'gold' ? '#000' : '#fff', border: 'none', borderRadius: 'var(--radius)', fontSize: '1rem', fontWeight: '700', cursor: 'pointer' }}>
            Back to Atlanta
          </button>
        </Link>
      </section>
