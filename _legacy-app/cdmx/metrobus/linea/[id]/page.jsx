import { metrobusLineas, metrobusEstaciones } from '@/data/cdmx/metrobus'
import AdBannerLazy from '@/app/components/AdBannerLazy'

export function generateStaticParams() {
  return Object.keys(metrobusLineas).map((id) => ({ id }))
}

export function generateMetadata({ params }) {
  const linea = metrobusLineas[params.id]
  if (!linea) return { title: 'Línea no encontrada' }
  return {
    title: linea.seo_title,
    description: linea.meta_description,
    alternates: { canonical: `https://metroguia.mx/cdmx/metrobus/linea/${linea.id}/` },
  }
}
...
export default function MetrobusLineaPage({ params }) {
  ...
  const estacionesLinea = metrobusEstaciones.filter(e => e.lineaId === linea.id)
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 4, name: linea.nombre, item: `https://metroguia.mx/cdmx/metrobus/linea/${linea.id}/` },
    ]
  }
      ...
      <section style={{ padding: '0 1rem 4rem' }}>
        <div className="container">
          ...
          <div style={{
            ...
            <p style={{ color: 'var(--text-muted)' }}><strong style={{ color: 'var(--text)' }}>Zona:</strong> {linea.zona}</p>
          </div>
        </div>
      </section>
    </div>
