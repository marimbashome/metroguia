...
export function generateStaticParams() {
  return metrobusEstaciones.map((e) => ({ slug: e.slug }))
}

export function generateMetadata({ params }) {
  const estacion = metrobusEstaciones.find((e) => e.slug === params.slug)
  if (!estacion) return { title: 'Estación no encontrada' }
  return {
    title: estacion.seo_title,
    description: estacion.meta_description,
    alternates: { canonical: `https://metroguia.mx/cdmx/metrobus/estacion/${estacion.slug}/` },
  }
}

export default function MetrobusEstacionPage({ params }) {
  const estacion = metrobusEstaciones.find((e) => e.slug === params.slug)
  if (!estacion) return <div style={{ padding: '4rem 2rem', textAlign: 'center' }}><h1>Estación no encontrada</h1></div>

  const linea = metrobusLineas[estacion.lineaId]
  const lineaColor = linea?.color || '#E30613'
  const stationLdPayloads = buildAllStationLdPayloads(estacion, {
    cityPath: 'cdmx/metrobus/',
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 4, name: linea?.nombre || 'Línea', item: `https://metroguia.mx/cdmx/metrobus/linea/${estacion.lineaId}/` },
      { '@type': 'ListItem', position: 5, name: estacion.nombre, item: `https://metroguia.mx/cdmx/metrobus/estacion/${estacion.slug}/` },
    ]
  }
      ...
      <section style={{
        ...
        <div className="container">
          <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginBottom: '1rem', letterSpacing: '0.15em', textTransform: 'uppercase', fontWeight: 600 }}>
            ...
            {' '}→{' '}
            <a href="/cdmx/metrobus/" style={{ color: 'var(--primary)', textDecoration: 'none' }}>Metrobús</a>
            {' '}→{' '}
            <a href={`/cdmx/metrobus/linea/${estacion.lineaId}/`} style={{ color: 'var(--primary)', textDecoration: 'none' }}>{linea?.nombre}</a>
            {' '}→ {estacion.nombre}
          </p>
      ...
      <section style={{ padding: '3rem 1rem' }}>
        <div className="container" style={{ maxWidth: '800px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem', marginBottom: '2rem' }}>
            ...
            <div style={{ padding: '1rem', borderRadius: 'var(--radius)', border: '1px solid var(--border)', backgroundColor: 'var(--surface)' }}>
              <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '0.25rem' }}>Alcaldía</div>
              <div style={{ fontWeight: 600, color: 'var(--text)' }}>{estacion.alcaldia}</div>
            </div>
