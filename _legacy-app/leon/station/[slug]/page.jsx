...
export async function generateStaticParams() {
  return estacionesLeon.map((estacion) => ({ slug: estacion.slug }));
}

export async function generateMetadata({ params }) {
  const estacion = estacionesLeon.find((e) => e.slug === params.slug);
  if (!estacion) return { title: 'Station not found' };
  const seoTitle = estacion.seo_title || `${estacion.nombre} Station — León Transit | MetroGuia`;
  const metaDesc = estacion.meta_description || estacion.descripcion_turistica || `Guide to ${estacion.nombre} station in León`;
  return {
    title: seoTitle,
    ...
    openGraph: {
      title: seoTitle,
      description: metaDesc,
      url: `https://metroguia.mx/leon/station/${estacion.slug}`,
      type: 'website',
    },
    robots: { index: true, follow: true },
    alternates: {
      canonical: `https://metroguia.mx/leon/station/${estacion.slug}`,
      languages: {
...
export default function StationLeonPage({ params }) {
  const estacion = estacionesLeon.find((e) => e.slug === params.slug);
  if (!estacion) {
    return (
      <main style={{ padding: '40px 20px', textAlign: 'center'}} >
        ...
        <p style={{ marginTop: '20px'}} >
          <Link href="/leon" style={{ color: '#059669', textDecoration: 'underline'}} >Back to León</Link>
        </p>
      </main>
    );
  }
  ...
  const linea = lineasLeon.find((l) => l.id === lineaId);
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'León', item: 'https://metroguia.mx/leon/' },
      { '@type': 'ListItem', position: 3, name: `Line ${lineaId}`, item: `https://metroguia.mx/leon/line/${lineaId}/` },
      { '@type': 'ListItem', position: 4, name: estacion.h1 || estacion.nombre, item: `https://metroguia.mx/leon/station/${estacion.slug}/` }
    ]
  };
    ...
    <main style={{ backgroundColor: '#ffffff'}} >
      ...
      <section style={heroStyles}>
        <div style={containerStyles}>
          <p style={{ fontSize: '14px', marginBottom: '10px', opacity: '0.9'}} >
            Line {lineaId} • {estacion.municipio || 'León'}
          </p>
      ...
      <section style={{ ...containerStyles, paddingTop: '40px', paddingBottom: '40px', textAlign: 'center'}} >
        <div style={{ display: 'flex', gap: '10px', justifyContent: 'center', flexWrap: 'wrap'}} >
          <Link href="/leon" style={buttonStyles}>Back to León</Link>
          {linea && <Link href={`/leon/line/${linea.id}`} style={buttonStyles}>View Line {linea.id}</Link>}
        </div>
      </section>
    </main>
  );
