...
const LINE_COLORS = {"skyline":"#0077C0"};

export async function generateStaticParams() {
  return estacionesHonolulu.map((estacion) => ({
    slug: estacion.slug,
  }));
}
...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    openGraph: {
      title: estacion.seo_title,
      description: estacion.meta_description,
      url: `https://metroguia.mx/honolulu/station/${estacion.slug}`,
    },
  };
}

export default function StationHonoluluPage({ params }) {
  const estacion = estacionesHonolulu.find((e) => e.slug === params.slug);
  ...
  if (!estacion) {
    return (
      <main style={{ padding: '80px 24px', textAlign: 'center', backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
        ...
        <Link href="/honolulu">
          <button style={{ marginTop: '24px', padding: '12px 24px', backgroundColor: '#0077C0', color: '#fff', border: 'none', borderRadius: 'var(--radius)', fontSize: '1rem', fontWeight: '700', cursor: 'pointer' }}>
            Back to Honolulu
          </button>
        </Link>
      </main>
  ...
  const colorLinea = LINE_COLORS[lineaArray[0]] || '#0077C0';
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: `Line ${lineaId}`, item: `https://metroguia.mx/honolulu/line/${lineaId}/` },
      { '@type': 'ListItem', position: 4, name: estacion.nombre, item: `https://metroguia.mx/honolulu/station/${estacion.slug}/` },
    ]
  };
  ...
  const stationSchema = {
    ...
    address: { '@type': 'PostalAddress', addressLocality: 'Honolulu', addressRegion: 'Hawaii', addressCountry: 'US' },
  };
      ...
      <section style={{ background: `linear-gradient(135deg, ${colorLinea} 0%, ${colorLinea}cc 100%)`, color: '#fff', padding: '60px 24px' }}>
        <div style={{ maxWidth: '1000px', margin: '0 auto' }}>
          <div style={{ marginBottom: '8px' }}>
            <Link href="/honolulu" style={{ color: 'rgba(255,255,255,0.8)', textDecoration: 'none', fontSize: '0.9rem' }}>
              ← Honolulu
            </Link>
          </div>
          <h1 style={{ fontSize: '2.5rem', fontWeight: '800', margin: '0 0 8px 0' }}>{estacion.nombre}</h1>
          <p style={{ fontSize: '1.125rem', opacity: 0.9, margin: 0 }}>
            {lineaArray.map(l => `${l.charAt(0).toUpperCase() + l.slice(1)} Line`).join(' · ')} — Skyline
          </p>
        </div>
      </section>
      ...
      <section style={{ padding: '60px 24px' }}>
        <div style={{ maxWidth: '800px', margin: '0 auto' }}>
          ...
          <div style={{ marginTop: '48px', textAlign: 'center' }}>
            <Link href={`/honolulu/line/${lineaId}`}>
