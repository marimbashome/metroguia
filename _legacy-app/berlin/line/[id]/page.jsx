...
export async function generateStaticParams() {
  return lineasBerlin.map(l => ({ id: l.id }));
}

export async function generateMetadata({ params }) {
  const l = lineasBerlin.find(l => l.id === params.id);
  if (!l) return { title: 'Line not found | MetroGuia' };
  return {
    title: l.seo_title,
    description: l.meta_description,
    alternates: { canonical: `https://metroguia.mx/berlin/line/${params.id}/` },
  };
}

export default function BerlinLine({ params }) {
  const linea = lineasBerlin.find(l => l.id === params.id);
  if (!linea) {
    return (
      <main style={{ padding: '80px 24px', textAlign: 'center', backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
        ...
        <Link href="/berlin/" style={{ color: 'var(--primary)' }}>← Berlin</Link>
      </main>
    );
  }

  const estacionesData = (linea.estaciones || []).map(slug => estacionesBerlin.find(e => e.slug === slug)).filter(Boolean);

  return (
    <main style={{ backgroundColor: 'var(--bg)', color: 'var(--text)', padding: '5rem 1.5rem 3rem', maxWidth: 800, margin: '0 auto' }}>
      <IntlLineSchema line={(lineasBerlin.find(l => l.id === params.id)) || null} stations={((lineasBerlin.find(l => l.id === params.id) || {}).estaciones || []).map(s => (typeof s === 'string' ? (estacionesBerlin.find(x => x.slug === s) || { slug: s, nombre: s }) : s))} city="Berlín" citySlug="berlin" linePathPrefix="/berlin/line" stationPathPrefix="/berlin/station" country="DE" systemName="BVG U-Bahn" locale="es" />
      {/* BREADCRUMB */}
      ...
      {linea.ruta_1_dia && (
        <div style={{ background: 'var(--surface)', border: `2px solid ${linea.color}`, borderRadius: 'var(--radius)', padding: '1.25rem', marginBottom: '1.5rem' }}>
          ...
          {linea.ruta_1_dia.paradas.map((parada, i) => {
            const est = estacionesBerlin.find(e => e.slug === parada.estacion);
            return (
              <div key={i} style={{ display: 'flex', gap: '0.75rem', alignItems: 'flex-start', marginBottom: '0.75rem' }}>
                ...
                <div>
                  {est ? (
                    <Link href={`/berlin/station/${parada.estacion}/`} style={{ fontWeight: 600, color: 'var(--primary)', textDecoration: 'none', fontSize: '0.9rem' }}>
                      {est.nombre}
                    </Link>
                  ) : (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(180px,1fr))', gap: '0.5rem', marginBottom: '2rem' }}>
        {estacionesData.length > 0 ? estacionesData.map(e => (
          <Link key={e.slug} href={`/berlin/station/${e.slug}/`} style={{ textDecoration: 'none', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: '0.5rem 0.75rem', fontSize: '0.85rem', fontWeight: 500, color: 'var(--text)', display: 'block' }}>
            {e.nombre}
          </Link>
          ...
          <div key={i} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: '0.5rem 0.75rem', fontSize: '0.82rem', color: 'var(--text-muted)' }}>
            {slug.replace('berlin-', '').replace(/-/g, ' ')}
          </div>
        ))}
      </div>

      <AdBannerLazy />
      <Link href="/berlin/" style={{ color: 'var(--primary)', fontWeight: 600, textDecoration: 'none' }}>← Volver a Berlin</Link>
    </main>
  );
}
