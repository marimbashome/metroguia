...
export async function generateMetadata({ params }) {
  const e = estacionesBerlin.find(e => e.slug === params.slug);
  ...
  return {
    ...
    openGraph: {
      title: e.seo_title,
      description: e.meta_description,
      url: `https://metroguia.mx/berlin/station/${e.slug}/`,
    },
    alternates: { canonical: `https://metroguia.mx/berlin/station/${e.slug}/` },
  };
}

export default function BerlinStation({ params }) {
  const e = estacionesBerlin.find(s => s.slug === params.slug);
  if (!e) {
    return (
      <main style={{ padding: '80px 24px', textAlign: 'center', backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
        ...
        <Link href="/berlin/" style={{ color: 'var(--primary)' }}>← Back to Berlin</Link>
      </main>
    );
  }
  ...
  return (
    <main style={{ backgroundColor: 'var(--bg)', color: 'var(--text)', padding: '5rem 1.5rem 3rem', maxWidth: 800, margin: '0 auto' }}>
      ...
      <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginBottom: '0.5rem' }}>
        ...
        {' '} › {' '}
        <Link href="/berlin/" style={{ color: 'var(--text-muted)', textDecoration: 'none' }}>Estaciones</Link>
        {' '} › {e.nombre}
      </p>
      ...
      <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginBottom: '1.5rem' }}>
        {lineaArr.map(l => {
          const linea = lineasBerlin.find(x => x.id === l);
          return (
            <Link key={l} href={`/berlin/line/${l}/`} style={{ background: linea?.color || 'var(--primary)', color: '#fff', padding: '0.3rem 0.8rem', borderRadius: 999, fontWeight: 700, fontSize: '0.85rem', textDecoration: 'none' }}>
              L{l}
            </Link>
          );
        })}
