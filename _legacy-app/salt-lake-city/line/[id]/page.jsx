...
export async function generateMetadata({ params }) {
  ...
  return {
    title: linea.seo_title || `${linea.id} Line — UTA TRAX | MetroGuia`,
    description: linea.meta_description || linea.descripcion,
    openGraph: {
      title: linea.seo_title || `${linea.id} Line — UTA TRAX`,
      description: linea.meta_description || linea.descripcion,
      url: `https://metroguia.mx/salt-lake-city/line/${linea.id}`,
    },
  };
}

export default function LineSaltLakeCityPage({ params }) {
  const linea = lineasSaltLakeCity.find((l) => l.id === params.id);
  ...
  return (
    <main style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
      <IntlLineSchema line={(lineasSaltLakeCity.find(l => l.id === params.id)) || null} stations={((lineasSaltLakeCity.find(l => l.id === params.id) || {}).estaciones || []).map(s => (typeof s === 'string' ? (estacionesSaltLakeCity.find(x => x.slug === s) || { slug: s, nombre: s }) : s))} city="Salt Lake City" citySlug="salt-lake-city" linePathPrefix="/salt-lake-city/line" stationPathPrefix="/salt-lake-city/station" country="US" systemName="UTA TRAX" locale="en" />
      {/* HERO */}
      <section style={{ background: `linear-gradient(135deg, ${linea.color} 0%, ${linea.color}cc 100%)`, color: '#fff', padding: '60px 24px' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          ...
          <h1 style={{ fontSize: 'clamp(2rem, 5vw, 3rem)', fontWeight: '800', margin: '0 0 16px 0', lineHeight: '1.2' }}>
            {linea.id.charAt(0).toUpperCase() + linea.id.slice(1)} Line — {linea.colorNombre}
          </h1>
      ...
      <section style={{ maxWidth: '1200px', margin: '0 auto', padding: '60px 24px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px', marginBottom: '48px' }}>
          ...
          <div style={{ padding: '20px', backgroundColor: 'var(--surface)', borderRadius: 'var(--radius)', border: '1px solid var(--border)' }}>
            ...
            <p style={{ fontSize: '1.125rem', fontWeight: '700', margin: '0' }}>15–30 min</p>
          </div>
          <div style={{ padding: '20px', backgroundColor: 'var(--surface)', borderRadius: 'var(--radius)', border: '1px solid var(--border)' }}>
              ...
              {linea.ruta_1_dia.paradas.map((parada, idx) => {
                ...
                return (
                  <div key={idx} style={{ display: 'grid', gridTemplateColumns: '60px 1fr', gap: '24px', alignItems: 'start' }}>
                    ...
                    <div>
                      <Link href={`/salt-lake-city/station/${parada.estacion}`}>
        ...
        <div style={{ marginBottom: '60px', marginTop: '40px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '16px' }}>
            {estacionesLinea.map((estacion, idx) => (
              <Link href={`/salt-lake-city/station/${estacion.slug}`} key={idx} style={{ textDecoration: 'none' }}>
                <div style={{
                  padding: '20px',
