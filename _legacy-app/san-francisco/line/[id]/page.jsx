...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    openGraph: {
      title: linea.seo_title,
      description: linea.meta_description,
      url: `https://metroguia.mx/san-francisco/line/${linea.id}`,
    },
  };
}

export default function LineSanFranciscoPage({ params }) {
  const linea = lineasSF.find((l) => l.id === params.id);
  ...
  const estacionesLinea = estacionesSF.filter((e) => e.linea === linea.id);

  return (
    <main style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
      <IntlLineSchema line={(lineasSF.find(l => l.id === params.id)) || null} stations={((lineasSF.find(l => l.id === params.id) || {}).estaciones || []).map(s => (typeof s === 'string' ? (estacionesSF.find(x => x.slug === s) || { slug: s, nombre: s }) : s))} city="San Francisco" citySlug="san-francisco" linePathPrefix="/san-francisco/line" stationPathPrefix="/san-francisco/station" country="US" systemName="BART" locale="en" />
      {/* HERO */}
      ...
      <section style={{ maxWidth: '1200px', margin: '0 auto', padding: '60px 24px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px', marginBottom: '48px' }}>
          ...
          <div style={{ padding: '20px', backgroundColor: 'var(--surface)', borderRadius: 'var(--radius)', border: '1px solid var(--border)' }}>
            ...
            <p style={{ fontSize: '1.125rem', fontWeight: '700', margin: '0' }}>Varies by system</p>
          </div>
          <div style={{ padding: '20px', backgroundColor: 'var(--surface)', borderRadius: 'var(--radius)', border: '1px solid var(--border)' }}>
            ...
            <p style={{ fontSize: '1.125rem', fontWeight: '700', margin: '0', color: 'var(--success)' }}>Varies</p>
          </div>
        </div>
              ...
              {linea.ruta_1_dia.paradas.map((parada, idx) => {
                ...
                return (
                  <div key={idx} style={{ display: 'grid', gridTemplateColumns: '60px 1fr', gap: '24px', alignItems: 'start' }}>
                    ...
                    <div>
                      <Link href={`/san-francisco/station/${parada.estacion}`}>
