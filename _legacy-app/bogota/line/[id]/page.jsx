...
export default function LineBogota({ params }) {
  ...
  return (
    <main style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
      <IntlLineSchema line={(lineasBogota.find(l => l.id === params.id)) || null} stations={((lineasBogota.find(l => l.id === params.id) || {}).estaciones || []).map(s => (typeof s === 'string' ? (estacionesBogota.find(x => x.slug === s) || { slug: s, nombre: s }) : s))} city="Bogotá" citySlug="bogota" linePathPrefix="/bogota/line" stationPathPrefix="/bogota/station" country="CO" systemName="TransMilenio" locale="es" />
      {/* HERO */}
