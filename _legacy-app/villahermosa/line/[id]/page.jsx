...
export async function generateMetadata({ params }) {
  ...
  const seoTitle = linea.seo_title || `${linea.nombre || linea.h1} — Villahermosa Transit | MetroGuia`;
  ...
  return {
    ...
    openGraph: {
      title: seoTitle,
      description: metaDesc,
      url: `https://metroguia.mx/villahermosa/line/${linea.id}`,
      type: 'website',
    },
    robots: { index: true, follow: true },
    alternates: {
      canonical: `https://metroguia.mx/villahermosa/line/${params.id}`,
      languages: {
...
export default function LineVillahermosaPage({ params }) {
  const linea = lineasVillahermosa.find((l) => l.id === params.id);
  if (!linea) {
    return (
      <main style={{ padding: '40px 20px', textAlign: 'center' }}>
        ...
        <p style={{ marginTop: '20px' }}>
          <Link href="/villahermosa" style={{ color: '#059669', textDecoration: 'underline' }}>Back to Villahermosa</Link>
        </p>
      </main>
    );
  }
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: linea.h1 || linea.nombre, item: `https://metroguia.mx/villahermosa/line/${linea.id}/` }
    ]
  };
    ...
    <main style={{ backgroundColor: '#ffffff' }}>
        ...
        <section style={{ ...containerStyles, ...sectionStyles, paddingTop: '40px' }}>
          ...
          <div style={{ backgroundColor: '#f9fafb', padding: '30px', borderRadius: '8px' }}>
            {linea.ruta_1_dia.paradas.map((parada, idx) => {
              const estacionInfo = estacionesVillahermosa.find((e) => e.slug === parada.estacion);
              return (
                <div key={idx} style={rutaParadaStyles}>
                  ...
                  <div style={{ flex: 1, color: '#374151', fontSize: '15px', lineHeight: '1.5' }}>
                    <Link href={`/villahermosa/station/${parada.estacion}`} style={{ color: linea.color, fontWeight: 'bold', textDecoration: 'underline' }}>
        ...
        <section style={{ ...containerStyles, ...sectionStyles, paddingTop: '40px' }}>
          ...
          <div style={gridStyles}>
            {linea.estaciones_turisticas.map((slugT) => {
              ...
              if (!est) return null;
              return (
                <Link key={est.slug} href={`/villahermosa/station/${est.slug}`} style={{ textDecoration: 'none' }}>
      ...
      <section style={{ ...containerStyles, paddingTop: '40px', paddingBottom: '40px', textAlign: 'center' }}>
        <div style={{ display: 'flex', gap: '10px', justifyContent: 'center', flexWrap: 'wrap' }}>
          <Link href="/villahermosa" style={buttonStyles}>Back to Villahermosa</Link>
          {lineasVillahermosa.filter(l => l.id !== linea.id).map(otherLine => (
            <Link key={otherLine.id} href={`/villahermosa/line/${otherLine.id}`} style={{ ...buttonStyles, backgroundColor: otherLine.color }}>
