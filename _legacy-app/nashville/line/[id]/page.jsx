...
export async function generateStaticParams() {
  return lineasNashville.map((linea) => ({
    id: linea.id,
  }));
}
...
export async function generateMetadata({ params }) {
  ...
  return {
    title: `Nashville Rapid Transit ${linea.colorNombre} Line — Nashville | MetroGuia`,
    description: linea.descripcion.substring(0, 160),
    openGraph: {
      title: `Nashville Rapid Transit ${linea.colorNombre} Line`,
      description: linea.descripcion.substring(0, 160),
      url: `https://metroguia.mx/nashville/line/${linea.id}`,
    },
  };
}

export default function LineNashvillePage({ params }) {
  const linea = lineasNashville.find((l) => l.id === params.id);
  ...
  return (
    <main style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
      <IntlLineSchema line={(lineasNashville.find(l => l.id === params.id)) || null} stations={((lineasNashville.find(l => l.id === params.id) || {}).estaciones || []).map(s => (typeof s === 'string' ? (estacionesNashville.find(x => x.slug === s) || { slug: s, nombre: s }) : s))} city="Nashville" citySlug="nashville" linePathPrefix="/nashville/line" stationPathPrefix="/nashville/station" country="US" systemName="WeGo Star" locale="en" />
