...
export async function generateStaticParams() {
  return lineasCampeche.map((linea) => ({ id: linea.id }));
}

export async function generateMetadata({ params }) {
  const linea = lineasCampeche.find((l) => l.id === params.id);
  ...
  return {
    ...
    openGraph: {
      title: linea.seo_title,
      description: linea.meta_description,
      url: `https://metroguia.mx/campeche/linea/${linea.id}`,
      type: 'website',
    },
    robots: { index: true, follow: true },
    alternates: {
      canonical: `https://metroguia.mx/campeche/linea/${params.id}`,
      languages: {
...
export default function LineaCampechePage({ params }) {
  const linea = lineasCampeche.find((l) => l.id === params.id);
  if (!linea) {
    return (
  ...
  const estacionesLinea = estacionesCampeche.filter((e) =>
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: linea.h1, item: `https://metroguia.mx/campeche/linea/${linea.id}/` }
    ]
  };
    ...
    <main style={{ backgroundColor: '#ffffff' }}>
      ...
      <section style={heroStyles}>
        <div style={containerStyles}>
          <h1 style={{ fontSize: '42px', fontWeight: 'bold', marginBottom: '10px' }}>{linea.h1}</h1>
          <p style={{ fontSize: '18px', opacity: '0.95', marginBottom: '15px' }}>De {linea.inicio} a {linea.fin}</p>
          <p style={{ fontSize: '14px', opacity: '0.9' }}>{linea.total} estaciones • {linea.municipios.join(', ')}</p>
        </div>
      </section>

      <section style={{ ...containerStyles, ...sectionStyles, paddingTop: '40px' }}>
        <AdBannerLazy adSlot="4434764790" format="auto" />
        <h2 style={sectionTitleStyles}>Descripción</h2>
          ...
          {linea.ruta_1_dia.paradas.map((parada, idx) => {
            ...
            return (
              <div key={idx} style={rutaParadaStyles}>
                <div style={{ minWidth: '120px', fontWeight: 'bold', color: linea.color, fontSize: '14px' }}>{parada.actividad.split(' - ')[1] || 'Parada'}</div>
                <div style={{ flex: 1, color: '#374151', fontSize: '15px', lineHeight: '1.5' }}>
                  <Link href={`/campeche/estacion/${parada.estacion}`} style={{ color: linea.color, fontWeight: 'bold', textDecoration: 'underline' }}>
      ...
      {linea.estaciones_turisticas.length > 0 && (
        <section style={{ ...containerStyles, ...sectionStyles, paddingTop: '40px' }}>
          <h2 style={sectionTitleStyles}>Estaciones Turísticas</h2>
          <div style={gridStyles}>
            {linea.estaciones_turisticas.map((slugT) => {
              const est = estacionesCampeche.find((e) => e.slug === slugT);
              if (!est) return null;
              return (
                <Link key={est.slug} href={`/campeche/estacion/${est.slug}`} style={{ textDecoration: 'none' }}>
                  <div style={{ backgroundColor: '#ffffff', border: `2px solid ${linea.color}`, borderRadius: '8px', padding: '20px', height: '100%', cursor: 'pointer' }}>
                    ...
                    <p style={{ fontSize: '15px', color: '#374151', lineHeight: '1.5' }}>{est.intro}</p>
                  </div>
                </Link>
              );
            })}
      ...
      <section style={{ ...containerStyles, ...sectionStyles, paddingTop: '40px' }}>
        ...
        <h2 style={sectionTitleStyles}>Todas las Estaciones</h2>
          ...
          {estacionesLinea.map((est, idx) => (
            <Link key={est.slug} href={`/campeche/estacion/${est.slug}`} style={{ textDecoration: 'none' }}>
              <div style={estacionCardStyles}>
                ...
                <p style={{ fontSize: '13px', color: '#6b7280', margin: 0 }}>{est.municipio} • {est.tipo_zona}</p>
              </div>
            </Link>
          ))}
        </div>
      ...
      <section style={{ ...containerStyles, ...sectionStyles, paddingTop: '40px' }}>
        ...
        <div style={gridStyles}>
          {[
            { title: 'Horarios', desc: 'De 5:30 AM a 11:00 PM. Frecuencia cada 10-15 minutos.' },
            { title: 'Tarifa', desc: 'Tren Ligero: $18 MXN. Tren Maya: desde $300 MXN según destino.' },
            { title: 'Accesibilidad', desc: 'Diseño universal en todas las estaciones. Rampas y señalización inclusiva.' },
          ].map((info, idx) => (
      ...
      <section style={{ ...containerStyles, paddingTop: '40px', paddingBottom: '40px', textAlign: 'center' }}>
        <div style={{ display: 'flex', gap: '10px', justifyContent: 'center', flexWrap: 'wrap' }}>
          ...
          {lineasCampeche.filter(l => l.id !== linea.id).map(otherLine => (
            <Link key={otherLine.id} href={`/campeche/linea/${otherLine.id}`} style={{ ...buttonStyles, backgroundColor: otherLine.color }}>
              {otherLine.h1}
            </Link>
          ))}
        </div>
