...
export async function generateStaticParams() {
  return lineasOrlando.map((linea) => ({
    id: linea.id,
  }));
}
...
export async function generateMetadata({ params }) {
  ...
  return {
    title: `Lynx ${linea.colorNombre} Line — Orlando | MetroGuia`,
    description: linea.descripcion.substring(0, 160),
    openGraph: {
      title: `Lynx ${linea.colorNombre} Line`,
      description: linea.descripcion.substring(0, 160),
      url: `https://metroguia.mx/orlando/line/${linea.id}`,
    },
  };
}

export default function LineOrlandoPage({ params }) {
  const linea = lineasOrlando.find((l) => l.id === params.id);
  ...
  return (
    <main style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
      <IntlLineSchema line={(lineasOrlando.find(l => l.id === params.id)) || null} stations={((lineasOrlando.find(l => l.id === params.id) || {}).estaciones || []).map(s => (typeof s === 'string' ? (estacionesOrlando.find(x => x.slug === s) || { slug: s, nombre: s }) : s))} city="Orlando" citySlug="orlando" linePathPrefix="/orlando/line" stationPathPrefix="/orlando/station" country="US" systemName="SunRail" locale="en" />
      {/* HERO */}
