...
export async function generateStaticParams() {
  return lineasCalgary.map((linea) => ({
    id: linea.id,
  }));
}
...
export async function generateMetadata({ params }) {
  ...
  return {
    title: `CTrain ${linea.colorNombre} Line — Calgary | MetroGuia`,
    description: linea.descripcion.substring(0, 160),
    openGraph: {
      title: `CTrain ${linea.colorNombre} Line`,
      description: linea.descripcion.substring(0, 160),
      url: `https://metroguia.mx/calgary/line/${linea.id}`,
    },
  };
}

export default function LineCalgaryPage({ params }) {
  const linea = lineasCalgary.find((l) => l.id === params.id);
  ...
  return (
    <main style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
      <IntlLineSchema line={(lineasCalgary.find(l => l.id === params.id)) || null} stations={((lineasCalgary.find(l => l.id === params.id) || {}).estaciones || []).map(s => (typeof s === 'string' ? (estacionesCalgary.find(x => x.slug === s) || { slug: s, nombre: s }) : s))} city="Calgary" citySlug="calgary" linePathPrefix="/calgary/line" stationPathPrefix="/calgary/station" country="CA" systemName="Calgary Transit CTrain" locale="en" />
      {/* HERO */}
      <section style={{ background: `linear-gradient(135deg, ${linea.color} 0%, ${linea.color}cc 100%)`, color: '#fff', padding: '60px 24px' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          <div style={{ marginBottom: '8px' }}>
            <Link href="/calgary" style={{ color: 'rgba(255,255,255,0.8)', textDecoration: 'none', fontSize: '0.9rem' }}>
              ← Calgary
            </Link>
          </div>
