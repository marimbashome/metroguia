...
export async function generateStaticParams() {
  return lineasCleveland.map((linea) => ({
    id: linea.id,
  }));
}
...
export async function generateMetadata({ params }) {
  ...
  return {
    title: `RTA Rapid Transit ${linea.colorNombre} Line — Cleveland | MetroGuia`,
    description: linea.descripcion.substring(0, 160),
    openGraph: {
      title: `RTA Rapid Transit ${linea.colorNombre} Line`,
      description: linea.descripcion.substring(0, 160),
      url: `https://metroguia.mx/cleveland/line/${linea.id}`,
    },
  };
}

export default function LineClevelandPage({ params }) {
  const linea = lineasCleveland.find((l) => l.id === params.id);
  ...
  return (
    <main style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
      <IntlLineSchema line={(lineasCleveland.find(l => l.id === params.id)) || null} stations={((lineasCleveland.find(l => l.id === params.id) || {}).estaciones || []).map(s => (typeof s === 'string' ? (estacionesCleveland.find(x => x.slug === s) || { slug: s, nombre: s }) : s))} city="Cleveland" citySlug="cleveland" linePathPrefix="/cleveland/line" stationPathPrefix="/cleveland/station" country="US" systemName="RTA Rapid Transit" locale="en" />
      {/* HERO */}
