import { trenmayaStations, STATION_DISPLAY_NAMES } from '@/data/rutas-engine'
...
export function generateStaticParams() {
  const hubs = ['cancun', 'tulum', 'chichen-itza', 'merida-oriente', 'palenque']
  const params = []
  for (const hub of hubs) {
    for (const station of trenmayaStations) {
      if (station !== hub) {
...
function isValidRouteSlug(slug) {
  ...
  const stationSet = new Set(trenmayaStations)
...
export default function RutaTrenMayaPage({ params }) {
  return <RutaClient slug={params.slug} ciudad="tren-maya" />
}
