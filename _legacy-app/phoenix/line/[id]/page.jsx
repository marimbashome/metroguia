...
export default function LinePhoenixPage({ params }) {
  ...
  return (
    <main style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
      <IntlLineSchema line={(lineasPhoenix.find(l => l.id === params.id)) || null} stations={((lineasPhoenix.find(l => l.id === params.id) || {}).estaciones || []).map(s => (typeof s === 'string' ? (estacionesPhoenix.find(x => x.slug === s) || { slug: s, nombre: s }) : s))} city="Phoenix" citySlug="phoenix" linePathPrefix="/phoenix/line" stationPathPrefix="/phoenix/station" country="US" systemName="Valley Metro Rail" locale="en" />
      {/* HERO */}
