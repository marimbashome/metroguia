...
export async function generateMetadata({ params }) {
  ...
  return {
    title: `Capital MetroRail ${linea.colorNombre} Line — Austin | MetroGuia`,
    description: linea.descripcion.substring(0, 160),
    openGraph: {
      title: `Capital MetroRail ${linea.colorNombre} Line`,
      description: linea.descripcion.substring(0, 160),
      url: `https://metroguia.mx/austin/line/${linea.id}`,
    },
  };
}

export default function LineAustinPage({ params }) {
  const linea = lineasAustin.find((l) => l.id === params.id);
  ...
  return (
    <main style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
      <IntlLineSchema line={(lineasAustin.find(l => l.id === params.id)) || null} stations={((lineasAustin.find(l => l.id === params.id) || {}).estaciones || []).map(s => (typeof s === 'string' ? (estacionesAustin.find(x => x.slug === s) || { slug: s, nombre: s }) : s))} city="Austin" citySlug="austin" linePathPrefix="/austin/line" stationPathPrefix="/austin/station" country="US" systemName="Capital MetroRail" locale="en" />
      {/* HERO */}
