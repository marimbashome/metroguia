...
export async function generateMetadata({ params }) {
  const linea = lineasMontreal.find(l => l.id === params.id);
  if (!linea) return { title: 'Line not found — MetroGuia' };
  return buildMetadata({
    lang: params.lang,
    title: `${linea.nombre || linea.name || 'Line ' + linea.id} — Montreal Transit | MetroGuia`,
    description: linea.meta_description || linea.descripcion || `${linea.nombre || linea.name || 'Line ' + linea.id} in Montreal`,
    path: `/montreal/line/${linea.id}/`,
    keywords: ['Montreal', linea.nombre || linea.name || linea.id, 'transit line'],
  });
}

export default function LineMontrealPageLang({ params }) {
  ...
  const estacionesLinea = estacionesMontreal.filter(e => {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: lineName, item: `https://metroguia.mx/${lang}/montreal/line/${linea.id}` },
    ],
  };
        ...
        <div style={{ marginBottom: '60px' }}>
          ...
          <div style={{ display: 'grid', gap: '16px' }}>
            {estacionesLinea.map((estacion, idx) => (
              <Link href={`/${lang}/montreal/station/${estacion.slug}`} key={idx} style={{ textDecoration: 'none' }}>
