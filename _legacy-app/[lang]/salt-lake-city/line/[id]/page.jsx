...
export function generateStaticParams() {
  ...
  LANGUAGES.filter(l => l !== defaultLang).forEach(lang => {
    lineasSaltLakeCity.forEach(linea => {
...
export async function generateMetadata({ params }) {
  const linea = lineasSaltLakeCity.find(l => l.id === params.id);
  if (!linea) return { title: 'Line not found — MetroGuia' };
  return buildMetadata({
    lang: params.lang,
    title: `${linea.nombre || linea.name || 'Line ' + linea.id} — Salt Lake City Transit | MetroGuia`,
    description: linea.meta_description || linea.descripcion || `${linea.nombre || linea.name || 'Line ' + linea.id} in Salt Lake City`,
    path: `/salt-lake-city/line/${linea.id}/`,
    keywords: ['Salt Lake City', linea.nombre || linea.name || linea.id, 'transit line'],
  });
}

export default function LineSaltLakeCityPageLang({ params }) {
  ...
  const lineColor = linea.color || '#0072CE';
  const lineName = linea.nombre || linea.name || `Line ${linea.id}`;
  const estacionesLinea = estacionesSaltLakeCity.filter(e => {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: lineName, item: `https://metroguia.mx/${lang}/salt-lake-city/line/${linea.id}` },
    ],
  };
        ...
        <div style={{ marginBottom: '60px' }}>
          ...
          <div style={{ display: 'grid', gap: '16px' }}>
            {estacionesLinea.map((estacion, idx) => (
              <Link href={`/${lang}/salt-lake-city/station/${estacion.slug}`} key={idx} style={{ textDecoration: 'none' }}>
