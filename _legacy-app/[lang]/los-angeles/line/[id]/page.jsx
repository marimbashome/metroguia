...
export async function generateMetadata({ params }) {
  ...
  return buildMetadata({
    lang: params.lang,
    title: `${linea.nombre || linea.name || 'Line ' + linea.id} — Los Angeles Transit | MetroGuia`,
    description: linea.meta_description || linea.descripcion || `${linea.nombre || linea.name || 'Line ' + linea.id} in Los Angeles`,
    path: `/los-angeles/line/${linea.id}/`,
    keywords: ['Los Angeles', linea.nombre || linea.name || linea.id, 'transit line'],
  });
}

export default function LineLosAngelesPageLang({ params }) {
  ...
  const lineColor = linea.color || '#E84855';
  const lineName = linea.nombre || linea.name || `Line ${linea.id}`;
  const estacionesLinea = estacionesLA.filter(e => {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: lineName, item: `https://metroguia.mx/${lang}/los-angeles/line/${linea.id}` },
    ],
  };
        ...
        <div style={{ marginBottom: '60px' }}>
          ...
          <div style={{ display: 'grid', gap: '16px' }}>
            {estacionesLinea.map((estacion, idx) => (
              <Link href={`/${lang}/los-angeles/station/${estacion.slug}`} key={idx} style={{ textDecoration: 'none' }}>
