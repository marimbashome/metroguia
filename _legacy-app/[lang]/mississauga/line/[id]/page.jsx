...
export function generateStaticParams() {
  ...
  LANGUAGES.filter(l => l !== defaultLang).forEach(lang => {
    lineasMississauga.forEach(linea => {
...
export async function generateMetadata({ params }) {
  const linea = lineasMississauga.find(l => l.id === params.id);
  if (!linea) return { title: 'Line not found — MetroGuia' };
  return buildMetadata({
    lang: params.lang,
    title: `${linea.nombre || linea.name || 'Line ' + linea.id} — Mississauga Transit | MetroGuia`,
    description: linea.meta_description || linea.descripcion || `${linea.nombre || linea.name || 'Line ' + linea.id} in Mississauga`,
    path: `/mississauga/line/${linea.id}/`,
    keywords: ['Mississauga', linea.nombre || linea.name || linea.id, 'transit line'],
  });
}

export default function LineMississaugaPageLang({ params }) {
  ...
  if (!linea) {
    return (
      <main style={{ padding: '80px 24px', textAlign: 'center', backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
        ...
        <Link href={`/${lang}/mississauga`}>
          <button style={{ marginTop: '24px', padding: '12px 24px', backgroundColor: '#00A5E3', color: '#fff', border: 'none', borderRadius: 'var(--radius)', fontSize: '1rem', fontWeight: '700', cursor: 'pointer' }}>
            ← Mississauga
          </button>
        </Link>
      </main>
  ...
  const lineColor = linea.color || '#00A5E3';
  const lineName = linea.nombre || linea.name || `Line ${linea.id}`;
  const estacionesLinea = estacionesMississauga.filter(e => {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: lineName, item: `https://metroguia.mx/${lang}/mississauga/line/${linea.id}` },
    ],
  };
        ...
        <div style={{ marginBottom: '60px' }}>
          ...
          <div style={{ display: 'grid', gap: '16px' }}>
            {estacionesLinea.map((estacion, idx) => (
              <Link href={`/${lang}/mississauga/station/${estacion.slug}`} key={idx} style={{ textDecoration: 'none' }}>
