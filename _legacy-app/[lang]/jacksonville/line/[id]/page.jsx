...
export function generateStaticParams() {
  ...
  LANGUAGES.filter(l => l !== defaultLang).forEach(lang => {
    lineasJacksonville.forEach(linea => {
...
export async function generateMetadata({ params }) {
  const linea = lineasJacksonville.find(l => l.id === params.id);
  if (!linea) return { title: 'Line not found — MetroGuia' };
  return buildMetadata({
    lang: params.lang,
    title: `${linea.nombre || linea.name || 'Line ' + linea.id} — Jacksonville Transit | MetroGuia`,
    description: linea.meta_description || linea.descripcion || `${linea.nombre || linea.name || 'Line ' + linea.id} in Jacksonville`,
    path: `/jacksonville/line/${linea.id}/`,
    keywords: ['Jacksonville', linea.nombre || linea.name || linea.id, 'transit line'],
  });
}

export default function LineJacksonvillePageLang({ params }) {
  ...
  const lineColor = linea.color || '#00687F';
  const lineName = linea.nombre || linea.name || `Line ${linea.id}`;
  const estacionesLinea = estacionesJacksonville.filter(e => {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: lineName, item: `https://metroguia.mx/${lang}/jacksonville/line/${linea.id}` },
    ],
  };
        ...
        <div style={{ marginBottom: '60px' }}>
          ...
          <div style={{ display: 'grid', gap: '16px' }}>
            {estacionesLinea.map((estacion, idx) => (
              <Link href={`/${lang}/jacksonville/station/${estacion.slug}`} key={idx} style={{ textDecoration: 'none' }}>
