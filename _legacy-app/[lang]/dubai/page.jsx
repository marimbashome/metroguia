...
export function generateStaticParams() {
  const defaultLang = getDefaultLangForCity('dubai') || 'en';
...
export async function generateMetadata({ params }) {
  return buildMetadata({
    lang: params.lang,
    title: 'Dubai Metro Guide | MetroGuia',
    description: 'UAE · 2 lines · 53 stations. Complete metro guide for tourists and commuters.',
    path: '/dubai/',
    keywords: ['dubai metro', 'dubai transit', 'dubai subway'],
  });
}

export default function DubaiPageLang({ params }) {
  const lang = params.lang;
  return (
    <main style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
      <section style={{ background: 'linear-gradient(135deg, #EF3D33 0%, #EF3D33cc 100%)', color: '#fff', padding: '80px 24px' }}>
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
          <p style={{ fontSize: '0.75rem', color: 'rgba(255,255,255,0.85)', letterSpacing: '0.15em', textTransform: 'uppercase', fontWeight: 600, marginBottom: '0.75rem' }}>🇦🇪 Dubai Metro · UAE</p>
          <h1 style={{ fontSize: 'clamp(1.75rem, 5vw, 2.75rem)', fontWeight: 800, marginBottom: '1rem' }}>Dubai Metro</h1>
          <p style={{ fontSize: '1.1rem', color: 'rgba(255,255,255,0.9)', marginBottom: '2rem' }}>2 lines · 53 stations</p>
          <SearchBar />
        </div>
      </section>
      <section style={{ padding: '3rem 1.5rem', maxWidth: 900, margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '1rem' }}>
          {lineasDubai.map(linea => (
            <Link key={linea.id} href={`/${lang}/dubai/line/${linea.id}/`} style={{ textDecoration: 'none' }}>
      ...
      <section style={{ padding: '2rem 1.5rem', maxWidth: 900, margin: '0 auto', borderTop: '1px solid var(--border)' }}>
        <Link href="/dubai/" style={{ color: 'var(--text-muted)', textDecoration: 'none', fontSize: '0.9rem' }}>View in English →</Link>
      </section>
    </main>
  );
}
