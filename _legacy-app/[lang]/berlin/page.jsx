...
export function generateStaticParams() {
  const defaultLang = getDefaultLangForCity('berlin');
...
export async function generateMetadata({ params }) {
  return buildMetadata({
    ...
    description: 'Madrid Metro guide: 9 lines, 175 stations. Sol, Gran Vía, Atocha, Retiro, Ópera. Tourist route planner.',
    path: '/berlin/',
...
export default function MadridPageLang({ params }) {
  ...
  const estacionesDestacadas = estacionesBerlin.filter(e =>
    ['berlin-sol', 'berlin-gran-via', 'berlin-atocha', 'berlin-opera', 'berlin-callao', 'berlin-retiro'].includes(e.slug)
  );
    ...
    <main style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
      ...
      <section style={{ background: 'linear-gradient(135deg, var(--surface) 0%, rgba(0,170,220,0.08) 100%)', padding: '5rem 1.5rem 3rem', borderBottom: '1px solid var(--border)' }}>
        <div className="container" style={{ maxWidth: 900, margin: '0 auto' }}>
          ...
          <p style={{ fontSize: '1.1rem', color: 'var(--text-muted)', marginBottom: '2rem' }}>9 lines · 175 stations · 155 km — Spain's largest metro network</p>
          <SearchBar />
        </div>
      </section>
      ...
      <section style={{ padding: '3rem 1.5rem', maxWidth: 900, margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(200px,1fr))', gap: '1rem' }}>
          {lineasBerlin.map(linea => (
            <Link key={linea.id} href={`/${lang}/berlin/line/${linea.id}/`} style={{ textDecoration: 'none' }}>
      ...
      <section style={{ padding: '2rem 1.5rem', maxWidth: 900, margin: '0 auto', borderTop: '1px solid var(--border)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(220px,1fr))', gap: '1rem' }}>
          {estacionesDestacadas.map(e => {
            ...
            return (
              <Link key={e.slug} href={`/${lang}/berlin/station/${e.slug}/`} style={{ textDecoration: 'none' }}>
      ...
      <section style={{ padding: '1.5rem', maxWidth: 900, margin: '0 auto', borderTop: '1px solid var(--border)' }}>
        <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>
          También disponible en: <Link href="/berlin/" style={{ color: 'var(--primary)', textDecoration: 'none' }}>Español</Link>
        </p>
      </section>
