...
export async function generateMetadata({ params }) {
  ...
  return buildMetadata({
    lang: params.lang,
    title: `${linea.nombre || linea.name || 'Line ' + linea.id} — El Paso Transit | MetroGuia`,
    description: linea.meta_description || linea.descripcion || `${linea.nombre || linea.name || 'Line ' + linea.id} in El Paso`,
    path: `/el-paso/line/${linea.id}/`,
    keywords: ['El Paso', linea.nombre || linea.name || linea.id, 'transit line'],
  });
}

export default function LineElPasoPageLang({ params }) {
  ...
  if (!linea) {
    return (
      <main style={{ padding: '80px 24px', textAlign: 'center', backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
        ...
        <Link href={`/${lang}/el-paso`}>
          <button style={{ marginTop: '24px', padding: '12px 24px', backgroundColor: '#800020', color: '#fff', border: 'none', borderRadius: 'var(--radius)', fontSize: '1rem', fontWeight: '700', cursor: 'pointer' }}>
            ← El Paso
          </button>
        </Link>
      </main>
  ...
  const lineColor = linea.color || '#800020';
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: lineName, item: `https://metroguia.mx/${lang}/el-paso/line/${linea.id}` },
    ],
  };
        ...
        <div style={{ marginBottom: '60px' }}>
          ...
          <div style={{ display: 'grid', gap: '16px' }}>
            {estacionesLinea.map((estacion, idx) => (
              <Link href={`/${lang}/el-paso/station/${estacion.slug}`} key={idx} style={{ textDecoration: 'none' }}>
