...
export function generateStaticParams() {
  ...
  LANGUAGES.filter(l => l !== defaultLang).forEach(lang => {
    lineasKansasCity.forEach(linea => {
...
export async function generateMetadata({ params }) {
  ...
  return buildMetadata({
    lang: params.lang,
    title: `${linea.nombre || linea.name || 'Line ' + linea.id} — Kansas City Transit | MetroGuia`,
    description: linea.meta_description || linea.descripcion || `${linea.nombre || linea.name || 'Line ' + linea.id} in Kansas City`,
    path: `/kansas-city/line/${linea.id}/`,
    keywords: ['Kansas City', linea.nombre || linea.name || linea.id, 'transit line'],
  });
}
...
export default function LineKansasCityPageLang({ params }) {
  ...
  const lineColor = linea.color || '#CE1141';
  const lineName = linea.nombre || linea.name || `Line ${linea.id}`;
  const estacionesLinea = estacionesKansasCity.filter(e => {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: lineName, item: `https://metroguia.mx/${lang}/kansas-city/line/${linea.id}` },
    ],
  };
        ...
        <div style={{ marginBottom: '60px' }}>
          ...
          <div style={{ display: 'grid', gap: '16px' }}>
            {estacionesLinea.map((estacion, idx) => (
              <Link href={`/${lang}/kansas-city/station/${estacion.slug}`} key={idx} style={{ textDecoration: 'none' }}>
