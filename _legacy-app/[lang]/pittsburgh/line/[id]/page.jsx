...
export function generateStaticParams() {
  ...
  LANGUAGES.filter(l => l !== defaultLang).forEach(lang => {
    lineasDetallePittsburgh.forEach(linea => {
...
export async function generateMetadata({ params }) {
  ...
  return buildMetadata({
    lang: params.lang,
    title: `${linea.nombre || linea.name || 'Line ' + linea.id} — Pittsburgh Transit | MetroGuia`,
    description: linea.meta_description || linea.descripcion || `${linea.nombre || linea.name || 'Line ' + linea.id} in Pittsburgh`,
    path: `/pittsburgh/line/${linea.id}/`,
    keywords: ['Pittsburgh', linea.nombre || linea.name || linea.id, 'transit line'],
  });
}

export default function LinePittsburghPageLang({ params }) {
  ...
  const lineColor = linea.color || '#EEB211';
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: lineName, item: `https://metroguia.mx/${lang}/pittsburgh/line/${linea.id}` },
    ],
  };
        ...
        <div style={{ marginBottom: '60px' }}>
          ...
          <div style={{ display: 'grid', gap: '16px' }}>
            {estacionesLinea.map((estacion, idx) => (
              <Link href={`/${lang}/pittsburgh/station/${estacion.slug}`} key={idx} style={{ textDecoration: 'none' }}>
