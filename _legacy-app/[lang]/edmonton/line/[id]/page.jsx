...
export async function generateMetadata({ params }) {
  const linea = lineasEdmonton.find(l => l.id === params.id);
  if (!linea) return { title: 'Line not found — MetroGuia' };
  return buildMetadata({
    lang: params.lang,
    title: `${linea.nombre || linea.name || 'Line ' + linea.id} — Edmonton Transit | MetroGuia`,
    description: linea.meta_description || linea.descripcion || `${linea.nombre || linea.name || 'Line ' + linea.id} in Edmonton`,
    path: `/edmonton/line/${linea.id}/`,
    keywords: ['Edmonton', linea.nombre || linea.name || linea.id, 'transit line'],
  });
}

export default function LineEdmontonPageLang({ params }) {
  ...
  const lineColor = linea.color || '#003087';
  const lineName = linea.nombre || linea.name || `Line ${linea.id}`;
  const estacionesLinea = estacionesEdmonton.filter(e => {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: lineName, item: `https://metroguia.mx/${lang}/edmonton/line/${linea.id}` },
    ],
  };
        ...
        <div style={{ marginBottom: '60px' }}>
          ...
          <div style={{ display: 'grid', gap: '16px' }}>
            {estacionesLinea.map((estacion, idx) => (
              <Link href={`/${lang}/edmonton/station/${estacion.slug}`} key={idx} style={{ textDecoration: 'none' }}>
