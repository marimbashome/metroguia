...
export async function generateMetadata({ params }) {
  const linea = lineasOttawa.find(l => l.id === params.id);
  if (!linea) return { title: 'Line not found — MetroGuia' };
  return buildMetadata({
    lang: params.lang,
    title: `${linea.nombre || linea.name || 'Line ' + linea.id} — Ottawa Transit | MetroGuia`,
    description: linea.meta_description || linea.descripcion || `${linea.nombre || linea.name || 'Line ' + linea.id} in Ottawa`,
    path: `/ottawa/line/${linea.id}/`,
    keywords: ['Ottawa', linea.nombre || linea.name || linea.id, 'transit line'],
  });
}

export default function LineOttawaPageLang({ params }) {
  ...
  if (!linea) {
    return (
      <main style={{ padding: '80px 24px', textAlign: 'center', backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
        ...
        <Link href={`/${lang}/ottawa`}>
          <button style={{ marginTop: '24px', padding: '12px 24px', backgroundColor: '#CF202F', color: '#fff', border: 'none', borderRadius: 'var(--radius)', fontSize: '1rem', fontWeight: '700', cursor: 'pointer' }}>
            ← Ottawa
          </button>
        </Link>
      </main>
  ...
  const lineColor = linea.color || '#CF202F';
  const lineName = linea.nombre || linea.name || `Line ${linea.id}`;
  const estacionesLinea = estacionesOttawa.filter(e => {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: lineName, item: `https://metroguia.mx/${lang}/ottawa/line/${linea.id}` },
    ],
  };
        ...
        <div style={{ marginBottom: '60px' }}>
          ...
          <div style={{ display: 'grid', gap: '16px' }}>
            {estacionesLinea.map((estacion, idx) => (
              <Link href={`/${lang}/ottawa/station/${estacion.slug}`} key={idx} style={{ textDecoration: 'none' }}>
