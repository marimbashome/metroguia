...
export async function generateMetadata({ params }) {
  const linea = lineasNewark.find(l => l.id === params.id);
  if (!linea) return { title: 'Line not found — MetroGuia' };
  return buildMetadata({
    lang: params.lang,
    title: `${linea.nombre || linea.name || 'Line ' + linea.id} — Newark Transit | MetroGuia`,
    description: linea.meta_description || linea.descripcion || `${linea.nombre || linea.name || 'Line ' + linea.id} in Newark`,
    path: `/newark/line/${linea.id}/`,
    keywords: ['Newark', linea.nombre || linea.name || linea.id, 'transit line'],
  });
}

export default function LineNewarkPageLang({ params }) {
  ...
  const lineColor = linea.color || '#E76F51';
  const lineName = linea.nombre || linea.name || `Line ${linea.id}`;
  const estacionesLinea = estacionesNewark.filter(e => {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: lineName, item: `https://metroguia.mx/${lang}/newark/line/${linea.id}` },
    ],
  };
        ...
        <div style={{ marginBottom: '60px' }}>
          ...
          <div style={{ display: 'grid', gap: '16px' }}>
            {estacionesLinea.map((estacion, idx) => (
              <Link href={`/${lang}/newark/station/${estacion.slug}`} key={idx} style={{ textDecoration: 'none' }}>
