...
export async function generateMetadata({ params }) {
  ...
  return buildMetadata({
    lang: params.lang,
    title: `${linea.nombre || linea.name || 'Line ' + linea.id} — Philadelphia Transit | MetroGuia`,
    description: linea.meta_description || linea.descripcion || `${linea.nombre || linea.name || 'Line ' + linea.id} in Philadelphia`,
    path: `/philadelphia/line/${linea.id}/`,
    keywords: ['Philadelphia', linea.nombre || linea.name || linea.id, 'transit line'],
  });
}
...
export default function LinePhiladelphiaPageLang({ params }) {
  ...
  const estacionesLinea = estacionesPhiladelphia.filter(e => {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: lineName, item: `https://metroguia.mx/${lang}/philadelphia/line/${linea.id}` },
    ],
  };
        ...
        <div style={{ marginBottom: '60px' }}>
          ...
          <div style={{ display: 'grid', gap: '16px' }}>
            {estacionesLinea.map((estacion, idx) => (
              <Link href={`/${lang}/philadelphia/station/${estacion.slug}`} key={idx} style={{ textDecoration: 'none' }}>
