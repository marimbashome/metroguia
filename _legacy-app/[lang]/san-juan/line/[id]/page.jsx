...
export async function generateMetadata({ params }) {
  const linea = lineasSanJuan.find(l => l.id === params.id);
  if (!linea) return { title: 'Line not found — MetroGuia' };
  return buildMetadata({
    lang: params.lang,
    title: `${linea.nombre || linea.name || 'Line ' + linea.id} — San Juan Transit | MetroGuia`,
    description: linea.meta_description || linea.descripcion || `${linea.nombre || linea.name || 'Line ' + linea.id} in San Juan`,
    path: `/san-juan/line/${linea.id}/`,
    keywords: ['San Juan', linea.nombre || linea.name || linea.id, 'transit line'],
  });
}

export default function LineSanJuanPageLang({ params }) {
  ...
  const lineColor = linea.color || '#E2231A';
  const lineName = linea.nombre || linea.name || `Line ${linea.id}`;
  const estacionesLinea = estacionesSanJuan.filter(e => {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: lineName, item: `https://metroguia.mx/${lang}/san-juan/line/${linea.id}` },
    ],
  };
        ...
        <div style={{ marginBottom: '60px' }}>
          ...
          <div style={{ display: 'grid', gap: '16px' }}>
            {estacionesLinea.map((estacion, idx) => (
              <Link href={`/${lang}/san-juan/station/${estacion.slug}`} key={idx} style={{ textDecoration: 'none' }}>
