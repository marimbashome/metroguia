...
export function generateStaticParams() {
  const defaultLang = getDefaultLangForCity('barcelona');
...
export async function generateMetadata({ params }) {
  return buildMetadata({
    ...
    description: 'Madrid Metro guide: 11 lines, 181 stations. Sol, Gran Vía, Atocha, Retiro, Ópera. Tourist route planner.',
    path: '/barcelona/',
...
export default function MadridPageLang({ params }) {
  ...
  const estacionesDestacadas = estacionesBarcelona.filter(e =>
    ['barcelona-sol', 'barcelona-gran-via', 'barcelona-atocha', 'barcelona-opera', 'barcelona-callao', 'barcelona-retiro'].includes(e.slug)
  );
    ...
    <main style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
      ...
      <section style={{ background: 'linear-gradient(135deg, var(--surface) 0%, rgba(0,170,220,0.08) 100%)', padding: '5rem 1.5rem 3rem', borderBottom: '1px solid var(--border)' }}>
        <div className="container" style={{ maxWidth: 900, margin: '0 auto' }}>
          ...
          <p style={{ fontSize: '1.1rem', color: 'var(--text-muted)', marginBottom: '2rem' }}>11 lines · 181 stations · 120 km — Spain's most-used metro network</p>
          <SearchBar />
        </div>
      </section>
      ...
      <section style={{ padding: '3rem 1.5rem', maxWidth: 900, margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(200px,1fr))', gap: '1rem' }}>
          {lineasBarcelona.map(linea => (
            <Link key={linea.id} href={`/${lang}/barcelona/line/${linea.id}/`} style={{ textDecoration: 'none' }}>
      ...
      <section style={{ padding: '2rem 1.5rem', maxWidth: 900, margin: '0 auto', borderTop: '1px solid var(--border)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(220px,1fr))', gap: '1rem' }}>
          {estacionesDestacadas.map(e => {
            ...
            return (
              <Link key={e.slug} href={`/${lang}/barcelona/station/${e.slug}/`} style={{ textDecoration: 'none' }}>
      ...
      <section style={{ padding: '1.5rem', maxWidth: 900, margin: '0 auto', borderTop: '1px solid var(--border)' }}>
        <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>
          También disponible en: <Link href="/barcelona/" style={{ color: 'var(--primary)', textDecoration: 'none' }}>Español</Link>
        </p>
      </section>
