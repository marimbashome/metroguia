...
export async function generateMetadata({ params }) {
  const linea = lineasBoston.find(l => l.id === params.id);
  if (!linea) return { title: 'Line not found — MetroGuia' };
  return buildMetadata({
    lang: params.lang,
    title: `${linea.nombre || linea.name || 'Line ' + linea.id} — Boston Transit | MetroGuia`,
    description: linea.meta_description || linea.descripcion || `${linea.nombre || linea.name || 'Line ' + linea.id} in Boston`,
    path: `/boston/line/${linea.id}/`,
    keywords: ['Boston', linea.nombre || linea.name || linea.id, 'transit line'],
  });
}

export default function LineBostonPageLang({ params }) {
  ...
  const lineColor = linea.color || '#DA291C';
  const lineName = linea.nombre || linea.name || `Line ${linea.id}`;
  const estacionesLinea = estacionesBoston.filter(e => {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: lineName, item: `https://metroguia.mx/${lang}/boston/line/${linea.id}` },
    ],
  };
        ...
        <div style={{ marginBottom: '60px' }}>
          ...
          <div style={{ display: 'grid', gap: '16px' }}>
            {estacionesLinea.map((estacion, idx) => (
              <Link href={`/${lang}/boston/station/${estacion.slug}`} key={idx} style={{ textDecoration: 'none' }}>
