...
export function generateStaticParams() {
  ...
  LANGUAGES.filter(l => l !== defaultLang).forEach(lang => {
    lineasDetalleCharlotte.forEach(linea => {
...
export async function generateMetadata({ params }) {
  const linea = lineasDetalleCharlotte.find(l => l.id === params.id);
  if (!linea) return { title: 'Line not found — MetroGuia' };
  return buildMetadata({
    lang: params.lang,
    title: `${linea.nombre || linea.name || 'Line ' + linea.id} — Charlotte Transit | MetroGuia`,
    description: linea.meta_description || linea.descripcion || `${linea.nombre || linea.name || 'Line ' + linea.id} in Charlotte`,
    path: `/charlotte/line/${linea.id}/`,
    keywords: ['Charlotte', linea.nombre || linea.name || linea.id, 'transit line'],
  });
}

export default function LineCharlottePageLang({ params }) {
  ...
  const lineColor = linea.color || '#1A8FDB';
  const lineName = linea.nombre || linea.name || `Line ${linea.id}`;
  const estacionesLinea = estacionesCharlotte.filter(e => {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: lineName, item: `https://metroguia.mx/${lang}/charlotte/line/${linea.id}` },
    ],
  };
        ...
        <div style={{ marginBottom: '60px' }}>
          ...
          <div style={{ display: 'grid', gap: '16px' }}>
            {estacionesLinea.map((estacion, idx) => (
              <Link href={`/${lang}/charlotte/station/${estacion.slug}`} key={idx} style={{ textDecoration: 'none' }}>
