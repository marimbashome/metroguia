...
export async function generateStaticParams() {
  return lineasDetroit.map((linea) => ({
    id: linea.id,
  }));
}

export async function generateMetadata({ params }) {
  const linea = lineasDetroit.find((l) => l.id === params.id);
  if (!linea) {
    return { title: 'Line not found', description: 'The requested line does not exist.' };
  }
  return {
    title: `QLine & People Mover ${linea.colorNombre} Line — Detroit | MetroGuia`,
    description: linea.descripcion.substring(0, 160),
    openGraph: {
      title: `QLine & People Mover ${linea.colorNombre} Line`,
      description: linea.descripcion.substring(0, 160),
      url: `https://metroguia.mx/detroit/line/${linea.id}`,
    },
  };
}

export default function LineDetroitPage({ params }) {
  const linea = lineasDetroit.find((l) => l.id === params.id);
  ...
  const estacionesLinea = estacionesDetroit.filter((e) => {
  ...
  return (
    <main style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
      <IntlLineSchema line={(lineasDetroit.find(l => l.id === params.id)) || null} stations={((lineasDetroit.find(l => l.id === params.id) || {}).estaciones || []).map(s => (typeof s === 'string' ? (estacionesDetroit.find(x => x.slug === s) || { slug: s, nombre: s }) : s))} city="Detroit" citySlug="detroit" linePathPrefix="/detroit/line" stationPathPrefix="/detroit/station" country="US" systemName="QLINE" locale="en" />
      {/* HERO */}
