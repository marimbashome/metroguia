...
const LINE_COLORS = { 'blue': '#0055A4', 'gold': '#FFB81C' };

export async function generateStaticParams() {
  return estacionesCharlotte.map((estacion) => ({
    slug: estacion.slug,
  }));
}

export async function generateMetadata({ params }) {
  const estacion = estacionesCharlotte.find((e) => e.slug === params.slug);
  if (!estacion) {
    return {
  ...
  return {
    ...
    openGraph: {
      title: estacion.seo_title,
      description: estacion.meta_description,
      url: `https://metroguia.mx/charlotte/station/${estacion.slug}`,
    },
  };
}

export default function StationCharlottePage({ params }) {
  const estacion = estacionesCharlotte.find((e) => e.slug === params.slug);
  ...
  const colorLinea = LINE_COLORS[lineaArray[0]] || '#0055A4';
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: `Line ${lineaId}`, item: `https://metroguia.mx/charlotte/line/${lineaId}/` },
      { '@type': 'ListItem', position: 4, name: estacion.nombre, item: `https://metroguia.mx/charlotte/station/${estacion.slug}` },
    ],
  };
  ...
  const transitStationSchema = {
    ...
    address: { '@type': 'PostalAddress', addressLocality: 'Charlotte', addressRegion: 'North Carolina', addressCountry: 'US' },
    geo: { '@type': 'GeoCoordinates', latitude: estacion.lat || 35.2271, longitude: estacion.lng || -80.8431 },
  ...
  const faqItems = [];
  faqItems.push({ '@type': 'Question', name: `How do I get to ${estacion.nombre} station?`, acceptedAnswer: { '@type': 'Answer', text: `${estacion.nombre} is served by the LYNX ${lineaId} Line. Use the MetroGuia trip planner for directions.` } });
  ...
  faqItems.push({ '@type': 'Question', name: `What are the hours and fares?`, acceptedAnswer: { '@type': 'Answer', text: `LYNX operates 5:00 AM to 11:00 PM. Fares: $2.20 USD.` } });
      ...
      <section style={{ background: `linear-gradient(135deg, ${colorLinea} 0%, ${colorLinea}cc 100%)`, color: '#fff', padding: '60px 24px' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          <div style={{ marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '12px' }}>
            <span style={{ width: '48px', height: '48px', backgroundColor: 'rgba(255,255,255,0.3)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '28px', fontWeight: '700', textTransform: 'uppercase' }}>
              {lineaId[0]}
            </span>
            <span style={{ fontSize: '1rem', fontWeight: '500' }}>Charlotte</span>
          </div>
          ...
          <div>
            ...
            <div style={{ backgroundColor: 'var(--surface)', padding: '24px', borderRadius: 'var(--radius)', borderTop: `4px solid var(--warning)` }}>
              ...
              <p style={{ fontSize: '0.9rem', color: 'var(--text-muted)', margin: '0 0 12px 0', lineHeight: '1.5' }}>
                Buy single tickets via GoTriangle app or at station kiosks.
              </p>
              <Link href="https://www.catslynx.com/" target="_blank" style={{ textDecoration: 'none' }}>
