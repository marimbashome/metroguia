...
export async function generateMetadata({ params }) {
  ...
  return { title: l.seo_title, description: l.meta_description, openGraph: { title: l.seo_title, url: `https://metroguia.mx/dubai/line/${l.id}/` }, alternates: { canonical: `https://metroguia.mx/dubai/line/${l.id}/` } };
}

export default function DubaiLine({ params }) {
  const linea = lineasDubai.find((l) => l.id === params.id);
  if (!linea) return <main style={{ padding: '80px 24px', textAlign: 'center' }}><h1>Line not found</h1><Link href="/dubai">Back</Link></main>;
  const estacionesLinea = linea.estaciones
    ? linea.estaciones.map(slug => estacionesDubai.find(e => e.slug === slug)).filter(Boolean)
      ...
      <section style={{ maxWidth: 1000, margin: '0 auto', padding: '40px 24px' }}>
        ...
        {linea.ruta_1_dia && (
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderLeft: `4px solid ${linea.color}`, borderRadius: 'var(--radius)', padding: '1.5rem', marginBottom: 32 }}>
            ...
            <div style={{ display: 'grid', gap: 10 }}>{linea.ruta_1_dia.paradas.map((p, i) => <div key={i} style={{ display: 'flex', gap: 12 }}><div style={{ width: 24, height: 24, borderRadius: '50%', background: linea.color, color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.75rem', fontWeight: 700, flexShrink: 0 }}>{i+1}</div><div><Link href={`/dubai/station/${p.estacion}/`} style={{ fontWeight: 600, color: linea.color, textDecoration: 'none', fontSize: '0.9rem' }}>{p.estacion.replace('dubai-', '').replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase())}</Link><p style={{ margin: '2px 0 0', fontSize: '0.85rem', color: 'var(--text-muted)' }}>{p.actividad}</p></div></div>)}</div>
          </div>
        )}
        {estacionesLinea.length > 0 && <div><h2 style={{ fontWeight: 700, fontSize: '1.2rem', marginBottom: 16 }}>Stations on this line</h2><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '0.75rem' }}>{estacionesLinea.map(e => <Link key={e.slug} href={`/dubai/station/${e.slug}/`} style={{ textDecoration: 'none' }}><div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: '0.75rem 1rem', display: 'flex', alignItems: 'center', gap: 8 }}><div style={{ width: 8, height: 8, borderRadius: '50%', background: linea.color, flexShrink: 0 }} /><p style={{ margin: 0, fontWeight: 600, fontSize: '0.9rem' }}>{e.nombre}</p></div></Link>)}</div></div>}
      </section>
      <section style={{ padding: '24px', maxWidth: 1000, margin: '0 auto' }}><Link href="/dubai/" style={{ color: 'var(--text-muted)', textDecoration: 'none', fontSize: '0.9rem' }}>← Back to Dubai Metro</Link></section>
      <AdBannerLazy />
    </main>
  );
}
