...
const LINE_COLORS = {"capital":"#003DA5","valley":"#7AB648"};

export async function generateStaticParams() {
  return estacionesEdmonton.map((estacion) => ({
    slug: estacion.slug,
  }));
}
...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    openGraph: {
      title: estacion.seo_title,
      description: estacion.meta_description,
      url: `https://metroguia.mx/edmonton/station/${estacion.slug}`,
    },
  };
}

export default function StationEdmontonPage({ params }) {
  const estacion = estacionesEdmonton.find((e) => e.slug === params.slug);
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: `Line ${lineaId}`, item: `https://metroguia.mx/edmonton/line/${lineaId}/` },
      { '@type': 'ListItem', position: 4, name: estacion.nombre, item: `https://metroguia.mx/edmonton/station/${estacion.slug}/` },
    ]
  };
      ...
      <section style={{ background: `linear-gradient(135deg, ${colorLinea} 0%, ${colorLinea}cc 100%)`, color: '#fff', padding: '60px 24px' }}>
        <div style={{ maxWidth: '1000px', margin: '0 auto' }}>
          ...
          <p style={{ fontSize: '1.125rem', opacity: 0.9, margin: 0 }}>
            {lineaArray.map(l => `${l.charAt(0).toUpperCase() + l.slice(1)} Line`).join(' · ')} — Edmonton LRT
          </p>
        </div>
      </section>
      ...
      <section style={{ padding: '60px 24px' }}>
        <div style={{ maxWidth: '800px', margin: '0 auto' }}>
          ...
          <div style={{ marginTop: '48px', textAlign: 'center' }}>
            <Link href={`/edmonton/line/${lineaId}`}>
