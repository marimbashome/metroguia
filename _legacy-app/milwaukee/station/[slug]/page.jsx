...
const LINE_COLORS = {"lakefront":"#FFC72C","omni":"#00A650"};

export async function generateStaticParams() {
  return estacionesMilwaukee.map((estacion) => ({
    slug: estacion.slug,
  }));
}
...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    openGraph: {
      title: estacion.seo_title,
      description: estacion.meta_description,
      url: `https://metroguia.mx/milwaukee/station/${estacion.slug}`,
    },
  };
}

export default function StationMilwaukeePage({ params }) {
  const estacion = estacionesMilwaukee.find((e) => e.slug === params.slug);
  ...
  const colorLinea = LINE_COLORS[lineaArray[0]] || '#FFC72C';
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: `Line ${lineaId}`, item: `https://metroguia.mx/milwaukee/line/${lineaId}/` },
      { '@type': 'ListItem', position: 4, name: estacion.nombre, item: `https://metroguia.mx/milwaukee/station/${estacion.slug}/` },
    ]
  };
  ...
  const stationSchema = {
    ...
    address: { '@type': 'PostalAddress', addressLocality: 'Milwaukee', addressRegion: 'Wisconsin', addressCountry: 'US' },
  };
      ...
      <section style={{ background: `linear-gradient(135deg, ${colorLinea} 0%, ${colorLinea}cc 100%)`, color: '#000', padding: '60px 24px' }}>
        <div style={{ maxWidth: '1000px', margin: '0 auto' }}>
          <div style={{ marginBottom: '8px' }}>
            <Link href="/milwaukee" style={{ color: 'rgba(0,0,0,0.7)', textDecoration: 'none', fontSize: '0.9rem' }}>
              ← Milwaukee
            </Link>
          </div>
          ...
          <p style={{ fontSize: '1.125rem', opacity: 0.85, margin: 0 }}>
            {lineaArray.map(l => `${l.charAt(0).toUpperCase() + l.slice(1)} Line`).join(' · ')} — Milwaukee Streetcar
          </p>
        </div>
      </section>
      ...
      <section style={{ padding: '60px 24px' }}>
        <div style={{ maxWidth: '800px', margin: '0 auto' }}>
          ...
          <div style={{ marginTop: '48px', textAlign: 'center' }}>
            <Link href={`/milwaukee/line/${lineaId}`}>
