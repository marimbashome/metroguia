...
export default function LinePittsburghPage({ params }) {
  ...
  return (
    <main style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
      <IntlLineSchema line={(lineasDetallePittsburgh.find(l => l.id === params.id)) || null} stations={((lineasDetallePittsburgh.find(l => l.id === params.id) || {}).estaciones || []).map(s => (typeof s === 'string' ? (estacionesPittsburgh.find(x => x.slug === s) || { slug: s, nombre: s }) : s))} city="Pittsburgh" citySlug="pittsburgh" linePathPrefix="/pittsburgh/line" stationPathPrefix="/pittsburgh/station" country="US" systemName="Pittsburgh Light Rail" locale="en" />
      {/* HERO */}
