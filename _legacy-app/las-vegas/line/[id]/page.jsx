...
export async function generateStaticParams() {
  return lineasLasVegas.map((linea) => ({
    id: linea.id,
  }));
}
...
export async function generateMetadata({ params }) {
  ...
  return {
    title: `Las Vegas Monorail ${linea.colorNombre} Line — Las Vegas | MetroGuia`,
    description: linea.descripcion.substring(0, 160),
    openGraph: {
      title: `Las Vegas Monorail ${linea.colorNombre} Line`,
      description: linea.descripcion.substring(0, 160),
      url: `https://metroguia.mx/las-vegas/line/${linea.id}`,
    },
  };
}

export default function LineLasVegasPage({ params }) {
  const linea = lineasLasVegas.find((l) => l.id === params.id);
  ...
  return (
    <main style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
      <IntlLineSchema line={(lineasLasVegas.find(l => l.id === params.id)) || null} stations={((lineasLasVegas.find(l => l.id === params.id) || {}).estaciones || []).map(s => (typeof s === 'string' ? (estacionesLasVegas.find(x => x.slug === s) || { slug: s, nombre: s }) : s))} city="Las Vegas" citySlug="las-vegas" linePathPrefix="/las-vegas/line" stationPathPrefix="/las-vegas/station" country="US" systemName="Las Vegas Monorail" locale="en" />
      {/* HERO */}
